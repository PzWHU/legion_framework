...
template <typename T>
void RosMessageManager<T>::Init(T* t) {
  is_init_ = false;
  instance_ = t;

  routing_request_pub_ = nh_.advertise<::ros_interface::RoutingRequest>(
      "/legion_bridge/RoutingRequest", 10);

  obu_cmd_msg_pub_ =
      nh_.advertise<::ros_interface::ObuCmdMsg>("/legion_bridge/ObuCmdMsg", 10);

  vehicle_task_pub_ = nh_.advertise<::ros_interface::VehicleTask>(
      "/legion_bridge/VehicleTask", 10);

  control_command_output_pub_ = nh_.advertise<::ros_interface::ControlCommand>(
      "/legion_bridge/TeleopControlCommand", 10);

  faults_pub_ =
      nh_.advertise<::ros_interface::Faults>("/legion_bridge/Faults", 10);

  control_command_input_sub_ =
      nh_.subscribe("/control/ControlCommand", 30,
                    &RosMessageManager::HandleControlCommandInputMessage, this);

  chassis_sub_ = nh_.subscribe("/drivers/canbus/Chassis", 30,
                               &RosMessageManager::HandleChassisMessage, this);

  events_sub_ = nh_.subscribe("/drivers/canbus/Events", 30,
                              &RosMessageManager::HandleEventsMessage, this);

  vehicle_sub_ = nh_.subscribe("/drivers/canbus/Vehicle", 30,
                               &RosMessageManager::HandleVehicleMessage, this);
...
template <typename T>
void RosMessageManager<T>::PublishRoutingRequest(
    legion::interface::RoutingRequest msg) {
  if (is_init_ == false) return;
  ::ros_interface::RoutingRequest routing_request;
  MESSAGE_HEADER_ASSIGN(std_msgs, routing_request)
  ...
  std::vector<ros_interface::KeyPoint> ros_key_point_list_vec;
  ...
  for (auto it_key_point_list_vec : legion_key_point_list_vec) {
    ::ros_interface::KeyPoint routing_request_key_point;
...
template <typename T>
void RosMessageManager<T>::PublishObuCmdMsg(legion::interface::ObuCmdMsg msg) {
  if (is_init_ == false) return;
  ::ros_interface::ObuCmdMsg obu_cmd_msg;
  MESSAGE_HEADER_ASSIGN(std_msgs, obu_cmd_msg)
  obu_cmd_msg.id = msg.id();
  obu_cmd_msg.name = msg.name();
  std::vector<ros_interface::ObuCmd> ros_obu_cmd_list_vec;
  ...
  for (auto it_obu_cmd_list_vec : legion_obu_cmd_list_vec) {
    ::ros_interface::ObuCmd obu_cmd_msg_obu_cmd;
...
template <typename T>
void RosMessageManager<T>::PublishVehicleTask(
    legion::interface::VehicleTask msg) {
  if (is_init_ == false) return;
  ::ros_interface::VehicleTask vehicle_task;
  MESSAGE_HEADER_ASSIGN(std_msgs, vehicle_task)
...
template <typename T>
void RosMessageManager<T>::PublishControlCommandOutput(
...
void RosMessageManager<T>::PublishFaults(legion::interface::Faults msg) {
  ...
  FAULTS_PARSER(ros, faults)

  faults_pub_.publish(faults);
}

template <typename T>
void RosMessageManager<T>::HandleControlCommandInputMessage(
    const ros_interface::ControlCommand& msg_obj) {
  if (is_init_ == false) return;
  const ros_interface::ControlCommand* msg_obj_ptr = &msg_obj;
  ros_interface::ControlCommand* msg =
      const_cast<ros_interface::ControlCommand*>(msg_obj_ptr);
...
template <typename T>
void RosMessageManager<T>::HandleEventsMessage(
    const ros_interface::Events& msg_obj) {
  if (is_init_ == false) return;
  const ros_interface::Events* msg_obj_ptr = &msg_obj;
  ros_interface::Events* msg = const_cast<ros_interface::Events*>(msg_obj_ptr);
...
template <typename T>
void RosMessageManager<T>::HandleVehicleMessage(
    const ros_interface::Vehicle& msg_obj) {
  if (is_init_ == false) return;
  const ros_interface::Vehicle* msg_obj_ptr = &msg_obj;
  ros_interface::Vehicle* msg =
      const_cast<ros_interface::Vehicle*>(msg_obj_ptr);
