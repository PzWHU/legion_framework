...
template <typename T> void DdsMessageManager<T>::Init(T* t) {
  ...
  prediction_obstacles_writer_ =
      publisher_->create_datawriter(prediction_obstacles_topic_, wqos, nullptr);
  ...
  faults_topic_ = participant_->create_topic(
      "rt/prediction/Faults", faults_type_->get_type_name(), TOPIC_QOS_DEFAULT);
  ...
  mm_obstacle_list_type_.reset(new eprosima::fastdds::dds::TypeSupport(
      new ros2_interface::msg::ObstacleListPubSubType()));
  mm_obstacle_list_type_->register_type(participant_);

  mm_obstacle_list_topic_ = participant_->create_topic(
      "rt/perception/fusion/motion_manager/MMObstacleList",
      mm_obstacle_list_type_->get_type_name(), TOPIC_QOS_DEFAULT);
  ...
  traffic_events_local_type_.reset(new eprosima::fastdds::dds::TypeSupport(
      new ros2_interface::msg::TrafficEventsPubSubType()));
  traffic_events_local_type_->register_type(participant_);

  traffic_events_local_topic_ = participant_->create_topic(
      "rt/routing/TrafficEventsLocal",
      traffic_events_local_type_->get_type_name(), TOPIC_QOS_DEFAULT);
  ...
  mf_lane_list_type_.reset(new eprosima::fastdds::dds::TypeSupport(
      new ros2_interface::msg::LaneListPubSubType()));
  mf_lane_list_type_->register_type(participant_);
  mf_lane_list_topic_ = participant_->create_topic(
      "rt/MFLaneList", mf_lane_list_type_->get_type_name(), TOPIC_QOS_DEFAULT);
...
template <typename T>
bool DdsMessageManager<T>::Activate()
{
...
void DdsMessageManager<T>::TaskStart()
{
  ...
  local_routing_response_reader_ = subscriber_->create_datareader(local_routing_response_topic_, rqos, this);

  location_reader_ = subscriber_->create_datareader(location_topic_, rqos, this);

  traffic_events_local_reader_ = subscriber_->create_datareader(traffic_events_local_topic_, rqos, this);

  adc_trajectory_reader_ = subscriber_->create_datareader(adc_trajectory_topic_, rqos, this);

  traffic_events_local_reader_ = subscriber_->create_datareader(mf_lane_list_topic_, rqos, this);

  mm_obstacle_list_reader_ = subscriber_->create_datareader(mm_obstacle_list_topic_, rqos, this);

  map_2local_tf_reader_ = subscriber_->create_datareader(map_2local_tf_topic_, rqos, this);
...
template <typename T>
void DdsMessageManager<T>::TaskStop()
{
    ...
    std::cout<<"start deactivate"<<std::endl;
...
template <typename T>
void DdsMessageManager<T>::PublishPredictionObstacles(
    legion::interface::PredictionObstacles msg) {
  if (is_init_ == false)
    return;
  ros2_interface::msg::PredictionObstacles prediction_obstacles;
  MESSAGE_DDS_HEADER_ASSIGN(ros2_interface::msg, prediction_obstacles)
  std::vector<ros2_interface::msg::PredictionObstacle>
      dds_prediction_obstacles_vector;
  ...
  for (auto it_prediction_obstacles_vector :
    ...
    std::vector<ros2_interface::msg::Point3D> dds_polygon_point;
    std::vector<legion::interface::Point3D> legion_polygon_point;
    ...
    for (auto it_polygon_point : legion_polygon_point) {
      ...
      prediction_obstacles_prediction_obstacle_perception_obstacle_point_3d
          .x() = it_polygon_point.x();
      prediction_obstacles_prediction_obstacle_perception_obstacle_point_3d
          .y() = it_polygon_point.y();
      prediction_obstacles_prediction_obstacle_perception_obstacle_point_3d
          .z() = it_polygon_point.z();
      dds_polygon_point.emplace_back(
          prediction_obstacles_prediction_obstacle_perception_obstacle_point_3d);
    }
    prediction_obstacles_prediction_obstacle_perception_obstacle
        .polygon_point() = dds_polygon_point;
    prediction_obstacles_prediction_obstacle_perception_obstacle
        .tracking_time() =
    ...
    prediction_obstacles_prediction_obstacle_perception_obstacle
        .anchor_point() =
        prediction_obstacles_prediction_obstacle_perception_obstacle_anchor_point;
    std::vector<ros2_interface::msg::Point3D> dds_bounding_box;
    std::vector<legion::interface::Point3D> legion_bounding_box;
    ...
    for (auto it_bounding_box : legion_bounding_box) {
      ...
      prediction_obstacles_prediction_obstacle_perception_obstacle_point_3d
          .z() = it_bounding_box.z();
      dds_bounding_box.emplace_back(
          prediction_obstacles_prediction_obstacle_perception_obstacle_point_3d);
    }
    prediction_obstacles_prediction_obstacle_perception_obstacle
        .bounding_box() = dds_bounding_box;
    ...
    std::vector<ros2_interface::msg::TrajectoryInPrediction> dds_trajectory;
    std::vector<legion::interface::TrajectoryInPrediction> legion_trajectory;
    ...
    for (auto it_trajectory : legion_trajectory) {
      ...
      prediction_obstacles_prediction_obstacle_trajectory_in_prediction
          .trajectory_points() = dds_trajectory_points;
      dds_trajectory.emplace_back(
          prediction_obstacles_prediction_obstacle_trajectory_in_prediction);
    }
    prediction_obstacles_prediction_obstacle.trajectory() = dds_trajectory;
    ...
    dds_prediction_obstacles_vector.emplace_back(
        prediction_obstacles_prediction_obstacle);
  }
  prediction_obstacles.prediction_obstacles() = dds_prediction_obstacles_vector;
  ...
  prediction_obstacles_writer_->write(&prediction_obstacles);
}
...
void DdsMessageManager<T>::HandleADCTrajectoryMessage(
  ...
  legion::interface::ADCTrajectory adc_trajectory;
  ...
  for (auto it_trajectory_points : msg->trajectory_points()) {
    legion::interface::TrajectoryPoint adc_trajectory_trajectory_point;
    legion::interface::PathPoint adc_trajectory_trajectory_point_path_point;
    ...
    adc_trajectory_trajectory_point.set_gear(
        (legion::common::GearPosition)it_trajectory_points.gear());
  ...
  adc_trajectory.set_car_action(
      (legion::interface::ADCTrajectory::CarAction)msg->car_action());
  ...
  adc_trajectory.set_right_of_way_status(
      (legion::interface::ADCTrajectory::RightOfWayStatus)
          msg->right_of_way_status());
  legion::interface::RSSInfo adc_trajectory_rss_info;
...
template <typename T>
void DdsMessageManager<T>::HandleMMObstacleListMessage(
    const ros2_interface::msg::ObstacleList* msg) {
  legion::interface::ObstacleList obstacle_list;
  MESSAGE_DDS_HEADER_PARSER(obstacle_list)
  ...
  for (auto it_obstacle : msg->obstacle()) {
    ...
    obstacle_list_obstacle_create_time.set_nsec(
    ...
    obstacle_list_obstacle_center_pos_abs.set_x(
        it_obstacle.center_pos_abs().x());
    obstacle_list_obstacle_center_pos_abs.set_y(
        it_obstacle.center_pos_abs().y());
    obstacle_list_obstacle_center_pos_abs.set_z(
    ...
    obstacle_list_obstacle_bbox2d_rear.set_xmin(
        it_obstacle.bbox2d_rear().xmin());
    obstacle_list_obstacle_bbox2d_rear.set_ymin(
        it_obstacle.bbox2d_rear().ymin());
    obstacle_list_obstacle_bbox2d_rear.set_xmax(
        it_obstacle.bbox2d_rear().xmax());
    obstacle_list_obstacle_bbox2d_rear.set_ymax(
...
template <typename T>
void DdsMessageManager<T>::HandleTrafficEventsLocalMessage(
...
template <typename T>
void DdsMessageManager<T>::HandleMFLaneListMessage(
  ...
  for (auto it_camera_laneline : msg->camera_laneline()) {
    ...
    lane_list_lane_line.set_lane_color((legion::interface::LaneLine::LaneColor)
                                           it_camera_laneline.lane_color());
...
void DdsMessageManager<T>::on_data_available(
  ...
  if (!reader->get_topicdescription()->get_name().compare(
    ...
    ros2_interface::msg::ObstacleList msg;
    eprosima::fastdds::dds::SampleInfo info;
    if (mm_obstacle_list_reader_->take_next_sample(&msg, &info) ==
        ReturnCode_t::RETCODE_OK) {
      HandleMMObstacleListMessage(&msg);
    }
  } else if (!reader->get_topicdescription()->get_name().compare(
                 "rt/localization/local_map_location/Map2LocalTF")) {
    ...
    eprosima::fastdds::dds::SampleInfo info;
    if (traffic_events_local_reader_->take_next_sample(&msg, &info) ==
        ReturnCode_t::RETCODE_OK) {
      HandleTrafficEventsLocalMessage(&msg);
    }
  } else if (!reader->get_topicdescription()->get_name().compare(
                 "rt/local_map/LocalRoutingResponse")) {
    ...
    eprosima::fastdds::dds::SampleInfo info;
    if (mf_lane_list_reader_->take_next_sample(&msg, &info) ==
        ReturnCode_t::RETCODE_OK) {
      HandleMFLaneListMessage(&msg);
    }
  }
  return;
}
