...
template <typename T> void RosMessageManager<T>::Init(T* t) {
  ...
  prediction_obstacles_pub_ =
      nh_.advertise<::ros_interface::PredictionObstacles>(
          "/prediction/PredictionObstacles", 10);

  faults_pub_ =
      nh_.advertise<::ros_interface::Faults>("/prediction/Faults", 10);

  obu_cmd_msg_sub_ =nh_.subscribe("/vui_client/ObuCmdMsg", 30,&RosMessageManager::HandleObuCmdMsgMessage, this);
...
template <typename T>
void RosMessageManager<T>::TaskStart()
{
    ...
    adc_trajectory_sub_ =
        nh_.subscribe("/planning/ADCTrajectory", 30,
                        &RosMessageManager::HandleADCTrajectoryMessage, this);

    obstacle_list_sub_ =
        nh_.subscribe("/perception/fusion/motion_manager/MMObstacleList", 30,
                        &RosMessageManager::HandleObstacleListMessage, this);
...
template <typename T>
void RosMessageManager<T>::TaskStop()
{
    if(is_active_==false)
    {
        return;
    ...
    adc_trajectory_sub_.shutdown();
    lane_list_sub_.shutdown();
    obstacle_list_sub_.shutdown();
...
template <typename T>
void RosMessageManager<T>::PublishPredictionObstacles(
    legion::interface::PredictionObstacles msg) {
  if (is_init_ == false)
    return;
  ::ros_interface::PredictionObstacles prediction_obstacles;
  MESSAGE_HEADER_ASSIGN(std_msgs, prediction_obstacles)
  std::vector<ros_interface::PredictionObstacle>
  ...
  for (auto it_prediction_obstacles_vector :
    ...
    std::vector<ros_interface::Point3D> ros_polygon_point;
    ...
    prediction_obstacles_prediction_obstacle_perception_obstacle.anchor_point =
        prediction_obstacles_prediction_obstacle_perception_obstacle_anchor_point;
    std::vector<ros_interface::Point3D> ros_bounding_box;
    ...
    std::vector<ros_interface::TrajectoryInPrediction> ros_trajectory;
...
template <typename T>
void RosMessageManager<T>::HandleADCTrajectoryMessage(
    const ros_interface::ADCTrajectory& msg_obj) {
  if (is_active_ == false)
    return;
  const ros_interface::ADCTrajectory* msg_obj_ptr = &msg_obj;
  ros_interface::ADCTrajectory* msg =
      const_cast<ros_interface::ADCTrajectory*>(msg_obj_ptr);
...
template <typename T>
void RosMessageManager<T>::HandleObstacleListMessage(
