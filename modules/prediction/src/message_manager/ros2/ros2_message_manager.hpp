...
template <typename T>
Ros2MessageManager<T>::Ros2MessageManager() : Node{"prediction"} {}
template <typename T> void Ros2MessageManager<T>::Init(T* t) {
  ...
  prediction_obstacles_pub_ =
      create_publisher<::ros2_interface::msg::PredictionObstacles>(
  ...
  adc_trajectory_sub_ = create_subscription<ADCTrajectory>(
      "/planning/ADCTrajectory", 
      ROS2_QOS_LARGE_DATA(),
      [this](const ros2_interface::msg::ADCTrajectory::SharedPtr msg) {
        Ros2MessageManager::HandleADCTrajectoryMessage(msg);
      },
      SubAllocT{});

  obstacle_list_sub_ = create_subscription<ObstacleList>(
      "/perception/fusion/motion_manager/MMObstacleList", 
      ROS2_QOS_PERCEPTION(),
      [this](const ros2_interface::msg::ObstacleList::SharedPtr msg) {
        Ros2MessageManager::HandleObstacleListMessage(msg);
      },
      SubAllocT{});
...
void Ros2MessageManager<T>::PublishPredictionObstacles(
  ...
  for (auto it_prediction_obstacles_vector :
    ...
    std::vector<ros2_interface::msg::Point3D> ros_polygon_point;
    ...
    for (auto it_polygon_point : legion_polygon_point) {
      ...
      prediction_obstacles_prediction_obstacle_perception_obstacle_point_3d.z =
          it_polygon_point.z();
      ros_polygon_point.emplace_back(
          prediction_obstacles_prediction_obstacle_perception_obstacle_point_3d);
    }
    ...
    prediction_obstacles_prediction_obstacle_perception_obstacle.anchor_point =
        prediction_obstacles_prediction_obstacle_perception_obstacle_anchor_point;
    std::vector<ros2_interface::msg::Point3D> ros_bounding_box;
    ...
    std::vector<ros2_interface::msg::TrajectoryInPrediction> ros_trajectory;
    ...
    for (auto it_trajectory : legion_trajectory) {
      ...
      prediction_obstacles_prediction_obstacle_trajectory_in_prediction
          .trajectory_points = ros_trajectory_points;
      ros_trajectory.emplace_back(
          prediction_obstacles_prediction_obstacle_trajectory_in_prediction);
    }
    prediction_obstacles_prediction_obstacle.trajectory = ros_trajectory;
    ...
    ros_prediction_obstacles_vector.emplace_back(
        prediction_obstacles_prediction_obstacle);
  }
  prediction_obstacles.prediction_obstacles = ros_prediction_obstacles_vector;
  ...
  prediction_obstacles_pub_->publish(prediction_obstacles);
}
...
template <typename T>
void Ros2MessageManager<T>::HandleADCTrajectoryMessage(
    const ros2_interface::msg::ADCTrajectory::SharedPtr msg_obj_ptr) {
  if (is_init_ == false)
    return;
  std::shared_ptr<ros2_interface::msg::ADCTrajectory> msg =
      std::const_pointer_cast<ros2_interface::msg::ADCTrajectory>(msg_obj_ptr);

  legion::interface::ADCTrajectory adc_trajectory;
  MESSAGE_HEADER_ROS2_PARSER(adc_trajectory)
...
template <typename T>
void Ros2MessageManager<T>::HandleObstacleListMessage(
