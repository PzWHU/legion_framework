...
void Prediction::Init() {
  ...
  // step3 配置文件初始化
  {
    prediction_conf_ = std::make_shared<legion::prediction::PredictionConf>();
...
void Prediction::TaskActivate() 
{
  ...
  std::cout<<"===================function activate=================="<<std::endl;
...
void Prediction::TaskStop() 
{
  ...
  }
  {
    ...
    obstacle_list_ = legion::interface::ObstacleList();
    odometry_ = legion::interface::Odometry();
...
void Prediction::HandleLocation(legion::interface::Location location) {
  if (is_init_ == false) {
    return;
  }
  if (function_activation_==false)
...
void Prediction::HandleOdometry(legion::interface::Odometry odometry) {
  if (is_init_ == false) {
      return;
  }
  if (function_activation_==false)
...
void Prediction::HandleObuCmdMsg(legion::interface::ObuCmdMsg obu_cmd_msg) {
  if (is_init_ == false) {
    return;
  }
...
void Prediction::HandleLaneList(legion::interface::LaneList lane_list) {
  if (is_init_ == false) {
    return;
  }
  if (function_activation_==false)
...
void Prediction::ComputePredictionCommandOnTimer() {
  ...
  if (!status.ok() || !map_loaded_) {
    ...
    {
      // time2=legion::common::TimeTool::NowToSeconds();
      // std::cout<<"1111111111111111111 pred_time  :   "<<time2-time1<<"     seconds "<<std::endl<<std::endl;
      legion::interface::Header header;
    ...
    // std::cout<<"222222222222222222222 pred_time  :   "<<time3-time2<<"     seconds "<<std::endl<<std::endl;
      ...
      // {
        ...
        // double time4=legion::common::TimeTool::NowToSeconds();
        // double time3=legion::common::TimeTool::TimeStruct2S(obstacle_list_.header().stamp());
        // std::cout<<std::setprecision(17)<<time4<<"  "<<time1<<" pred_time = "<<time4-time1<<"  seconds "<<std::endl;
        // std::cout<<"33333333333333333333 pred_time  :   "<<time4-time3<<"     seconds "<<std::endl<<std::endl;
        PublishPredictionObstacles(prediction_obstacles_);
        // double time5=legion::common::TimeTool::NowToSeconds();
        // std::cout<<"44444444444444444 pred_time  :   "<<time5-time4<<"     seconds "<<std::endl<<std::endl;
      // }
    }
  }
}
...
void Prediction::FaultMonitorInit() {
  legion::interface::FaultCodeCallback sendheart_callback_func = std::bind(&Prediction::PublishFaults, this);
