...
namespace legion
{
  namespace prediction
  {
    ...
    double cs = s[0];
    legion::prediction::LanePointInner smooth_point;
    ...
    void VectorMapPredictor::PredictLocalMap(const legion::interface::RoutingResponse &routing)
    {
      // process routing
      // 平滑并插值后的中心线跟roting的lane_list 数量相等
      std::vector<std::vector<legion::prediction::LanePointInner>> smooth_lines;
      std::vector<legion::prediction::LanePointInner> smooth_line;
    ...
    bool VectorMapPredictor::PredictVehicleLocalMap(const std::vector<std::vector<legion::prediction::LanePointInner>> &smooth_lines)
    {
      std::vector<legion::prediction::PredictionTrajectory> trajectories;
      std::vector<legion::interface::TrajectoryInPrediction> predict_trajectories;
      ...
      for (auto iter = vehicle_obstacle_ids_.begin(); iter != vehicle_obstacle_ids_.end(); iter++)
      {
        ...
        if (filter_mode_by_lane_ == FILTER_LL_RR && 
             (iter_obstacle->front().lane_position() == legion::common::NEXT_LEFT_LANE ||
               iter_obstacle->front().lane_position() == legion::common::NEXT_RIGHT_LANE))  //filter based on LanePosition
          continue;
    ...
    bool VectorMapPredictor::PredictPedestrianLocalMap(const std::vector<std::vector<legion::prediction::LanePointInner>> &smooth_lines)
    {
      std::vector<legion::prediction::PredictionTrajectory> trajectories;
      std::vector<legion::interface::TrajectoryInPrediction> predict_trajectories;
      legion::interface::TrajectoryInPrediction predict_trajectory;
