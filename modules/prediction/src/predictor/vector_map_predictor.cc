...
namespace legion
{
  namespace prediction
  {
    ...
    {
      // double time1=legion::common::TimeTool::NowToSeconds();
      ...
      { 
        ...
        if(/*obstacle.fusion_type() == legion::interface::Obstacle::FusionType::LIDAR && */area <= 0.03)
          continue;
        ...
        if (obstacle.id() != -1)
        {
          ...
          {
            legion::interface::Obstacle obstacle_gps;
      ...
      // double time2=legion::common::TimeTool::NowToSeconds();
      ...
      if (localiztion_mode_ == 2)  //lane模式，暂时按照匀速直线运动预测（TODO: 按车道线预测）
        filter_mode_ = NO_MAP;
      else if ( local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARKING_INITIAL_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARKING_FINISH_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_BEGIN_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_FORWARD_BEGIN_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_FORWARD_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_FORWARD_FINISH_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_BACKWARD_BEGIN_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_BACKWARD_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_BACKWARD_FINISH_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_FINISH_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_BEGIN_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_FORWARD_BEGIN_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_FORWARD_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_FORWARD_FINISH_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_BACKWARD_BEGIN_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_BACKWARD_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_BACKWARD_FINISH_STATE ||
           local_view.adc_trajectory_.behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_FINISH_STATE )
      ...
      // double time3=legion::common::TimeTool::NowToSeconds();
    ...
    legion::interface::Point3D Vector3dToPoint3d(Eigen::Vector3d pos_in)
    {
      legion::interface::Point3D pos_out;
    ...
    bool VectorMapPredictor::TransformObstacleOdom2GPS(const legion::interface::Obstacle &obs_in, legion::interface::Obstacle &obs_out)
    {
      obs_out = obs_in;
    ...
    bool VectorMapPredictor::TransformPredictionPerceptionObstacleGPS2Odom(const legion::interface::PerceptionObstacle &obs_in, 
                                                                            legion::interface::PerceptionObstacle &obs_out)
    {
      obs_out = obs_in;
    ...
    bool VectorMapPredictor::TransformTrajectoriesObstacleGPS2Odom(const double &z,  const std::vector<legion::interface::TrajectoryInPrediction> &in, 
                                                                    std::vector<legion::interface::TrajectoryInPrediction> &out)
    {
      out = in;
      ...
      // for (auto trajectory : out)
      {
        ...
        {
          legion::interface::TrajectoryPoint trajectory_point;
    ...
    void VectorMapPredictor::InsertObstacleToHistory(const legion::interface::Obstacle &object)
    {
      ...
      if (object.type() == legion::common::OBSTACLE_PEDESTRIAN ||
          object.sub_type() == legion::common::ST_PEDESTRIAN ||
          object.sub_type() == legion::common::ST_CYCLIST) {
        pedestrian_obstacle_ids_.push_back(id);
      } else if (object.type() == legion::common::OBSTACLE_UNKNOWN ||
                 object.type() == legion::common::OBSTACLE_UNKNOWN_MOVABLE ||
                 object.type() == legion::common::OBSTACLE_UNKNOWN_UNMOVABLE ||
                 object.type() == legion::common::OBSTACLE_VEHICLE ||
                 object.sub_type() == legion::common::ST_CAR ||
                 object.sub_type() == legion::common::ST_VAN ||
                 object.sub_type() == legion::common::ST_TRUCK ||
                 object.sub_type() == legion::common::ST_BUS) {
    ...
    std::deque<legion::interface::Obstacle> *VectorMapPredictor::GetObstacleWithLRUUpdate(const int obstacle_id)
    {
    ...
    std::unique_ptr<std::deque<legion::interface::Obstacle>> VectorMapPredictor::Create(const legion::interface::Obstacle &object)
    {
      std::unique_ptr<std::deque<legion::interface::Obstacle>> ptr_obstacle(new std::deque<legion::interface::Obstacle>());
      ptr_obstacle->emplace_front(object);
      return ptr_obstacle;
    }

    std::vector<legion::interface::Point3D> VectorMapPredictor::ConstructRect(const double &x, const double &y, const double &yaw_rad,
                                                                     const double &width, const double &length)
    {
      std::vector<legion::interface::Point3D> box;
      legion::interface::Point3D p_front_center, p_back_center, p1, p2, p3, p4;
    ...
    legion::interface::PerceptionObstacle VectorMapPredictor::ConvertFusedObject2PerceptionObstacle(const legion::interface::Obstacle &fused_object)
    {
      legion::interface::PerceptionObstacle perception_obstacle;

      //#感知障碍物ID
      perception_obstacle.set_id(fused_object.id());
      //#地图坐标系下障碍物的Point位置
      legion::interface::Point3D position;
      ...
      //#速度，障碍物在地图坐标系下的速度分量
      legion::interface::Point3D velocity;
      ...
      //#障碍物加速度
      legion::interface::Point3D acceleration;
      ...
      //#障碍物的锚点
      legion::interface::Point3D anchor_point;
    ...
    legion::interface::PredictionObstacle VectorMapPredictor::GetPredictionResult(const legion::interface::Obstacle &object,
                            const std::vector<legion::interface::TrajectoryInPrediction> &trajectories,
                            const legion::interface::ObstacleIntent &intent)
    {
      legion::interface::PredictionObstacle prediction_obstacle; // ------ one predicted obstacle
      ...
      std::vector<legion::interface::TrajectoryInPrediction> tmp_trajectories;

      if (localiztion_mode_ == 1 && map_mode_ == 0)  // odometry模式，预测信息从gps坐标系转回odometry坐标系
      {
        legion::interface::PerceptionObstacle perception_obstacle_odom;
