...
// using legion::prediction::Prediction;
/**
 ...
 */
    ...
    public:
       ...
       * @return    void.
       */
      void InsertObstacleToHistory(const legion::interface::Obstacle &object);
      ...
      bool TransformObstacleOdom2GPS(const legion::interface::Obstacle &obs_in, legion::interface::Obstacle &obs_out);

      bool TransformPredictionPerceptionObstacleGPS2Odom(const legion::interface::PerceptionObstacle &obs_in, 
                                                                                                                             legion::interface::PerceptionObstacle &obs_out);

      bool TransformTrajectoriesObstacleGPS2Odom(const double &z,  const std::vector<legion::interface::TrajectoryInPrediction> &in, 
       ...
       * @return    相应的障碍物队列指针.
       */
      std::deque<legion::interface::Obstacle> *GetObstacleWithLRUUpdate(const int obstacle_id);
       ...
       * @return    新建的障碍物队列.
       */
      static std::unique_ptr<std::deque<legion::interface::Obstacle>> Create(const legion::interface::Obstacle &object);
       ...
       * @return    矩形的四个顶点.
       */
      std::vector<legion::interface::Point3D> ConstructRect(const double &x, const double &y, const double &yaw,
       ...
       */
      legion::interface::PerceptionObstacle ConvertFusedObject2PerceptionObstacle(const legion::interface::Obstacle &fused_object);
       ...
       */
      legion::interface::PredictionObstacle GetPredictionResult(const legion::interface::Obstacle &object,
       ...
       * @return    void.
       */
      void PredictOSM(const legion::interface::Location &location);
      ...
      bool SmoothCenterLines(std::vector<legion::interface::LanePoint> center_line, double delta_mileage,
                                                           std::vector<legion::prediction::LanePointInner> &smooth_line);
                                  
      void PredictLocalMap(const legion::interface::RoutingResponse &routing);
      bool PredictVehicleLocalMap(const std::vector<std::vector<legion::prediction::LanePointInner>> &smooth_lines);
      bool PredictPedestrianLocalMap(const std::vector<std::vector<legion::prediction::LanePointInner>> &smooth_lines);
      bool PredictOtherObstacleLocalMap();

      void Stop() override {}

      legion::interface::PredictionObstacles PredictionObstacles()  const
      {
    ...
    private:
      ...
      legion::common::util::LRUCache<int, std::unique_ptr<std::deque<legion::interface::Obstacle>>> ptr_obstacles_; //id，障碍物队列
  
      // legion::interface::PredictionOutArray prediction_out_array_;
