...
public:
    ...
    bool GeneratePath(const std::vector<legion::interface::Point3D> &center_line,
    ...
    bool GeneratePath(const std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr> &refer_line,
                const std::vector<double> &mileage_list,
                const legion::interface::PathPoint &start_point,
                const double &delta_mileage,
                std::vector<legion::interface::PathPoint> &predict_path);
     ...
     */
    bool PredictPedestrian(const std::vector<std::vector<legion::prediction::LanePointInner>> &smooth_lines,
                           const std::deque<legion::interface::Obstacle> &object_deque,
                           const PredictionConf &prediction_conf,
                           const int &filter_mode, 
                           std::vector<legion::prediction::PredictionTrajectory> &trajectories);
     ...
     */
    bool PredictVehicle(const std::vector<std::vector<legion::prediction::LanePointInner>> &smooth_lines,
                        const std::deque<legion::interface::Obstacle> &object_deque,
                        const PredictionConf &prediction_conf,
                        const int &filter_mode, 
                        std::vector<legion::prediction::PredictionTrajectory> &trajectories);
    ...
    bool TranslateLine(const std::vector<legion::interface::PathPoint> &raw_line,
     ...
     */
    bool PredictTrajectoriesUsingVelocity(const legion::interface::TrajectoryPoint &start_pose,
                                          const double &max_predict_distance, const double trajectory_density,
                                          const double &threshold_static_speed,
                                          std::vector<legion::prediction::PredictionTrajectory> &trajectories);

    bool PredictTrajectoriesUsingVelocityWithT(const legion::interface::TrajectoryPoint &start_pose,
