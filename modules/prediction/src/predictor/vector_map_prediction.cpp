...
bool VectorMapPrediction::PredictPedestrian(const lanelet::LaneletMapUPtr &map,
                                            ...
                                            const std::deque<legion::interface::Obstacle> &object_deque,
                                            const PredictionConf &prediction_conf,
                                            const int &filter_mode, 
                                            std::vector<legion::prediction::PredictionTrajectory> &trajectories)
{
    ...
    legion::interface::Obstacle object;
    ...
    legion::interface::TrajectoryPoint fake_pose;
    legion::interface::PathPoint obstacle_point;
    ...
    std::vector<std::vector<std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr>>> refer_line_arry;
    bool in_lane = false;
    if(filter_mode == FILTER_BY_CENTER_POINT || filter_mode == FILTER_BY_POLYGON)
    {        
        // std::cout << "object.polygon_point_abs() : " << object.polygon_point_abs().size() << std::endl;
    ...
    if (refer_line_arry.size() > 0)
    {
        ...
        legion::prediction::PredictionTrajectory trajectory;
        legion::interface::TrajectoryInPrediction predict_trajectory;
        legion::interface::PathPoint start_point, end_point;
        ...
        double dis = 0.0;
        line.clear();
        legion::interface::Point3D raw_point;
        ...
        std::vector<legion::interface::PathPoint> ref_path, predict_path;
        ...
        trajectory.set_predict_mode(legion::prediction::PREDICT_MODE_VECTORMAP);
        trajectory.set_predict_direction(legion::prediction::PREDICT_DIR_STRAIGHT);
...
bool VectorMapPrediction::GeneratePath(const std::vector<legion::interface::Point3D> &center_line,
                const std::vector<double> &mileage_list,
                const legion::interface::PathPoint &start_point,
                const double &delta_mileage,
                std::vector<legion::interface::PathPoint> &predict_path)
{
    if (mileage_list.size() <= 2 || (center_line.size() != mileage_list.size()))
        return false;

    legion::prediction::spline::spline spline_s_x, spline_s_y;
    ...
    legion::interface::PathPoint smooth_point;
...
bool VectorMapPrediction::GeneratePath(const std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr> &refer_line,
                const std::vector<double> &mileage_list,
                const legion::interface::PathPoint &start_point,
                const double &delta_mileage,
                std::vector<legion::interface::PathPoint> &predict_path)
{
    std::vector<legion::interface::Point3D> center_line = refer_line.first;
    if (mileage_list.size() <= 2 || (center_line.size() != mileage_list.size()))
        return false;
    ...
    double coef = 0.0;
    // std::cout << "------------ " << refer_line.second.dis / mileage_list.back() << std::endl;
                                         ...
                                         std::vector<legion::prediction::PredictionTrajectory> &trajectories)
{
    ...
    int theta_mode = prediction_conf.theta_mode();
    double ele_diff_threshold = prediction_conf.ele_diff_threshold();
    // double begin_time=legion::common::TimeTool::NowToSeconds();
    trajectories.clear();
    // std::vector<std::vector<PlannerHNS::WayPoint>> paths;
    legion::interface::Obstacle object;
    if (object_deque.size() > 0)
    ...
    // if (obj_v >= prediction_conf.threshold_static_speed() && theta_mode == USE_VELOCITY_THETA)
       theta_start = math::GetHeadingRadian(object.velocity_abs().x(), object.velocity_abs().y());
    // std::cout << "-----id : " << object.id() << ", -----velocity_abs().x() = : " << object.velocity_abs().x() << ", velocity_abs().y() = " << object.velocity_abs().y() << std::endl; 
    // std::cout << "0-----obj_v : " << obj_v << std::endl; 
    // std::cout << "1-----几何方向 : " << math::R2D(object.theta_abs()) << std::endl; 
    // std::cout << "2-----速度方向 : " << math::R2D(theta_start) << std::endl;
    ...
    if (refer_line_arry.size() > 0)
    {
        ...
        legion::prediction::PredictionTrajectory trajectory;
        legion::interface::TrajectoryInPrediction predict_trajectory;
        legion::interface::PathPoint end_point;
        ...
        for (unsigned int i = 0; i < refer_line_arry.size(); i++)
        {
            for (unsigned int j = 0; j < refer_line_arry[i].size(); j++)
            {
                ...
                std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr> refer_line;
                double dis = 0.0;
                ...
                if (line_num > 1)
                {
                    ...
                    // std::cout << "------------ max_kappa.first = " << max_kappa.first << std::endl;
                    ...
                    // legion::interface::PathPoint predict_point;
                    // legion::interface::TrajectoryPoint trajectory_point;
                    ...
                    case lanelet2_ex::LEFT:
                        trajectory.set_predict_direction(legion::prediction::PREDICT_DIR_LEFT);
                        break;
                    case lanelet2_ex::RIGHT:
                        trajectory.set_predict_direction(legion::prediction::PREDICT_DIR_RIGHT);
                        break;
                    default:
                        break;
                    }
                    ...
                    //         <<average_kappa_abs<<", average_kappa_max = "<<average_kappa_max<<", average_kappa = "<<average_kappa<<std::endl;
                    scores.push_back(tmp_score);
                    // if (object.id() == 11492)
                    // std::cout << "******** score_state = " << score_state << ", average_kappa = " << average_kappa*5
                    //           << "score_theta_diff = " << score_theta_diff << std::endl;
                    // std::cout << "dis = " << refer_line.second.dis << ", theta_diff = " << math::NormalizeAngle(math::D2R(refer_line.second.theta_diff)) 
                    //           << ", score = " << threshold_dis - fabs(refer_line.second.dis) - 5.0 * (fabs(math::NormalizeAngle(math::D2R(refer_line.second.theta_diff))) - M_PI_2) << std::endl;
                    trajectories.push_back(trajectory);
                }
            }
        }
        ...
        {
            std::cout<<"probilities.size() != trajectories.size()"<<std::endl;
            return false;
        }
    ...
    // double pred_time=legion::common::TimeTool::NowToSeconds();
    // std::cout<<"222222222222222222222pred_time  :   "<<pred_time-route_time<<"     seconds "<<std::endl<<std::endl;
 ...
 */
bool VectorMapPrediction::PredictPedestrian(const std::vector<std::vector<legion::prediction::LanePointInner>> &smooth_lines,
                                            const std::deque<legion::interface::Obstacle> &object_deque,
                                            const PredictionConf &prediction_conf,
                                            const int &filter_mode, 
                                            std::vector<legion::prediction::PredictionTrajectory> &trajectories)
 ...
 */
bool VectorMapPrediction::PredictVehicle(const std::vector<std::vector<legion::prediction::LanePointInner>> &smooth_lines,
                                         const std::deque<legion::interface::Obstacle> &object_deque,
                                         const PredictionConf &prediction_conf,
                                         const int &filter_mode, 
                                         std::vector<legion::prediction::PredictionTrajectory> &trajectories)
...
bool VectorMapPrediction::ComputeProbability(const std::vector<double> &scores, std::vector<double> &probilities)
{
    ...
    for (auto ex : exs)
    {
        ...
        // if (isnan(ex / sum))
        // {
        //     std::cout << ex << " xxxxxxxxxxxxxxxxxxxxxxx " << sum << std::endl;
        //     for (auto score : scores)
...
bool VectorMapPrediction::TranslateLine(const std::vector<legion::interface::PathPoint> &raw_line,
                                        const int &start_index, const double &lat_offset, //left - right +
                                        std::vector<legion::interface::PathPoint> &translated_line)
{
    if (start_index < 0 || start_index > int(raw_line.size() - 1))
        return false;

    legion::interface::PathPoint tmp_point;
...
bool VectorMapPrediction::PredictTrajectoriesUsingVelocity(const legion::interface::TrajectoryPoint &start_pose,
                                                           ...
                                                           std::vector<legion::prediction::PredictionTrajectory> &trajectories)
{
    ...
    legion::prediction::PredictionTrajectory trajectory;
    legion::interface::TrajectoryInPrediction predict_trajectory;
    legion::interface::TrajectoryPoint tp = start_pose;
    legion::interface::PathPoint wp = start_pose.path_point();
    ...
    {
        // std::cout << "====== max_predict_distance = " << max_predict_distance << ", start_pose.v() = " << start_pose.v() << std::endl;
        wp.set_x(wp.x() + dx);
        wp.set_y(wp.y() + dy);
    ...
    trajectory.set_trajectory_in_prediction(predict_trajectory);
    trajectory.set_predict_mode(legion::prediction::PREDICT_MODE_VELOCITY);
    trajectory.set_predict_direction(legion::prediction::PREDICT_DIR_STRAIGHT);
...
bool VectorMapPrediction::PredictTrajectoriesUsingVelocityWithT(const legion::interface::TrajectoryPoint &start_pose,
