                                                  ...
                                                  legion::interface::ObstacleIntent &intent)
{
    ...
    {
        intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_STOPPING_STATE);
        return true;
    }
    ...
    // std::vector<legion::interface::TrajectoryPointInPrediction> intent_points;
    // trajectories.at(max_index).trajectory_in_prediction().trajectory_points(intent_points);
    legion::prediction::PredictionTrajectory intent_trajectory;
    ...
    if (intent_trajectory.predict_mode() == legion::prediction::PREDICT_MODE_VELOCITY)
    {
        ...
        {
             intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_CROSS_ROAD_STATE);
            return true;
        }
        else if (yaw_diff < 0.1 * M_PI)
        {
             intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_ALONG_ROAD_STATE);
            return true;
        }
        else if (yaw_diff > 0.85 * M_PI)
        {
             intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_ONCOMMING_STATE);
            return true;
        }
    }
    ...
    {
         intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_ACCELERATING_STATE);
        return true;
    }
    else if (accumulate_time_dec > 0.7 * predict_time && accumulate_time_acc < accumulate_time_dec)
    {
         intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_SLOWDOWN_STATE);
        return true;
    }
    else if (accumulate_time_keep > 0.7 * predict_time)
    {
         intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_KEEP_SPEED_STATE);
        return true;
    }
    ...
    intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_FORWARD_STATE);
 ...
 */
bool IntentPrediction::PredictIntentForVehicle(const std::vector<legion::prediction::PredictionTrajectory> &trajectories,
                                               const double &predict_time,
                                               legion::interface::ObstacleIntent &intent)
{
    /*取出概率最大的轨迹*/
    ...
    double delta_heading = legion::common::math::AngleDiff(start_heading, end_heading);
    if (delta_heading > M_PI_4) //Turn Left
    {
        intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_TURN_LEFT_STATE);
        return true;
    }
    else if (delta_heading < -M_PI_4) //Turn Right
    {
        intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_TURN_RIGHT_STATE);
        return true;
    }
    ...
    if (intent_trajectory.predict_direction() == legion::prediction::PREDICT_DIR_LEFT)
    {
         intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_CHANGE_LANE_LEFT_STATE);
        return true;
    }
    else if (intent_trajectory.predict_direction() == legion::prediction::PREDICT_DIR_RIGHT)
    {
         intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_CHANGE_LANE_RIGHT_STATE);
        return true;
    }
    else if (intent_trajectory.predict_direction() == legion::prediction::PREDICT_DIR_STRAIGHT)
    {
        intent.set_type(legion::interface::ObstacleIntent::Type::INTENT_FORWARD_STATE);
        return true;
    }
