...
#include "common/interface/path_point.hpp"
...
namespace lanelet2_ex
{
                                ...
                                const double x, const double y) 
    {
        ...
        return legion::common::math::InterpolateUsingLinearApproximation(p0, p1, p0.s() + delta_s);
    }
    ...
    lanelet::Ids GetClosestLanelets(const lanelet::LaneletMapUPtr &map, const lanelet::Point3d point)
    {
        ...
        {
            if (std::string(lane.attributeOr("sub_type", "unknown")) == std::string("virtual")
                || std::string(lane.attributeOr("subtype", "unknown")) == std::string("virtual")) //路口虚拟车道，匹配条件放宽
            // if (lane.attribute("subtype") == "virtual") //路口虚拟车道，匹配条件放宽
            {
                // std::cout << "****************" << std::endl;
                if (lanelet::geometry::distance2d(point.basicPoint2d(), lane.polygon2d()) < 3.0)
                {
                    ids_.push_back(lane.id());
                }
            }
            else if (boost::geometry::covered_by(point.basicPoint2d(), lane.polygon2d()))
            {
                ids_.push_back(lane.id());
            }
        }
    ...
    lanelet::Ids GetClosestLaneletsInRange(const lanelet::LaneletMapUPtr &map, const lanelet::Point3d point, const double range)
    {
        ...
        for (auto lane : map->laneletLayer)
        {
            if (lanelet::geometry::distance2d(point.basicPoint2d(), lane.polygon2d()) < range)
            {
                ids_.push_back(lane.id());
            }
        }
                                  ...
                                  lanelet::ConstPoints3d &cpoints, lanelet::Ids &ids, bool &in_lane)
    {
        ...
        ids.clear();
        in_lane = false;
        // lanelet::Ids _ids = GetClosestLaneletsInRange(map, point, 2.0);
        // if (_ids.size() == 0)
        // {
        //     // in_lane = false;
        //     return 0;
                                    ...
                                    std::pair<lanelet::ConstPoint3d, double>  &path_point)
    {
        ...
        double d = 0;
        // double min_d = DBL_MAX;
        double min_d_total = DBL_MAX;
        // double yaw = 0.0, angle = 0.0;
        for (auto id : lane_ids)
        {
            lanelet::Lanelet lane = map->laneletLayer.get(id);
            lanelet::ConstLineString3d cline = lane.centerline();
            ...
            for (size_t i = 0; i < cline.size(); i++)
            {
                ...
                double yaw = calculate_angle(front, behind);
                double angle = 90.0 - theta; //航向角转正常角度
                double angle_diff = calculate_diff_angle(yaw, angle);
                double temp_d_total = angle_diff * 10 / 180.0 + d;
                if (temp_d_total < min_d_total)
                {
                    match_valid = true;

                    if (!belast)
                    {
                        path_point.first = front;
                    }
                    else
                    {
                        path_point.first = behind;
                    }
                    path_point.second = yaw;

                    min_d_total = temp_d_total;
                }
            }
        }    
    ...
    std::pair<lanelet::ConstPoint3d, lanelet::Id> GetClosestPoint_LaneletInRange(const lanelet::LaneletMapUPtr &map, const lanelet::Point3d point, const double theta, const double range, const double weight_times)
    {
        ...
        for (auto id : lane_ids)
        {
            ...
            double true_min_d = lanelet::geometry::distance2d(cline.basicLineString(), point.basicPoint2d());
            double yaw = calculate_angle(front, behind);
            double angle = 90.0 - theta; //航向角转正常角度
    ...
    {
        double min_d = DBL_MAX;
        double temp_d = 0.0, mileage = 0.0;
        unsigned int index = 0;
        std::vector<PathPoint> path_line;
        PathPoint tmp_path_point, matched_path_point;
        path_line.clear();
        for (unsigned int i = 0; i < cline.size(); i++)
        {
            if (i > 0)
            {
                mileage += lanelet::geometry::distance2d(cline[i-1].basicPoint2d(), cline[i].basicPoint2d());
            }
            temp_d = lanelet::geometry::distance2d(point.basicPoint2d(), cline[i].basicPoint2d());
            if (temp_d < min_d)
            {
            ...
            tmp_path_point.set_x(cline[i].x());
            tmp_path_point.set_y(cline[i].y());
            tmp_path_point.set_s(mileage);

            path_line.push_back(tmp_path_point);
        }

        std::size_t index_start = (index == 0) ? index : index - 1;
        std::size_t index_end =
            (index + 1 == path_line.size()) ? index : index + 1;

        if (index_start == index_end) {
            matched_path_point = path_line[index_start];
        }

        matched_path_point = FindProjectionPoint(path_line[index_start], path_line[index_end], point.x(), point.y());
        // matched_path_point = path_line[index];
        double dist_min = legion::common::math::Norm(matched_path_point.x()-point.x(), matched_path_point.y()-point.y());
        double sign = legion::common::math::CrossProd({path_line[index_start].x(), path_line[index_start].y()}, 
                                                    {point.x(), point.y()}, 
                                                    {path_line[index_end].x(), path_line[index_end].y()});
        if (sign > 0)  //点在向量右侧
            dist_min *= -1;
        ...
        lan_atr.dir = dir;
        lan_atr.index = index;
        lan_atr.dis = dist_min;
        double lat_angle = legion::common::math::GetHeading(point.x(), point.y(), matched_path_point.x(), matched_path_point.y());
        lan_atr.theta_diff = calculate_diff_angle(angle, lat_angle);
    ...
    */
    ...
    {
        std::vector<lanelet::Ids>::iterator iter = paths.begin();
        for (; iter != paths.end(); ++iter)
        {
            iter->insert(iter->begin(), start_id);
        }
        return paths;
    }
    ...
    std::pair<double, int> GetRemainDistance(const lanelet::ConstPoint3d point, const lanelet::ConstLineString3d cline)
    {
        ...
        {
            distance += lanelet::geometry::distance2d(cline[i].basicPoint2d(), cline[i + 1].basicPoint2d());
        }
    ...
    std::vector<std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr>> Ids2LinesWithTag(const lanelet::LaneletMapUPtr &map, lanelet2_ex::Lan_Atr attr, std::vector<lanelet::Ids> paths)
    {
        std::vector<std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr>> result;
        for (auto path : paths)
        {
            std::vector<legion::interface::Point3D> line;
            line.clear();
            for (auto lane_id : path)
            {
                lanelet::Lanelet lane = map->laneletLayer.get(lane_id);
                lanelet::ConstLineString3d cline = lane.centerline();
                auto cline_=lanelet2_ex::Cline2Points(cline);
                line.insert(line.end(), cline_.begin(), cline_.end() - 1);
            }
    ...
    std::vector<legion::interface::Point3D> Cline2Points(lanelet::ConstLineString3d cline)
    {
        ...
        for (auto point : cline)
        {
            ...
            point_.set_z(point.z());
            points.insert(points.end(), point_);
        }
        return points;
    }
    ...
    std::vector<std::vector<std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr>>> GetRoutes(const lanelet::LaneletMapUPtr &map, 
    ...
    {
        std::vector<std::vector<std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr>>> all_routes;
        all_routes.clear();
        lanelet::ConstPoints3d locate_cpoints;
        lanelet::Ids locate_lane_ids;
        in_lane = false;
        int closest_num = GetClosestPoints_Lanelets(map, point, theta, use_theta, locate_cpoints, locate_lane_ids, in_lane);
        for (int i = 0; i < closest_num; i++)
        {
            std::vector<std::pair<std::vector<legion::interface::Point3D>, lanelet2_ex::Lan_Atr>> all_trajectory;
            all_trajectory.clear();
            std::pair<lanelet::Id, lanelet::Id> besides;
            ...
            auto centerline = Cline2Points(lane_.centerline());
            if (fabs(point.basicPoint().z() - centerline.front().z()) > ele_diff_threshold)   //高程偏差太大，匹配失败
                continue;
            auto follows = routing_map->following(lane_);
            auto self_remain = GetRemainDistance(point, lane_.centerline()); //first 剩余距离
            if (self_remain.first > max_distance)
            {
                ...
                // self_attr.index = GetPointIndex(point, lane_.centerline());
                self_attr = GetLanAtr(point, theta, lane_.centerline(), lanelet2_ex::Trajectory_dir::STRAIGHT);
                auto self_tra = std::make_pair(centerline, self_attr);
                all_trajectory.push_back(self_tra);
                if (besides_t.first)
                {
                    lanelet::Lanelet left = map->laneletLayer.get(besides.first);
                    ...
                    // left_attr.index = GetPointIndex(point, left.centerline());
                    left_attr = GetLanAtr(point, theta, left.centerline(), lanelet2_ex::Trajectory_dir::LEFT);
                ...
                if (besides_t.second)
                {
                    ...
                    // right_attr.index = GetPointIndex(point, right.centerline());
                    right_attr = GetLanAtr(point, theta, right.centerline(), lanelet2_ex::Trajectory_dir::RIGHT);
            ...
            else
            {
                ...
                for (auto follow : follows)
                {
                    ...
                    // follow_attr.index = GetPointIndex(point, lane_.centerline());
                    follow_attr = GetLanAtr(point, theta, lane_.centerline(), lanelet2_ex::Trajectory_dir::STRAIGHT);
                ...
                if(!left_sets.empty())
                {
                    ...
                    // left_follow_attr.index = GetPointIndex(point, map->laneletLayer.get(left_sets.front()).centerline());
                    left_follow_attr = GetLanAtr(point, theta, map->laneletLayer.get(left_sets.front()).centerline(), lanelet2_ex::Trajectory_dir::LEFT);
                    //auto left_paths = LinkFollows(map, left_sets);
                    std::vector<lanelet::Ids> left_paths = {left_sets};
                ...
                if(!right_sets.empty())
                {
                    ...
                    // right_follow_attr.index = GetPointIndex(point, map->laneletLayer.get(right_sets.front()).centerline());
                    right_follow_attr = GetLanAtr(point, theta, map->laneletLayer.get(right_sets.front()).centerline(), lanelet2_ex::Trajectory_dir::RIGHT);
                    //auto right_paths = LinkFollows(map, right_sets);
                    std::vector<lanelet::Ids> right_paths = {right_sets};
    ...
    unsigned int GetClosestIndexInPath(const std::vector<int> path, const lanelet::Point3d point, const lanelet::LaneletMapUPtr &map)
    {
        ...
        else
        {
            ...
            for (unsigned int i = 0; i < path.size(); i++)
            {
                ...
                auto temp_dis = lanelet::geometry::distance2d(lane.polygon2d(), point.basicPoint2d());
                if (temp_dis < min_dis)
                {
                    index = i;
                    min_dis = temp_dis;
