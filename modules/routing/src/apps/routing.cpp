...
void Routing::Init() {
  ...
  // step2 变量初始化
  {
    ...
    cur_event_.mutable_limit_speed_info()->set_limitspeed_valid_flag(
        legion::common::IsValid(0));
    cur_speed_limit.set_limitspeed_valid_flag(legion::common::IsValid(0));
  ...
  if (task_thread_ == nullptr) {
    std::cerr << "Unable to create task_thread_ thread " << std::endl;
    return;
  }
  // step9 初始化状态为true
  {
...
void Routing::PublishNaviInfoMsg(legion::interface::NaviInfoMsg navi_info_msg) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Routing::PublishGuideInfo(legion::interface::GuideInfo guide_info) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Routing::PublishLaneList(legion::interface::LaneList lane_list) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Routing::PublishParkingInfo(legion::interface::ParkingInfo parking_info) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Routing::PublishStopInfo(legion::interface::StopInfo stop_info) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Routing::PublishFaults() {
  ...
  // 填app的id，唯一标识符
  faults.set_id(faultcodeset_->get_target_id());
  // 填状态
...
void Routing::PublishGlobalRouteMsg(
    legion::interface::GlobalRouteMsg global_route_msg) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Routing::PublishPolygon2D(legion::interface::Polygon2D polygon_2d) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Routing::PublishTrafficLightMsgOutput(
    legion::interface::TrafficLightMsg traffic_light_msg_output) {
  // #if LCM_ENABLE
  //       if (message_manager_.count("LCM") > 0)
...
void Routing::HandleTrafficLightMsg(
  ...
  }
  {
    if (routing_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = traffic_light_msg.header();
...
void Routing::HandleTrafficEventsInput(
    legion::interface::TrafficEvents traffic_events_input) {
  if (is_init_ == false) {
    return;
  }
  {
    if (routing_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = traffic_events_input.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Routing::HandleOdometry(legion::interface::Odometry odometry) {
  if (is_init_ == false) {
    return;
  }
  {
    if (routing_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = odometry.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Routing::HandleLocation(legion::interface::Location location) {
  if (function_activation_ == false) {
    return;
  }
  std::unique_lock<std::mutex> lock(mutex_);
  ...
  if (!location_init_) {
    ...
    std::cout << "Origin  : " << "  lat:  " << setprecision(10)
              ...
              << routing_conf_->origin_lon() << std::endl;
...
void Routing::HandleChassis(legion::interface::Chassis chassis) {
  if (is_init_ == false) {
    return;
  }
  {
...
void Routing::HandleRoutingRequest(
  ...
  if (!location_init_) {
    std::cerr << "location not init" << std::endl;
    return;
  }
...
void Routing::HandleObuCmdMsg(legion::interface::ObuCmdMsg obu_cmd_msg) {
  if (!is_init_) {
    return;
  }
  // if (!location_init_)
  ...
  for (auto obucmd : obu_cmd_list) {
    if (obucmd.code() == 10042 && obucmd.val() == 1) {
      ...
      if (new_parking_space_) {
        ...
        std::cout << "No Parking Space Choosed" << std::endl;
      }
    } else if (obucmd.code() == 10001 && obucmd.val() == 2) {
      std::unique_lock<std::mutex> lock(mutex_);
      Clear();
      std::cout << "Exit Global Planning" << std::endl;
    } else if (obucmd.code() == 10086) {
      std::cout << "code : " << obucmd.code() << std::endl;
      switch (obucmd.val()) {
...
bool Routing::ParkingPlan() {
  std::cout << "Choosing the Nearest Parking Area " << std::endl;
  if (parking_areas_.empty()) {
    std::cout << "No Parking Space in Map" << std::endl;
    return false;
  }
  ...
  if (!get_it) {
    std::cout << "Can not Locate Vehicle in map" << std::endl;
  }
  int index = -1;
  ...
  for (unsigned int j = 0; j < parking_areas_.size(); j++) {
    ...
    // std::cout << "shortest path in lanelet2" << std::endl;
  ...
  if (index == -1) {
    std::cout << "No Parking Area Selected" << std::endl;
    return false;
  }
  auto parking_selected = parking_areas_[index];

  interface::RoutingRequest request;
  std::vector<legion::interface::KeyPoint> key_points;
...
void Routing::LoadStopLines() {
  ...
  for (auto light : lights) {
    ...
    for (auto lane : lanes) {
      std::cout << "test_stoplane: " << lane.id() << std::endl;
      auto temp_dir = FindNextJunctionDirection(lane.id());
      std::cout << "NextJunctionDirection= " << temp_dir << std::endl;
      if (temp_dir != legion::common::Direction::DIR_INVALID) {
  ...
  for (size_t i = 0; i < all_stop_lines_.size(); i++) {
    std::cout << "stop_line_" << i << std::endl;
    for (auto id : all_stop_lines_[i].lane_ids) {
      std::cout << id << " -- ";
    }
    std::cout << "\nstop_line_" << i << std::endl;
  }
  return;
}

legion::common::Direction Routing::FindNextJunctionDirection(int id) {
  legion::common::Direction dir = legion::common::Direction::DIR_INVALID;
  for (auto ids : local_paths_) {
    // std::cout<<"--local_path"<<std::endl;

    for (size_t i = 0; i < ids.size() - 1; i++) {
      // std::cout<<"LaneId= "<<ids[i]<<std::endl;
      if (ids[i] == id) {
        auto next_lane = map_->laneletLayer.get(ids[i + 1]);
        // std::cout<<"NextJunctionLaneId= "<<ids[i + 1]<<"  LaneType=
        // "<<next_lane.attributeOr("subtype","unknown")<<std::endl;
        if (next_lane.attributeOr("subtype", "unknown") == "virtual") {
          ...
          if (next_dir == std::string("left") ||
            ...
            dir = legion::common::Direction::LEFT;
          } else if (next_dir == std::string("right")) {
            dir = legion::common::Direction::RIGHT;
          } else if (next_dir == std::string("straight")) {
            dir = legion::common::Direction::UP;
          } else {
            dir = legion::common::Direction::DIR_INVALID;
          }
        }
      }
    }
...
legion::common::Direction Routing::FindJunctionDirection(int id) {
  auto lane = map_->laneletLayer.get(id);
  legion::common::Direction dir = legion::common::Direction::DIR_INVALID;
  // std::cout<<"lane.attribute =
  // "<<lane.attributeOr("subtype","unknown")<<std::endl; if
...
int Routing::GetLaneInfoType(int id) {
  ...
  legion::routing::LaneInfoType lane_type;
  if (lane_guidance == std::string("left")) {
    lane_type = legion::routing::LaneInfoType::LEFT_TURN_NO_TURN_AROUND;
  } else if (lane_guidance == std::string("straight")) {
    lane_type = legion::routing::LaneInfoType::STRAIGHT;
  } else if (lane_guidance == std::string("right")) {
    lane_type = legion::routing::LaneInfoType::RIGHT_TURN;
  } else if (lane_guidance == std::string("straight_left")) {
    lane_type = legion::routing::LaneInfoType::STRAIGHT_AND_LEFT_TURN;
  } else if (lane_guidance == std::string("straight_right")) {
    lane_type = legion::routing::LaneInfoType::STRAIGHT_AND_RIGHT_TURN;
  } else if (lane_guidance == std::string("left_turnaround")) {
    lane_type = legion::routing::LaneInfoType::LEFT_TURN_AND_TURN_AROUND;
  } else if (lane_guidance == std::string("turnaround")) {
    lane_type = legion::routing::LaneInfoType::NO_LEFT_TURN_ONLY_TURN_AROUND;
  } else {
    lane_type = legion::routing::LaneInfoType::LANE_TYPE_UNKNOWN;
  }
  return lane_type;
}
...
void Routing::LoadJunctionInfo(const std::vector<int>& ids) {
  ...
  for (auto id : no_change_lane_) {
    ...
    if (lanetype == "virtual" &&
        cur_dir != legion::common::Direction::DIR_INVALID) {
  ...
  std::cout << "Junction  Size:   " << junctions_directions_.size()
            << std::endl;
  std::cout << "Junction Directions : " << std::endl;
  for (size_t i = 0; i < junctions_directions_.size(); i++) {
    std::cout << i << " : " << junctions_directions_[i] << "  " << std::endl;
  }
  for (size_t i = 0; i < one_way_plan_.size(); i++) {
    std::cout << "one_way_plan_" << i << std::endl;
    for (auto id : one_way_plan_[i]) {
      std::cout << id << " -- ";
    }
    std::cout << "\none_way_plan_" << i << std::endl;
  }
...
void Routing::LoadParkingAreas() {
  ...
  for (auto area : map_->areaLayer) {
    if (area.hasAttribute("subtype") &&
      ...
      parking_area.parking_info_.set_parking_status(
          legion::interface::ParkingStatus::PARKING_ENABLE);
  ...
  std::cout << "Loaded Parking Space Num : 	" << num_count << std::endl;
}
  ...
  if (UsagedIndex(start_lane.id()) == false) {
    ...
    if (local_adjleft_lanes.size() == 0) {
      ...
      if (MergedIndex(adjleft_lane.id()) == false) {
        ...
        // adjacentLeft lane : " << adjleft_lane.id() << std::endl;
    ...
    if (local_adjright_lanes.size() == 0) {
      ...
      if (MergedIndex(adjright_lane.id()) == false) {
        ...
        // std::cout << "Lanelet : " << start_lane.id() << " merges with
        // adjacentRight lane : " << adjright_lane.id() << std::endl;
...
void Routing::MergePolygons(void* param) {
  ...
  // <<"location y : "<< location_.utm_position().y() << std::endl; auto
  ...
  if (!localmap->laneletLayer.exists(local_locate_lanepair.second)) {
    // std::cout<<"not locate  lanelet in polygon gen "<<std::endl;
    return;
  }
  lanelet::Lanelet start_lane =
  ...
  legion::interface::Polygon2D polygon_to_send;
  polygon_to_send.clear_points();
  for (auto point : res_polygon) {
    legion::interface::Point2D new_point;
...
void Routing::GlobalMsgGen(void* param) {
  ...
  // if (!send_global_)
  // {
  //   // std::cout << "Send global route msg error! " << std::endl;
  //   return;
  // }
  // 初始化
  ...
  */
  ...
  for (auto area : areas) {
    ...
    global_route_msg_.add_cur_slice(lanelet_info);
    // std::cout<<"pub region"<<std::endl;
  }

  if (!temp_map->laneletLayer.exists(temp_locate_lanepair.second)) {
    // std::cout<<"Vehicle out of routing map , no mileage info "<<std::endl;
    // std::cout<<"Please send a new routing request !"<<std::endl;
    std::cout << "Vehicle out of map , no mileage info " << std::endl;
    return;
  ...
  std::vector<legion::interface::PointLLH> global_path;
  for (auto id : no_change_lane) {
    ...
    for (auto point : tmp_lane) {
      legion::interface::PointLLH point_llh;
...
void Routing::CheckStopLine(void* param) {
  ...
  std::vector<legion::common::Direction> junctions_directions;
  std::unique_lock<std::mutex> lock(mutex_);
  ...
  cur_event_.mutable_route_fusion_info()->set_fusion_flag(
      legion::common::IsValid::INVALID);  // 默认不融合直接透传
  if (map_->laneletLayer.exists(match_id)) {
    ...
    // std::cout<<location_area<<std::endl;
    if (location_area == std::string("tunnel")) {
      cur_event_.mutable_route_fusion_info()->set_fusion_flag(
          legion::common::IsValid::VALID);
  ...
  if (junctions_directions_.empty()) {
    cur_junc_.set_direction_flag(legion::common::IsValid::INVALID);
  ...
  if (match_junction_success) {
    ...
    if (junction_seq_index < cur_seq.size() - 1)  // 快到拐弯处
    {
      ...
      if (dis < routing_conf_->junction_distance()) {
        cur_junc_.set_direction_flag(legion::common::IsValid::VALID);
        ...
        if (curr_dir == legion::common::Direction::LEFT) {
          navi_info_msg_.set_crossing_behavior(1);
        } else if (curr_dir == legion::common::Direction::RIGHT) {
  ...
  for (auto data : all_stop_lines) {
    ...
    for (auto id : data.lane_ids) {
      if (match_id == id) {
        ...
        cur_junc_.set_light_flag(legion::common::IsValid::VALID);
        cur_junc_.set_stop_line(data.stop_points);

        legion::interface::TrafficLightMsg TrafficLightsOutput;
        std::vector<legion::interface::TrafficLight> lights_vec;
        auto cur_lane = map_->laneletLayer.get(match_id);
        auto traffic_lights =
            cur_lane.regulatoryElementsAs<lanelet::TrafficLight>();
        for (auto trafic_light : traffic_lights) {
          ...
          for (size_t i = 0; i < lights_linestr.size(); i++) {
            lanelet::LineString3d light_line =
                map_->lineStringLayer.get(lights_linestr[i].id());
            for (size_t j = 0; j < light_line.size(); j++) {
              // 根据地图中交通灯的转向信息,设置消息通信中的交通灯转向信息
              // 后续封装成函数调用,目前对于交通灯实际长宽等属性,消息未定义后续补充完善
              legion::interface::TrafficLightType lightType =
                  legion::interface::TrafficLightType::TYPE_UNKNOWN;
              std::string light_dir =
                  light_line[j].attributeOr("direction", "unknown");
              if (light_dir == "unknown") {
                lightType = legion::interface::TrafficLightType::TYPE_UNKNOWN;
              } else if (light_dir == "straight") {
                lightType = legion::interface::TrafficLightType::STRAIGHT;
              } else if (light_dir == "left") {
                lightType = legion::interface::TrafficLightType::TURN_LEFT;
              } else if (light_dir == "right") {
                lightType = legion::interface::TrafficLightType::TURN_RIGHT;
              } else if (light_dir == "straight_left") {
                lightType =
                    legion::interface::TrafficLightType::STRAIGHT_TURN_LEFT;
              } else if (light_dir == "straight_right") {
                lightType =
                    legion::interface::TrafficLightType::STRAIGHT_TURN_RIGHT;
              } else {
                lightType = legion::interface::TrafficLightType::TYPE_UNKNOWN;
              }
              /* // 后续根据需求新增其他交通灯类型
              // legion::interface::TrafficLightType::CIRCULAR;
              // legion::interface::TrafficLightType::PEDESTRIAN;
              // legion::interface::TrafficLightType::CYCLIST; */

              // double light_length = light_line[j].attributeOr("length", 0.0);
              // double light_width = light_line[j].attributeOr("width", 0.0);

              legion::interface::Point3D light_pt;
              light_pt.set_x(light_line[j].attributeOr("x", 0.0));
              light_pt.set_y(light_line[j].attributeOr("y", 0.0));
              light_pt.set_z(light_line[j].z());

              legion::interface::TrafficLight trafficLight;
        ...
        if (routing_conf_->use_tf_flag()) {
          if (transformer_init_) {
            for (unsigned int i = 0; i < cur_junc_.stop_line_size(); i++) {
        ...
        cur_junc_.set_light_color(
            legion::common::TrafficLightColor::COLOR_UNKNOWN);
        break;
      }
    }
  }
  ...
  // if (!match_success)
  // {
  //   cur_junc_.set_light_flag(legion::common::IsValid::INVALID);
  ...
  // cur_event_.mutable_route_fusion_info()->set_fusion_flag(legion::common::IsValid::VALID);
...
void Routing::LanelinesGen(void* param) {
  ...
  legion::interface::LaneList local_lanelist = legion::interface::LaneList();
  std::vector<legion::interface::LaneLine> local_lanelines;
  ...
  {
    legion::interface::LaneLine temp_laneline = legion::interface::LaneLine();
    std::vector<legion::interface::Point3D> laneline_points;
    ...
    for (auto line_id : lane_line) {
      ...
      temp_laneline.set_lane_type(
          legion::interface::LaneLine::LaneType::SOLID);  // 默认给实线
      if (line_attr == "Dashed") {
        temp_laneline.set_lane_type(
            legion::interface::LaneLine::LaneType::DASHED);
      }
      for (size_t i = 0; i < line.size(); i++) {
        lanelet::Point3d line_point = line[i];
        double d = lanelet::geometry::distance2d(line_point, location_point);
        if (d < distance_threshold) {
          legion::interface::Point3D tmp_p;
    ...
    // temp_laneline.set_lane_type(legion::interface::LaneLine::LaneType::SOLID);
    temp_laneline.set_lane_color(legion::interface::LaneLine::LaneColor::WHITE);
  ...
  for (auto leftmost_line : leftmost_lines)  // 道路左边线
  {
    ...
    temp_laneline.set_pts_abs(laneline_points);
    temp_laneline.set_lane_type(legion::interface::LaneLine::LaneType::CURB);
    temp_laneline.set_lane_color(
        legion::interface::LaneLine::LaneColor::LANE_COLOR_UNKNOWN);
    local_lanelines.push_back(temp_laneline);
  }
...
void Routing::GuideInfoGen(void* param) {
  ...
  if (1) {
    ...
    // cur_lanetype << "\nresidual_length: " << residual_dis <<std::endl;
    ...
    legion::interface::GuideInfo guide_info = legion::interface::GuideInfo();
    legion::interface::GuideRoad current_guide_road =
        legion::interface::GuideRoad();
    legion::interface::GuideRoad next_guide_road =
        legion::interface::GuideRoad();
...
void Routing::NaviInfoPublisher(void* param) {
  ...
  for (size_t i = 0; i < cur_road_slice.size(); i++) {
...
void Routing::MainLoop(void* param) {
  ...
  if (new_request_received_) {
    ...
    if (bMakeNewPlan) {
      ...
      if (bNewPlan) {
        ...
        std::cout << "Plan Status Error : REQUEST_ERROR" << std::endl;
  ...
  while (send_flag_) {
    ...
    if (GetSection(no_change_lane_, local_paths_, location_,
      ...
      send_flag_ = false;
      std::cout << "Plan Status Error : OUT_OF_MAP " << std::endl;
      PublishRoutingResponse(response_);
      return;
    }
    LoadStopLines();
    ...
    section_mileage_ = 0;
    send_flag_ = false;
    std::cout << "Plan Status Success : PLAN_SUCCESS " << std::endl;
    PublishRoutingResponse(response_);
    std::cout << "publish section ok !" << std::endl;
    usleep(1000 * routing_conf_->sleep_time());
    if (match_stop_points_) {
      std::cout << "Stop Path Check Num : " << stop_indexs_.size() << std::endl;
      stop_check_ = true;
    } else {
      std::cout << "Stop Point Not Match" << std::endl;
    }
    lock.unlock();
  }
  return;
  ...
  if (res) {
    ...
    if (routing_conf_->use_tf_flag()) {
      if (transformer_init_) {
        ...
        std::cout << "stop theta in utm " << theta << std::endl;
        std::cout << "yaw_diff " << yaw_diff << std::endl;
        std::cout << "stop theta in odom " << theta + yaw_diff << std::endl;
    ...
    std::cout << "publish stop info ok !" << std::endl;
    stop_check_ = false;
    mileage_enable_ = false;
  }
  return;
...
bool Routing::SingleRoad2FullRoute(const std::vector<int>& single_road,
  ...
  legion::routing::GetPathID(start_lane.id(), all_ids, lanelets_ids,
  ...
  if (local_route.size() > 0 && local_route.at(0).size() > 0) {
    ...
    // {
    //   std::cout << "local_route_"<<i<< std::endl;
    //   for (auto id : local_route[i])
    //   {
    //     std::cout << id <<" -- ";
    //   }
    //   std::cout << "\nlocal_route_"<<i<< std::endl;
    // }
    return true;
  } else {
    std::cout << "Failed To Get Local Route" << std::endl;
    return false;
  }
}

legion::interface::RoutingResponse::PlanStatus Routing::GetSection(
  ...
  for (auto const_lane : const_lanes_region)
  {
    ...
    // std::cout<<const_lane.id()<<std::endl;
  }
  // std::cout << "\n-- lanes_region --" << std::endl;
  // for (auto local_path : local_paths_) {
  //   for (auto id : local_path) {
  ...
  if (!map_->laneletLayer.exists(locate_id)) {
    std::cout << "vehicle not in map, can not decide local route" << std::endl;
  ...
  // std::cout << "\n-- locate_id -- " <<locate_id<< std::endl;
  // std::cout << "\n-- locate_lanes --" << std::endl;
  ...
  if (!match_success) {
    std::cout << " warning : vehicle not match route , can not decide local "
              ...
              << std::endl;
    std::cout << "\n-- singleroad --" << std::endl;
    for (size_t i = 0; i < singleroad.size(); i++) {
      std::cout << "singleroad_" << i << "  " << singleroad[i] << std::endl;
    }
    std::cout << "\n-- singleroad --" << std::endl;
  ...
  for (size_t i = locate_index + 1; i < no_change_lane_.size(); i++) {
    // 往前找
    if (remain_forward < 0) {
      break;
    } else {
  ...
  if (local_singleroad.size() <= 0) {
    std::cout << "local route size =0 warning" << std::endl;
    response_.set_plan_status(
        interface::RoutingResponse::PlanStatus::OUT_OF_MAP);
    return response_.plan_status();
  }
...
void Routing::Section2Response(const std::vector<std::vector<int>>& local_route,
       ...
       i++)  // lanelets_ids_list to vector_map LanePoints_list
  {
    ...
    // std::cout << "lane_id[0]: " << lane_id_list[0]<<std::endl;
    ...
    if (routing_conf_->enable_path_smooth()) {
      legion::routing::FixPathDensity(path, routing_conf_->path_density());
      legion::routing::SmoothPath(path, 0.49, 0.35, 0.01);
    }
    if (path.lane_points_size() > 0)
      std::cout << "size -> " << path.lane_points_size()
                << "  gid = " << int32_t(path.global_id()) << std::endl;
  ...
  if (temp_stop_check) {
    ...
    for (auto stop_lane : temp_stop_lanes) {
      unsigned int stop_index = GetNearestPointInPath(
...
legion::interface::RoutingResponse::PlanStatus Routing::GlobalPlan(
    interface::RoutingRequest request) {
  if (request.num_of_kp() < 2) {
    std::cerr << legion::common::Status::ROUTING_ERROR_REQUEST << std::endl;
    response_.set_plan_status(
  ...
  for (unsigned int i = 0; i < request.key_point_list_size(); i++) {
    ...
    if (match_dis_2d > routing_conf_->search_range()) {
      std::cout << "error : key point not match lanelet : " << lane_pair.second
                ...
                << " , check your  x y poistion" << std::endl;
      std::cout << "closest point :" << lane_pair.first << std::endl;
      response_.set_plan_status(
          interface::RoutingResponse::PlanStatus::REQUEST_ERROR);
      return response_.plan_status();
    }
    ...
    if (!map_->laneletLayer.exists(lane_id)) {
      std::cerr << "Can not find lanelet in range !"
                << legion::common::Status::ROUTING_ERROR_REQUEST << std::endl;
      response_.set_plan_status(
          interface::RoutingResponse::PlanStatus::REQUEST_ERROR);
      return response_.plan_status();
    }
  ...
  if (!route) {
    std::cout << "failed to generate route graph" << std::endl;
    std::cout << "start_lanelet: " << lanes.front().id()
              << " goal_lanelet: " << lanes.back().id() << std::endl;
    response_.set_plan_status(
        interface::RoutingResponse::PlanStatus::REQUEST_ERROR);
    return response_.plan_status();
  }
...
int Routing::GetNearestPointInPath(double x, double y, double z, double yaw,
  ...
  for (size_t i = 0; i < lane.lane_points().size(); i++) {
    auto cpoint = lane.lane_points()[i];
...
int Routing::MatchTrafficlight(
    legion::common::Direction dir,
    const std::vector<legion::interface::TrafficLight>& lights) {
  int idx = -1;
  for (size_t i = 0; i < lights.size(); i++) {
    if (dir == legion::common::Direction::UP)  // 直行
    {
      if (lights[i].type() == legion::common::TrafficLightType::STRAIGHT ||
          lights[i].type() ==
              legion::common::TrafficLightType::STRAIGHT_TURN_LEFT ||
          lights[i].type() ==
              legion::common::TrafficLightType::STRAIGHT_TURN_RIGHT) {
        idx = i;
        break;
      }
    }
    if (dir == legion::common::Direction::LEFT)  // 左转
    {
      if (lights[i].type() == legion::common::TrafficLightType::TURN_LEFT ||
          lights[i].type() ==
              legion::common::TrafficLightType::STRAIGHT_TURN_LEFT) {
        idx = i;
        break;
    ...
    if (dir == legion::common::Direction::RIGHT)  // 右转
    {
      if (lights[i].type() == legion::common::TrafficLightType::TURN_RIGHT ||
          lights[i].type() ==
              legion::common::TrafficLightType::STRAIGHT_TURN_RIGHT) {
        idx = i;
        break;
...
bool Routing::PointInPolygon(legion::interface::Point2D p,
                             std::vector<legion::interface::Point2D>& ptPolygon,
                             int nCount) {
  // 交点个数
  int nCross = 0;
  for (int i = 0; i < nCount; i++) {
    legion::interface::Point2D p1 = ptPolygon[i];
    legion::interface::Point2D p2 =
...
std::vector<legion::interface::Point2D> Routing::getRectVertex(
    legion::interface::Point2D center, float theta, float w, float h) {
  std::vector<legion::interface::Point2D> RectVertex;
  legion::interface::Point2D a, b, c, d;
