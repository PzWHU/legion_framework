...
template <typename T>
void RosMessageManager<T>::PublishGuideInfo(legion::interface::GuideInfo msg) {
...
template <typename T>
void RosMessageManager<T>::PublishLaneList(legion::interface::LaneList msg) {
  if (is_init_ == false) return;
...
template <typename T>
void RosMessageManager<T>::PublishStopInfo(legion::interface::StopInfo msg) {
  if (is_init_ == false)
    return;
...
template <typename T>
void RosMessageManager<T>::PublishPolygon2D(legion::interface::Polygon2D msg) {
  if (is_init_ == false)
    return;
