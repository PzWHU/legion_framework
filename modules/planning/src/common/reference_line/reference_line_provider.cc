/******************************************************************************
 * Copyright 2017 The legion Authors. All Rights Reserved.
 *
...
// using legion::common::VehicleConfigHelper;
using legion::interface::VehicleState;
using legion::common::math::AngleDiff;
using legion::common::math::Vec2d;
// using legion::cyber::Clock;
// using legion::hdmap::HDMapUtil;
// using legion::hdmap::LaneWaypoint;
// using legion::hdmap::MapPathPoint;
// using legion::hdmap::PncMap;
// using legion::hdmap::RouteSegments;
...
void ReferenceLineProvider::UpdateVehicleState(
    const legion::interface::VehicleState& vehicle_state) {
...
void ReferenceLineProvider::GenerateThread() {
  ...
  while (!is_stop_) {
    ...
    std::list<legion::planning::RouteSegments> segments;
...
bool ReferenceLineProvider::SmoothRouteSegment(const legion::planning::RouteSegments &segments,
...
bool ReferenceLineProvider::ExtendSegments(
    const std::vector<legion::planning::LanePointInner> &segments,
    int start_index, int end_index,
    std::vector<legion::planning::LanePointInner> *const truncated_segments)
    const {
  if (segments.empty()) {
...
bool ReferenceLineProvider::CreateRouteSegments(
    std::list<legion::planning::RouteSegments> *route_segments,
    const std::vector<std::pair<std::vector<legion::planning::LanePointInner>,
                                int>> &lane_line,
  ...
  for (unsigned int i = 0; i < lane_line.size(); i++) {
    ...
    std::vector<legion::planning::LanePointInner> lane_points;
...
bool ReferenceLineProvider::UpdateSmoothLines(const interface::RoutingResponse &routing) {
  ...
  std::vector<legion::planning::LanePointInner> smooth_line;
  std::vector<std::vector<legion::interface::LanePoint>> lane_list_points;
  std::vector<int> id;
...
void ReferenceLineProvider::GetEgoLaneType(
    const legion::interface::VehicleState &vehicle_state,
    const legion::interface::RoutingResponse &routing) {
...
void ReferenceLineProvider::UpdateReferenceLine(
    ...
    const std::list<legion::planning::RouteSegments> &route_segments) {
...
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  legion::interface::VehicleState vehicle_state;
  {
    std::lock_guard<std::mutex> lock(vehicle_state_mutex_);
...
bool ReferenceLineProvider::ExtendReferenceLine(
    const legion::interface::VehicleState &vehicle_state,
    legion::planning::RouteSegments *segments,
    reference_line::ReferenceLine *reference_line) {
  // case A
  ...
  std::pair<std::vector<legion::planning::LanePointInner>, int> lane_line;
  ...
  if (future_end_index == prev_segment->end_index()) {
    *segments = *prev_segment;
    *reference_line = *prev_ref;
    // AWARN << "Extend lane line has been smoothed";
    return true;
  }

  std::vector<legion::planning::LanePointInner> shifted_segments;
...
void ReferenceLineProvider::GetAnchorPoints(
  ...
  std::vector<legion::common::LaneLineType> RWTypes;
  std::vector<legion::common::LaneLineType> LWTypes;
...
bool ReferenceLineProvider::SmoothCenterLines(
    std::vector<legion::interface::LanePoint> center_line, double delta_mileage,
    std::vector<legion::planning::LanePointInner> &smooth_line) {
  //里程计算
  int start_pos = 0;
  ...
  y.clear();
  s.clear();
  legion::planning::spline::spline spline_s_x, spline_s_y;
  ...
  smooth_line.clear();
  // legion::planning::LanePointInner smooth_point;
  // legion::interface::Point3D point_3d;
  ...
  // legion::interface::LanePoint smooth_point = center_line[0];
  legion::interface::Point3D point_3d;
  double ks = DBL_MIN;
