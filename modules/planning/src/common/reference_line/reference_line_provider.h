...
using namespace legion::reference_line;
...
class ReferenceLineProvider {
 public:
  ...
  ReferenceLineProvider(const legion::planning::PlanningConf &planning_conf);
   ...
   * @brief
   */
  void UpdateVehicleState(const legion::interface::VehicleState& vehicle_state);
 ...
 private:
   ...
   */
  bool ExtendReferenceLine(const legion::interface::VehicleState &vehicle_state,
   ...
   * @brief
   */
  bool SmoothRouteSegment(const legion::planning::RouteSegments &segments,
                          reference_line::ReferenceLine *reference_line);
   ...
   */
  bool SmoothCenterLines(std::vector<legion::interface::LanePoint> center_line,
   ...
   * @brief
   */
  bool Shrink(const legion::interface::VehicleState &vehicle_state,
              reference_line::ReferenceLine *reference_line);
   ...
   * @brief
   */
  bool UpdateSmoothLines(const legion::interface::RoutingResponse &routing);
 ...
 public:
  ...
  legion::planning::MapMatchInfo GetMapMatchInfo() const {
    return map_match_info_;
  }

  std::vector<std::vector<legion::planning::LanePointInner>> GetSmoothLines()
      /*const*/ {
 ...
 private:
  ...
  std::shared_ptr<legion::reference_line::QpSplineReferenceLineSmoother> smoother_;
  interface::RoutingResponse routing_;
  // 平滑并插值后的中心线跟roting的lane_list 数量相等
  std::vector<std::vector<legion::planning::LanePointInner>> smooth_lines_;
  // 只有三条,并且按固定结构存储，顺序依次为：左、自车道、右
  std::vector<std::pair<std::vector<legion::planning::LanePointInner>, int>> passage_smooth_lines_;
  //针对可行驶的相邻车道实时产生的route_segments，数量为0～3，用于extend
  std::list<legion::planning::RouteSegments> route_segments_cache_;
  ...
  legion::interface::VehicleState vehicle_state_;
