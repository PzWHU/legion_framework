...
void Frame::Init(const legion::planning::PlanningConf *planning_conf) {
...
void Frame::UpdateLaneChangeCmd(
  ...
  static legion::interface::Time last_obu_cmd_msg_time;
...
bool Frame::ExtractVehicleState(
  ...
  legion::interface::PathPoint pose;
...
void Frame::ExtractParkingInfo(const interface::ParkingInfo &parking_info) {
    static legion::interface::Time last_parking_info_time;
...
void Frame::ExtractParkingOutInfo(
    ...
    static legion::interface::Time last_parking_out_info_time;
...
void Frame::ExtractDrivableRegion(
    const legion::interface::DrivableRegion &drivable_region) {
  drivable_region_ = drivable_region;
}

void Frame::ExtractStopInfo(const interface::StopInfo &stop_info) {
  static legion::interface::Time last_stop_info_time;
...
void Frame::CreateDecisionOfTruckAvoidance() {
  ...
  for (auto it : truck_list_) {
    ...
    if (decision_distance >= 0 &&
      ...
      if (it.lane_position() == legion::common::LanePosition::LEFT_LANE) {
        ++left_truck_num;
...
bool Frame::CopyObstacleMsg(
  ...
  std::vector<legion::interface::PredictionObstacle> prediction_obstacles_list;
  ...
  for (auto it_prediction_obstacles : prediction_obstacles_list) {
    ...
    prediction_obstacle_perception_obstacle.set_type(
        (legion::common::ObstacleType)it_prediction_obstacles
            .perception_obstacle()
            .type());
    ...
    prediction_obstacle_perception_obstacle
        .set_lane_position((legion::common::LanePosition)it_prediction_obstacles
                               .perception_obstacle()
                               .lane_position());

    prediction_obstacle_perception_obstacle.set_sub_type(
        (legion::common::ObstacleSubType)it_prediction_obstacles
            .perception_obstacle()
            .sub_type());
    ...
    if ((prediction_obstacle_perception_obstacle.sub_type() ==
             legion::common::ObstacleSubType::ST_BUS ||
         prediction_obstacle_perception_obstacle.sub_type() ==
             legion::common::ObstacleSubType::ST_TRUCK) &&
        (prediction_obstacle_perception_obstacle.lane_position() ==
             legion::common::LanePosition::LEFT_LANE ||
         prediction_obstacle_perception_obstacle.lane_position() ==
             legion::common::LanePosition::RIGHT_LANE)) {
    ...
    for (auto it_trajectory : *it_prediction_obstacles.mutable_trajectory()) {
      ...
      for (auto it_trajectory_points : it_trajectory.trajectory_points()) {
        ...
        // legion::interface::TrajectoryPointInPrediction
        //     prediction_out_array_prediction_obstacle_trajectory_in_prediction_trajectory_point_in_prediction;
    ...
    // legion::planning::PredictionObstacleMsg::Intent intent;
    ...
    if (traffic_events.junction_info().light_color() ==
      ...
      if (stop_line_s <
          ...
          last_behaviour_.lon_state !=
              legion::interface::ADCTrajectory::BehaviourLonState::TRAFFIC_LIGHT_STOP_STATE) {
        return;
      }
