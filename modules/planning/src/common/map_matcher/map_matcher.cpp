...
int MapMatcher::QueryVerticalDistanceWithBufferII(
  ...
  // std::cout << "time diff1 = " << time2-time1 << std::endl;
  // std::cout << "time diff2 = " << time3-time2 << std::endl;
  // std::cout << "time diff3 = " << time4-time3 << std::endl;
  // std::cout << "time diff4 = " << time5-time4 << std::endl << std::endl;
  return index_min;
}
...
int MapMatcher::QueryNearestPointWithBuffer(
    const std::vector<legion::interface::PathPoint> &trajectory,
...
int MapMatcher::QueryNearestPointWithBuffer(
    const std::vector<legion::interface::TrajectoryPoint> &trajectory,
...
int MapMatcher::QueryNearestPointWithBuffer(
    const std::vector<ReferencePoint> &trajectory, const math::Vec2d &position,
    const double yaw, const double buffer, double &dist_min,const legion::common::Direction &direction) {
  if (trajectory.size() < 2) return -1;
  dist_min = DBL_MAX;
  int index_min = -1;
  // 粗搜
  ...
  if (sign > 0)  //点在向量右侧
    ...
    dist_min = 0;
  if(direction==legion::common::Direction::LEFT && dist_min > 8){
    return -1;
  }else if(direction==legion::common::Direction::RIGHT&& dist_min < -8){
    return -1;
  }
  return index_min;
}
...
int MapMatcher::QueryNearestPointWithBuffer(
    const std::vector<legion::interface::LanePoint> &trajectory,
    const math::Vec2d &position, const double yaw, const double buffer,
    double &dist_min) {
  if (trajectory.size() < 2) return -1;
  dist_min = DBL_MAX;
...
int MapMatcher::QueryVerticalDistanceWithBuffer(
    const std::vector<legion::planning::LanePointInner> &trajectory, const math::Vec3d &position,
    const double yaw, const double buffer, double &dist_min) {
  if (trajectory.size() < 2) return -1;  
  dist_min = std::numeric_limits<double>::max();
  int index_min = 0;
...
int MapMatcher::QueryVerticalDistanceWithBuffer(
    const std::vector<legion::interface::LanePoint> &trajectory, const math::Vec3d &position,
    const double yaw, const double buffer, double &dist_min) {
  if (trajectory.size() < 2) return -1;  
  dist_min = std::numeric_limits<double>::max();
  int index_min = 0;
...
int MapMatcher::QueryNearestPointWithBuffer(
    const std::vector<legion::planning::LanePointInner> &trajectory,
