...
// This file defines 2D points, halfplanes and polyhedra as templates. The
// templates work both for concrete numerical types such as float and double as
// well as for "symbol" types such as CasADi variables. The advantage of this is
// that they can be used both when formulating constraints for an optimization
// problem as well as for numerical evaluation outside of such formulations.
...
class Point2D {
 public:
  ...
  Point2D() noexcept { this->m_coord = std::pair<T, T>(T{}, T{}); }

  /// \brief Scalar type Constructor
  Point2D(T x, T y) noexcept { this->m_coord = std::pair<T, T>(x, y); }
  ...
  T norm2() const noexcept { return sqrt(this->dot(*this)); }
  ...
  const std::pair<T, T>& get_coord() const noexcept { return this->m_coord; }
...
class Halfplane2D {
 public:
  ...
  /// \brief Get the coefficients of the left-hand side of the halfplane
  /// equation
 ...
 private:
  ...
  // \brief  This template may be used for concrete number types as well as for
  // symbolic
  //         variable objects. In the case of symbolic variable objects,
  //         normalization is not desired, and the norm check makes no sense
  //         anyway. The enable_if_t feature gating here turns the normalization
  //         on only if there is a concrete numeric type. If not, then the
  //         variant below that does nothing is used.
  template <typename S = T,
            std::enable_if_t<std::is_arithmetic<S>::value, int> = 0>
  void normalize() {
...
class Polytope2D {
 public:
  ...
  /// \param[in] vertices vector of vertices, assumed to be "ordered
  /// anti-clockwise"
  ...
  void rotate_and_shift(const T rotation_angle,
  ...
  void update_halfplanes_from_vertices() noexcept {
    ...
    if (it_begin != it_end) {
      for (auto it = it_begin + 1; it != it_end; ++it) {
        ...
        this->m_halfplanes.push_back(
            Halfplane2D<T>(norm_vector, norm_vector.dot(vertex_current)));
        it_last = it;
      }
      const Point2D<T>& vertex_last = *it_last;
