...
using legion::common::types::float64_t;

// Implement some operations on vectors of scalars
template <typename T>
std::vector<T> operator+(const std::vector<T>& vector1,
...
/// \brief Template for an explicit Runge-Kutta integrator. CasADi has its own,
/// but it
///        for some reason doesn't compose with the IPOPT solver generator
///        :shrug:
template <class X, class U, typename P>
X RK4(X x, U u, P p, std::function<X(X, U, P)> f, const float64_t stepsize,
...
// \brief Version where the function closes over the parameters, this is useful
// where we want to
//        for example have the function be a member of an object (like
//        BicycleModel). The reason this is not used everywhere is that CasADi
//        wants its parameters to be passed in explicitly, otherwise its AD
//        doesn't realize the parameter is there.
template <class X, class U>
X RK4(X x, U u, std::function<X(X, U)> f, const float64_t stepsize,
      const std::size_t steps) {
  // Create a shim that takes a dummy parameter, then ignores it so we can call
  // the above
