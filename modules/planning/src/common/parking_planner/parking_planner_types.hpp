...
class VehicleState {
 public:
  /// \brief No-input constructor sets everything to the zero of the template
  /// parameter.
  //         This is used for example in constructs such as VehicleState{}.
  VehicleState() noexcept : VehicleState({}, {}, {}, {}, {}) {}
  ...
  /// \brief Turn the object into flat vector of doubles (for use with numerical
  /// code)
  std::vector<T> serialize(void) const {
    return std::vector<T>{m_position_x, m_position_y, m_velocity, m_heading,
                          m_steering};
  }

  // \brief Turn a previously serialized state vector back into a VehicleState
  // object
  static VehicleState deserialize(std::vector<T> serialized_states) {
    ...
    return VehicleState(serialized_states[0], serialized_states[1],
                        serialized_states[2], serialized_states[3],
                        serialized_states[4]);
  }
...
class VehicleCommand {
 public:
  ...
  VehicleCommand() noexcept : VehicleCommand({}, {}) {}
  ...
  /// \brief Turn a previously serialized command vector back into a
  /// VehicleCommand object
  static VehicleCommand deserialize(std::vector<T> serialized_commands) {
    if (serialized_commands.size() !=
        VehicleCommand::internal_commands_number) {
      throw std::length_error{
          "Need a vector of length " +
...
class TrajectoryStep {
 public:
  TrajectoryStep(const VehicleCommand<T> command,
                 const VehicleState<T> state) noexcept {
    m_command = command;
    m_state = state;
  }
