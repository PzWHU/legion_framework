...
#include "modules/planning/src/common/math/include/smoothing_spline/spline_seg_kernel.h"
#include "modules/planning/src/common/math/include/smoothing_spline/spline_2d_kernel.h"
    ...
    namespace math
    {
          ...
          : t_knots_(t_knots), spline_order_(spline_order)
      {
        total_params_ =
            (t_knots_.size() > 1 ? 2 * (t_knots_.size() - 1) * (1 + spline_order_)
                                 : 0);
        kernel_matrix_ = Eigen::MatrixXd::Zero(total_params_, total_params_); ///1248*1248
        offset_ = Eigen::MatrixXd::Zero(total_params_, 1);                    ///1248*1
      }
      ...
      const Eigen::MatrixXd Spline2dKernel::kernel_matrix() const
      {
        if (0)
        {
          ...
          for (unsigned int i = 0; i < kernel_matrix_.rows(); i++)
          {
            for (unsigned int j = 0; j < kernel_matrix_.cols(); j++)
            {
              outfile << kernel_matrix_(i, j) << " ";
              //             << endl;
            }
            outfile << endl;
      ...
      const Eigen::MatrixXd Spline2dKernel::offset() const
      {
        if (0)
        {
          ...
          for (unsigned int i = 0; i < offset_.rows(); i++)
          {
            for (unsigned int j = 0; j < offset_.cols(); j++)
            {
              outfile << offset_(i, j) << " ";
              //<< endl;
            }
                                                        ...
                                                        const double weight)
      {
        for (uint32_t i = 0; i + 1 < t_knots_.size(); ++i)
        {
          ...
          Eigen::MatrixXd cur_kernel =
              SplineSegKernel::instance()->NthDerivativeKernel( //SplineSegKernel::instance()->NthDerivativeKernel
