...
namespace legion
{
  namespace planning
  {
    namespace math
    {
      ...
      //**********************
      int Spline2d::set_(const std::vector<double> &t_knots, const uint32_t order)
      {
        t_knots_ = t_knots;
        spline_order_ = order;
        return 1;
      ...
      //**********************
      int Spline2d::set_spline2d(const std::vector<double> &t_knots, const uint32_t order)
      //    : t_knots_(t_knots), spline_order_(order) {
      {
        set_(t_knots, order);
        if (t_knots.size() > 1)
