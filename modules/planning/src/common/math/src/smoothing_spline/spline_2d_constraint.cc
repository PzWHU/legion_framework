...
#include "modules/planning/src/common/math/include/smoothing_spline/spline_2d_constraint.h"
 ...
 **/
          ...
          const std::vector<double> &lateral_bound)
      {
        ...
        Eigen::MatrixXd affine_inequality =
            Eigen::MatrixXd::Zero(4 * t_coord.size(), total_param_); ///2056*1248
        Eigen::MatrixXd affine_boundary =
            Eigen::MatrixXd::Zero(4 * t_coord.size(), 1); ///2056*1
        for (uint32_t i = 0; i < t_coord.size(); ++i)
        {
          const double d_lateral = SignDistance(ref_point[i], angle[i]); ///投影L轴上
          const double d_longitudinal =
              SignDistance(ref_point[i], angle[i] - M_PI / 2.0); ///投影F轴上

          const uint32_t index = FindIndex(t_coord[i]);

          //rel_t是anchor point累积距离s相对于下界knots累积距离s的相对差，说白了就是自变量归一化到[0,1]之间
          ...
          affine_boundary(4 * i + 3, 0) = -d_longitudinal - longitudinal_bound[i]; //设置F轴下界不等式的边界
        }
          ...
          const std::vector<double> &lateral_bound)
      {
        ...
        for (uint32_t i = 0; i < t_coord.size(); ++i)
        {
          ...
          std::vector<double> longitudinal_coef =
              AffineDerivativeCoef(angle[i] - M_PI / 2.0, rel_t);
          for (uint32_t j = 0; j < 2 * (spline_order_ + 1); ++j)
          {
            // upper longi
            affine_inequality(4 * i, index_offset + j) = longi_coef[j];
          ...
          const std::vector<double> &lateral_bound)
      {
        ...
        for (uint32_t i = 0; i < t_coord.size(); ++i)
        {
          ...
          std::vector<double> longitudinal_coef =
              AffineSecondDerivativeCoef(angle[i] - M_PI / 2.0, rel_t);
          for (uint32_t j = 0; j < 2 * (spline_order_ + 1); ++j)
          {
            // upper longi
            affine_inequality(4 * i, index_offset + j) = longi_coef[j];
                                                       ...
                                                       const double angle)
      {
        ...
        Eigen::MatrixXd affine_equality = Eigen::MatrixXd::Zero(1, total_param_); ///1*1248
        Eigen::MatrixXd affine_boundary = Eigen::MatrixXd::Zero(1, 1);            ///1*1
        ...
        Eigen::MatrixXd affine_inequality = Eigen::MatrixXd::Zero(2, total_param_); ///2*1248
        const Eigen::MatrixXd affine_inequality_boundary =
            Eigen::MatrixXd::Zero(2, 1); ///2*1
        ...
        {
          cout << " AddEqualityConstraint ###### : " << endl;
          return false;
        }
      ...
      bool Spline2dConstraint::AddSecondDerivativeSmoothConstraint()
      {
        ...
        Eigen::MatrixXd affine_equality =
            Eigen::MatrixXd::Zero(6 * (t_knots_.size() - 2), total_param_); ///618*1248
        Eigen::MatrixXd affine_boundary =
            Eigen::MatrixXd::Zero(6 * (t_knots_.size() - 2), 1); ///618*1
 ...
 **/
                                                         ...
                                                         const double t) const
      {
        ...
        double x_coef = -sin(angle); //-math::sin(math::Angle16::from_rad(angle));
        double y_coef = cos(angle);  //math::cos(math::Angle16::from_rad(angle));
      ...
      {
        //  return math::InnerProd(
        //      xy_point.x(), xy_point.y(),-sin( angle ),cos( angle ));
        //        math::cos(math::Angle16::from_rad(angle))
        //        -math::sin(math::Angle16::from_rad(angle))
        return xy_point.x() * (-sin(angle)) + xy_point.y() * cos(angle);
      }
