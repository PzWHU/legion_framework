...
#include "modules/planning/src/common/math/include/spiral_curve/spiral_formula.h"
namespace legion {
namespace planning {
namespace spiral {
...
bool CubicSpiralCurve::CalculatePath() {
  if (spiral_config().simpson_size <= 0) {
    return false;
  }
  ...
  for (int32_t nt = 0; nt < spiral_config().newton_raphson_max_iter; ++nt) {
    ...
    jacobi(0, 0) =
        -math::IntegrateBySimpson(sin_ptp_p1, ds, spiral_config().simpson_size);
    jacobi(0, 1) =
        -math::IntegrateBySimpson(sin_ptp_p2, ds, spiral_config().simpson_size);
    jacobi(0, 2) =
        cos_theta[spiral_config().simpson_size - 1] -
        math::IntegrateBySimpson(sin_ptp_sg, ds, spiral_config().simpson_size);

    jacobi(1, 0) =
        math::IntegrateBySimpson(cos_ptp_p1, ds, spiral_config().simpson_size);
    jacobi(1, 1) =
        math::IntegrateBySimpson(cos_ptp_p2, ds, spiral_config().simpson_size);
    jacobi(1, 2) =
        sin_theta[spiral_config().simpson_size - 1] +
        math::IntegrateBySimpson(cos_ptp_sg, ds, spiral_config().simpson_size);
    ...
    q_guess(0) =
        math::IntegrateBySimpson(cos_theta, ds, spiral_config().simpson_size);
    q_guess(1) =
        math::IntegrateBySimpson(sin_theta, ds, spiral_config().simpson_size);
...
bool CubicSpiralCurve::GetPathVec(const std::uint32_t n,
                                  std::vector<PathPoint> *path_points) const {
  CHECK_NOTNULL(path_points);

  // initialization
  if (n < 2 || error() > spiral_config().newton_raphson_tol) {
    return false;  // Status(ErrorCode::PLANNING_ERROR,
                   // "CubicSpiralCurve::get_path_vec");
  }
...
bool CubicSpiralCurve::GetPathVecWithS(
  ...
  for (std::uint32_t i = 0; i < n; ++i) {
    ...
    result[i].set_theta(
        spiral::SpiralFormula::theta_func_k3_a(vec_s[i], a_params) +
        ref_point.theta());
    result[i].set_kappa(
        spiral::SpiralFormula::kappa_func_k3_a(vec_s[i], a_params));
    result[i].set_dkappa(
        spiral::SpiralFormula::dkappa_func_k3_a(vec_s[i], a_params));
  }
