...
bool QuinticSpiralCurve::CalculatePath() {
  ...
  for (int32_t nt = 0; nt < spiral_config().newton_raphson_max_iter; ++nt) {
    ...
    jacobi(0, 0) =
        -math::IntegrateBySimpson(sin_ptp_p3, ds, spiral_config().simpson_size);

    jacobi(0, 1) =
        -math::IntegrateBySimpson(sin_ptp_p4, ds, spiral_config().simpson_size);
    ...
    jacobi(1, 0) =
        math::IntegrateBySimpson(cos_ptp_p3, ds, spiral_config().simpson_size);

    jacobi(1, 1) =
        math::IntegrateBySimpson(cos_ptp_p4, ds, spiral_config().simpson_size);
...
bool QuinticSpiralCurve::GetPathVec(const std::uint32_t n,
...
bool QuinticSpiralCurve::GetPathVecWithS(
