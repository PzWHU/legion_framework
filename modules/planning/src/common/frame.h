...
struct VehicleState {
  ...
  legion::common::math::Polygon2d vehicle_polygon2d;
  double speed;
  double acceleration;
  //The angle between vehicle front wheel and vehicle longitudinal axis
  double steer;
  legion::common::MovingStatus moving_status;
  legion::common::GearPosition gear;
};
...
class Frame {
 public:
  ...
  void Init(const legion::planning::PlanningConf *planning_conf);
   ...
   * interface functions
   ************************/
  legion::planning::VehicleState VehicleState() const { return vehicle_state_; }

  inline void set_map_match_info(
      const legion::planning::MapMatchInfo &map_match_info) {
    map_match_info_ = map_match_info;
    }

    inline const legion::planning::MapMatchInfo &MapMatchInfo() const {
      return map_match_info_;
    }

    inline void set_smooth_lines(
        const std::vector<std::vector<legion::planning::LanePointInner>>
            &smooth_lines) {
      smooth_lines_ = smooth_lines;
    }

    inline const std::vector<std::vector<legion::planning::LanePointInner>>
    SmoothLines() const {
      return smooth_lines_;
    }

    legion::planning::TrajectoryMatchInfo TrajectoryMatchInfo() const {
    ...
    legion::interface::ParkingInfo GetParkingInfo() { return parking_info_; }

    legion::interface::DrivableRegion GetDrivableRegion() {
      return drivable_region_;
    }

    inline const legion::interface::TrafficEvents &GetTrafficEvents() const {
      return traffic_events_;
    }

    inline const legion::interface::GuideInfo &GetGuideInfo() const {
      return guide_info_;
    }
   ...
   private:
    const legion::planning::PlanningConf *planning_params_;  // 参数

    legion::planning::VehicleState vehicle_state_;  // TODO:车辆状态，未赋值
