...
// using legion::common::util::PointFactory;
// using legion::cyber::Clock;
...
void TrajectoryGenerator::Init(const legion::planning::PlanningConf *planning_conf)
{
                                                                 ...
                                                                 const int &global_index)
{
  ...
  if (global_index == 1) {
    if (frame_.LastBehaviour().lat_state ==
        legion::interface::ADCTrajectory::BehaviourLatState::LANE_CHANGE_LEFT_STATE) {
      // 计算侵占程度
      ...
      if (lane_occupancy < reference_point.left_road_width() - 0.1 && lane_occupancy > 0.1) {
        is_need_replan = true;
               ...
               legion::interface::ADCTrajectory::BehaviourLatState::LANE_CHANGE_RIGHT_STATE) {
      // 计算侵占程度
      auto reference_point = frame_.ReferenceLine(frame_.MapMatchInfo().current_lane_index)
                                 .ReferencePoints(frame_.MapMatchInfo().lon_index);
      double angle_diff =
  ...
  for (auto lat_trajectory : lat_trajectory_bundle) {
    ...
    for (; relative_s <= planning_length; relative_s += path_density) {
      ...
      if (d < 0 && fabs(d + matched_ref_point.right_road_width()) <= path_density &&
          !(matched_ref_point.right_line_type() == legion::common::LaneLineType::WHITE_DASHED ||
            matched_ref_point.right_line_type() == legion::common::LaneLineType::YELLOW_DASHED)) {
      ...
      if (d > 0 && fabs(d - matched_ref_point.left_road_width()) <= path_density &&
          !(matched_ref_point.left_line_type() == legion::common::LaneLineType::WHITE_DASHED ||
            matched_ref_point.left_line_type() == legion::common::LaneLineType::YELLOW_DASHED)) {
...

{
  ...
  //         legion::common::LaneLineType::CURB ||
  //     matched_end_point.left_line_type() ==
  //         legion::common::LaneLineType::WHITE_SOLID ||
  //     matched_end_point.left_line_type() ==
  //         legion::common::LaneLineType::YELLOW_SOLID) {
  //   left_margin = left_road_width - trajectory_generator_conf_.out_side_margin();
  // } else if (matched_end_point.left_line_type() ==
  //                legion::common::LaneLineType::WHITE_DASHED ||
  //            matched_end_point.left_line_type() ==
  //                legion::common::LaneLineType::YELLOW_DASHED ||
  //            matched_end_point.left_line_type() ==
  //                legion::common::LaneLineType::LANE_LINE_TYPE_UNKNOWN)
  ...
  if (global_index == 1) {
    ...
    for (auto iter = frame_.ObstacleList().begin(); iter != frame_.ObstacleList().end(); iter++) {
      ...
      if (last_behaviour_lat ==
              legion::interface::ADCTrajectory::BehaviourLatState::AVOIDANCE_BACK_STATE ||
          last_behaviour_lat ==
              legion::interface::ADCTrajectory::BehaviourLatState::AVOIDANCE_BACK_PRE_STATE) {
        min_horizon_index =
            matched_point_index -
      ...
      //     (obs.lane_position() == legion::common::LanePosition::EGO_LANE)*/)
      if (obs_speed < planning_conf_->threshold_static_speed() &&
        ...
        if (left_road_width > right_road_width) {
          if (driving_area_r < driving_area_l) {
            if (driving_area_l < vehicle_param_.width() /*||
                matched_end_point.left_line_type() == legion::common::LaneLineType::CURB ||
                matched_end_point.left_line_type() == legion::common::LaneLineType::WHITE_SOLID ||
                matched_end_point.left_line_type() == legion::common::LaneLineType::YELLOW_SOLID*/) {
            ...
            if (driving_area_r < vehicle_param_.width() /*||
                /*matched_end_point.right_line_type() == legion::common::LaneLineType::CURB ||
                matched_end_point.right_line_type() == legion::common::LaneLineType::WHITE_SOLID ||
                matched_end_point.right_line_type() == legion::common::LaneLineType::YELLOW_SOLID*/) {
              AINFO << " The area is impassable.";
              break;
            } else {
  ...
  if (matched_index >= 0) {
    ...
    for (unsigned int i = matched_index + 1; i < reference_points.size(); ++i) {
      ...
      if ((reference_points[i].right_road_width() + dist_min < safe_margin &&
           (reference_points[i].right_line_type() == legion::common::LaneLineType::CURB ||
            reference_points[i].right_line_type() == legion::common::LaneLineType::WHITE_SOLID ||
            reference_points[i].right_line_type() == legion::common::LaneLineType::YELLOW_SOLID)) ||
          (reference_points[i].left_road_width() - dist_min < safe_margin &&
           (reference_points[i].left_line_type() == legion::common::LaneLineType::CURB ||
            reference_points[i].left_line_type() == legion::common::LaneLineType::WHITE_SOLID ||
            reference_points[i].left_line_type() == legion::common::LaneLineType::YELLOW_SOLID))) {
        is_narrow = true;
        break;
      }
