...
void BehaviourSelector::Init(const legion::planning::PlanningConf *planning_conf)
{
...
void BehaviourSelector::BehaviourParamsInit(const legion::planning::PlanningConf *planning_conf)
{
...
void BehaviourSelector::UpdateTargetSpeed()
{
  ...
  ahead_map_limit_speed_ = (ahead_map_limit_speed_ > vehicle_param_.speed_limit())
                               ...
                               : ahead_map_limit_speed_;
  // 动态限速
  legion::interface::TrafficEvents traffic_events = frame_.GetTrafficEvents();
...
void BehaviourSelector::UpdateInwardOutwardStatus()
{
  ...
  if (traffic_events.junction_info().direction_flag() == legion::common::IsValid::VALID &&
    ...
    if (traffic_events.junction_info().direction() == legion::common::Direction::RIGHT &&
    ...
    } else if (traffic_events.junction_info().direction() == legion::common::Direction::LEFT &&
    ...
    } else if (traffic_events.junction_info().direction() == legion::common::Direction::UP &&
...
void BehaviourSelector::GenerateBehaviorState()
{
  ...
  if (final_trajectory_.trajectory_cost().id == "greenbelt_occlusion_stop_wall") {
    current_behaviour_.lon_state = legion::interface::ADCTrajectory::BehaviourLonState::EMYIELD;
  }
                                                          ...
                                                          std::vector<TrajectoryPoint> &trajectory)
{
  ...
  for (unsigned int i = 0; i < trajectory.size(); ++i) {
    ...
    if (temp_velocity <= 0.0) {
      temp_velocity = 0.0;
      if (accel <= 0.0)
        trajectory.at(i).set_gear(legion::common::GEAR_PARKING);
      else
        trajectory.at(i).set_gear(legion::common::GEAR_DRIVE);
    } else {
                                              ...
                                              double &motion_length, double &motion_accel)
{
  ...
  //     legion::interface::ADCTrajectory::BehaviourLonState::ACC) {
  //       std::cout << std::fixed;
