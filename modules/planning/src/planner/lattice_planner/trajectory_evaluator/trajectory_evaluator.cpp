...
void TrajectoryEvaluator::Init(const legion::planning::PlanningConf *planning_conf)
{
  EvaluatorParamsInit(planning_conf);
  EvaluatorVariableInit();
}

void TrajectoryEvaluator::EvaluatorParamsInit(const legion::planning::PlanningConf *planning_conf)
{
...
bool TrajectoryEvaluator::CalculateRightmostCosts()
{
  ...
  if (station_index_global_ > -1 ||
      ...
      (frame_->MapMatchInfo().priority[1] == 1 &&
       ...
       frame_->GetTrafficEvents().junction_info().direction() != legion::common::Direction::LEFT)) {
    return true;
  }
    ...
    const legion::interface::JunctionInfo &junction_info)
{
  ...
  if (frame_->GetGuideInfo().round_status() == 0 &&
      junction_info.direction() == legion::common::Direction::LEFT &&
  ...
  } else if (junction_info.direction() == legion::common::Direction::UP &&
             junction_info.distance_to_junction() < evaluator_params_->distance_to_junction()) {
    // 自车在可左转车道
    if (ego_lane_type == LaneInfoType::LEFT_TURN_NO_TURN_AROUND ||
        ego_lane_type == LaneInfoType::LEFT_TURN_AND_TURN_AROUND ||
  ...
  } else if (junction_info.direction() == legion::common::Direction::RIGHT &&
             junction_info.distance_to_junction() < evaluator_params_->distance_to_junction()) {
          ...
          if (fabs(collision_time_diff) < evaluator_params_->collision_time_diff()) {
            ...
            if (fabs(diff_theta_prediction) > 0.75 * M_PI &&
                ...
                (obj.perception_obstacle().lane_position() ==
                     legion::common::LanePosition::NEXT_LEFT_LANE ||
                 obj.perception_obstacle().lane_position() ==
                     legion::common::LanePosition::NEXT_RIGHT_LANE)) {
            ...
            if (fabs(diff_theta_prediction) >= M_PI / 6.0 &&
                ...
                (obj.perception_obstacle().sub_type() == legion::common::ObstacleSubType::ST_TRUCK ||
                 obj.perception_obstacle().sub_type() == legion::common::ObstacleSubType::ST_VAN)) {
...
void TrajectoryEvaluator::CaluculateCollisionCosts(
    ...
    const std::map<int, int> &obstacles_lane_id, const legion::interface::GuideRoad &current_road,
        ...
        if (CalculateDynamicObjPolygonCollision(obj_list.at(io), extract_path.at(ie), adc_polygon,
          ...
          if (current_road.road_type() == 2 &&
              obj_list.at(io).intent().type() ==
                  legion::interface::ObstacleIntent::Type::INTENT_TURN_LEFT_STATE) {
    ...
    const PotentialField &potential_field, const std::vector<TrajectoryPoint> &extract_path)
{
  ...
  for (size_t ir = 0; ir < extract_path.size(); ir += ir_step) {
    ...
    // AINFO << " INDEX START : " << index_start << std::endl;

    if (imatched < 0) {
      // std::cout << "-------- trajectory point project potential field,
      // matched failed! -------- imatched: " << imatched << std::endl;
      break;
    }
