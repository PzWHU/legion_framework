...
void PotentialField::InitPotentialFieldFromFrenet(
  ...
  for (size_t irow = 0; irow < sample_matrix.size(); ++irow) {
    ...
    for (size_t icol = 0; icol < sample_matrix.at(irow).size(); ++icol) {
      ...
      // std::cout<< "single_grid.s : " << single_grid.s <<std::endl;
      single_grid.d = sample_matrix.at(irow).at(icol).second;
      // std::cout<< "single_grid.d : " << single_grid.d <<std::endl;
...
void PotentialField::CalculatePotentialField(
  ...
  for (size_t i = 0; i < grids_.size(); ++i) {
    ...
    // length_front_ << std::endl;
    grids_.at(i).potential_val = grids_.at(i).obj_potential_val * w;
  }
  // std::cout<< "Finish -- Potential -- Field" <<std::endl;
}
...
for (size_t j = 0; j < grids_.size(); ++j)  //grids_ loop
{
	...
	// std::cout<< "x_index : " << x_index << ", value : "<< *x_addr <<std::endl;
	...
	// std::cout<< "y_index : " << y_index << ", value : "<< *y_addr <<std::endl;

	int final_index = num_col*(y_index-1) + x_index;
	final_grids.at(final_index).potential_val += grids_.at(j).potential_val;
	// std::cout<< "j: " << j << ", value : "<< grids_.at(j).potential_val <<std::endl;
	// std::cout<< "final index : " << final_index << ", value : "<< final_grids.at(final_index).potential_val <<std::endl;
}
#endif
}
...
void PotentialField::EvaluateSidePassageSafety(
  ...
  for (unsigned int i = 0; i < frame->ReferenceLines().size(); ++i) {
    ...
    for (unsigned int j = 0; j < frame->ObstacleList().size(); ++j) {
      ...
      if (((obs.lane_position() == legion::common::LanePosition::LEFT_LANE ||
            obs.lane_position() ==
                legion::common::LanePosition::LANE_POSITION_UNKNOWN) &&
           i == 0) ||
          ((obs.lane_position() == legion::common::LanePosition::EGO_LANE ||
            obs.lane_position() ==
                legion::common::LanePosition::LANE_POSITION_UNKNOWN) &&
           i == 1) ||
          ((obs.lane_position() == legion::common::LanePosition::RIGHT_LANE ||
            obs.lane_position() ==
                legion::common::LanePosition::LANE_POSITION_UNKNOWN) &&
           i == 2)) {
