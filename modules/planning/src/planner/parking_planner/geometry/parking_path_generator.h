...
class ParkingPathGenerator {
 public:
   ...
   * @return 1设置成功，0不成功。
   */
  virtual Status ComputeForwardPathInStage1(const legion::interface::PathPoint &cur_pose) = 0;
   ...
   */
  bool ExtendPath(const legion::interface::PathPoint &start_pos, const double &extend_length,
                  const double &interval, const legion::common::GearPosition &gear_position,
                  std::vector<legion::interface::PathPoint> &extend_path);
  /**
  ...
  bool CalculateCirclePath(const double &ox, const double &oy, const double &theta0,
                           ...
                           std::vector<legion::interface::PathPoint> &out_path);
   ...
   * @return 1设置成功，0不成功。
   */
  virtual Status ComputeBackwardPathInStage1(const legion::interface::PathPoint &cur_pose) = 0;

  virtual Status ComputeForwardPathInStage2(const legion::interface::PathPoint &cur_pose) = 0;

  virtual Status ComputeBackwardPathInStage2(const legion::interface::PathPoint &cur_pose) = 0;
  ...
  virtual void SetParkingSpot(const legion::interface::ParkingInfo &parking_spot) = 0;

  /**
   * @brief 判断pose与road_polygon_是否有碰撞
   */
  bool IsEnoughFreeSpace(const legion::interface::PathPoint &pose);

  // 接口函数
  legion::interface::ParkingInfo GetParkingSpot() { return parking_spot_; }
  // std::vector<legion::interface::PathPoint> GetKeyPose() { return key_pose_;
  // }
  std::vector<legion::interface::PathPoint> GetForwardPath() { return forward_path_; }
  std::vector<legion::interface::PathPoint> GetBackwardPath() { return backward_path_; }

  void SetVehicleParam(const legion::interface::VehicleParam &vehicle_param);

  void UpdateSpiralCurveConfig(SpiralCurveConf spiral_curve_conf);

 protected:
  legion::interface::ParkingInfo parking_spot_;              // 泊车位信息
  std::vector<legion::interface::PathPoint> forward_path_;   // 前进部分的路径
  std::vector<legion::interface::PathPoint> backward_path_;  // 倒车部分的路径

  double in_distance_;  // 倒车过程中，方向盘开始回正时进入车库的深度

  std::vector<math::Polygon2d> road_polygon_;  // 用于描述停车位附近不可通行的区域

  legion::interface::VehicleParam vehicle_param_;   // 车辆参数
