...
void ParkingManager::VerticalParkingBeginStateUpdate(const std::string &state_name, int state)
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_BEGIN_STATE;
  if (park_in_) {
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_FORWARD_BEGIN_STATE;
  if (keep_steer) {
    steer_first_ = true;
    keep_steer = false;
  }
  ...
  if (parking_status_ == Status::Ok()) {
    ...
    target_gear_position_ = legion::common::GearPosition::GEAR_DRIVE;
    start_time_ = TimeTool::NowToSeconds();
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_FORWARD_STATE;
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_FORWARD_FINISH_STATE;
  keep_steer = true;
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_BACKWARD_BEGIN_STATE;
  if (keep_steer) {
    steer_first_ = true;
    keep_steer = false;
  }

  legion::interface::PathPoint start_pose;
  // 计算倒车路径
  ...
  if (parking_status_ == Status::Ok()) {
    ...
    target_gear_position_ = legion::common::GearPosition::GEAR_REVERSE;
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_BACKWARD_STATE;
  // 基于倒车路径和当前位置更新路径
  if (parking_path_.size() > 0) {
    ParkingPathEraseUpdate();
    target_gear_position_ = legion::common::GearPosition::GEAR_REVERSE;
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_BACKWARD_FINISH_STATE;
  keep_steer = true;
  if (stage1_move_ == Stage1Move::MOVE1 && TimeTool::GetTimeDiffNow(start_time_) > 1.0) {
    start_time_ = TimeTool::NowToSeconds();
    parking_status_ = Status::Ok();
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::VERTICAL_PARKING_FINISH_STATE;
  keep_steer = true;
  if (parking_status_ != Status::Ok()) {
