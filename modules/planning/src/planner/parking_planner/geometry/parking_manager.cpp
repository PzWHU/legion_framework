...
void ParkingManager::Init(const PlanningConf *planning_conf)
{
  ...
  behaviour_lat_state_ = legion::interface::ADCTrajectory::BehaviourLatState::LAT_NOT_ACTIVE_STATE;
  behaviour_lon_state_ = legion::interface::ADCTrajectory::BehaviourLonState::NORMAL;
  parking_stage_ = ParkingStage::STAGE_NONE;
  stage1_move_ = Stage1Move::MOVE_NONE;
  target_gear_position_ = legion::common::GearPosition::GEAR_PARKING;
...
bool ParkingManager::JudgeNeedMaxTurn(const legion::interface::PathPoint p1,
                                      const legion::interface::PathPoint p2)
{
  double dx = p1.x() - p2.x();
  double dy = p1.y() - p2.y();
...
legion::common::Status ParkingManager::UpdateParkingInfo(legion::common::ParkingType parking_type)
{
  switch (parking_type) {
    case legion::common::ParkingType::PARALLEL_PARKING: {
    ...
    case legion::common::ParkingType::VERTICAL_PARKING: {
      double back_distance =
  ...
  return legion::common::Status::Ok();
}

legion::common::Status ParkingManager::UpdateContext()
{
  ...
  switch (parking_type_) {
    ...
    case legion::common::ParkingType::VERTICAL_PARKING: {
      ...
      //           << std::endl;
      // std::cout << "theta: " << parking_info_.theta() << std::endl;
      // std::cout << "width: " << parking_info_.width() << std::endl;
      // std::cout << "length: " << parking_info_.length() << std::endl;
...
legion::common::Status ParkingManager::IsGeometryOK(Frame *frame)
{
  legion::interface::ParkingInfo parking_info = frame->GetParkingInfo();
  legion::interface::PathPoint cur_pose = frame->VehicleState().pose;
  Transfer transfer_pg;
  legion::interface::PathPoint local_cur_pose;
  ...
  if (local_cur_pose.y() - vehicle_param_.front_edge_to_center() < out_d) {
    ...
    legion::interface::PathPoint Aim_pathpoint;
    ...
    double yaw =
        legion::common::math::NormalizeAngle(Aim_pathpoint.theta() - local_cur_pose.theta());
...
legion::common::Status ParkingManager::Process(Frame *frame,
                                              ...
                                              legion::interface::PlanningAnalysis *ptr_analysis)
{
  ...
  if (park_in_) {
    ...
    if (parking_info_.parking_type() == legion::common::ParkingType::INVALID_PARKING) {
      return Status(Status::ErrorCode::PLANNING_ERROR,
                    "parking_info parking_type() is "
                    "legion::common::ParkingType::INVALID_PARKING.");
    }
    ...
    if (first_hit_ == false) {
      // TODO 添加水平泊出方案
      parking_type_ = legion::common::ParkingType::VERTICAL_PARKING;
      UpdateContext();
  ...
  transfer_pg_.TransferG2L(vehicle_current_pose_,
                           local_vehicle_current_pose_);  // 始点坐标转换
  parking_status_ = legion::common::Status::Ok();
  // 状态机更新
  parking_sm_->OnUpdate();

  if (parking_status_ != legion::common::Status::Ok()) {
  ...
  std::vector<legion::interface::PathPoint> parking_path_global;
  ...
  if (in_collision) {
    behaviour_lon_state_ = legion::interface::ADCTrajectory::BehaviourLonState::STOP;
  } else if (parking_sm_->GetCurrentStateName() == "ParkingFinishState") {
    behaviour_lon_state_ = legion::interface::ADCTrajectory::BehaviourLonState::PRECISE_STOP;
  } else {
    behaviour_lon_state_ = legion::interface::ADCTrajectory::BehaviourLonState::NORMAL;
  }
...
{
  if (vehicle_state_.moving_status == legion::common::MovingStatus::STATIONARY) {
    return true;
  }
  return false;
}
...
bool ParkingManager::IsParkingPrecisely()
{
  ...
  if (parking_type_ == legion::common::ParkingType::PARALLEL_PARKING) {
    theta = theta + M_PI_2;
  }
...
void ParkingManager::UpdateADCTrajectory(legion::interface::Trajectory *trajectory,
                                         legion::interface::ADCTrajectory *ptr_adc_trajectory)
{
  ...
  ptr_adc_trajectory->set_driving_mode(legion::common::DrivingMode::COMPLETE_AUTO_DRIVE);
  ptr_adc_trajectory->set_trajectory_points(trajectory->mutable_trajectory_points());
}

bool ParkingManager::TransferPark2Global(const std::vector<legion::interface::PathPoint> &path_p,
                                         std::vector<legion::interface::PathPoint> &path_g)
{
  int num = path_p.size();
  if (num <= 0) {
    AERROR << "!!!path_p is empty" << std::endl;
    return false;
  }
...
void ParkingManager::GetSafetyBorderPolygon(const legion::interface::PathPoint &curr_state,
                                            const legion::interface::VehicleParam &vehicle_param,
...
bool ParkingManager::ComputeLonInfoLinear(
    const std::vector<legion::interface::PathPoint> &parking_path, const double &cur_speed,
    ...
    bool in_collision, Trajectory *ptr_trajectory)
{
  ...
  for (unsigned int i = 1; i < parking_path.size(); ++i) {
    legion::interface::PathPoint path_point = parking_path[i];
...
bool ParkingManager::ComputeLonInfo(const std::vector<legion::interface::PathPoint> &parking_path,
...
bool ParkingManager::IsVehicleSteerNearCmd()
{
  ...
  std::cout << steer_diff << "Wheel steering did not meet expectations." << std::endl;
                                    ...
                                    const double &collision_distance, Trajectory *ptr_trajectory)
{
  ...
  std::vector<legion::interface::TrajectoryPoint> *ptr_trajectory_points =
...
bool ParkingManager::ExtendTrajectory(const legion::interface::PathPoint &start_pos,
                                      ...
                                      const GearPosition &gear_flag, Trajectory &extend_trajectory)
{
  ...
  if (gear_flag == legion::common::GearPosition::GEAR_REVERSE) interval_tmp *= -1.0;
  ...
  while (s <= extend_length) {
    ...
    path_point.set_y(y);
    trajectory_point.set_a(0.0);
    trajectory_point.set_gear(legion::common::GearPosition::GEAR_PARKING);
                                 ...
                                 legion::interface::Trajectory trajectory)
{
  ...
  std::vector<legion::interface::PathPoint> tra_points;
  legion::interface::PathPoint tra_point;
  ...
  if ((tra_points.size() > 0) == true) {
    ...
    for (int32_t i = 0; i < preview_window_size; i++) {
      ...
      legion::common::math::Polygon2d safeboundingbox(t_safeboundingbox);
...
legion::interface::Polygon2D ParkingManager::Polygon2d_to_Polygon2D(
    const legion::common::math::Polygon2d p1)
{
  Polygon2D p2;
...
bool ParkingManager::CollisionCheck(const std::vector<legion::interface::PathPoint> &parking_path,
                                    const double &safe_width, const double &safe_length,
...
legion::interface::TrajectoryPoint ParkingManager::PathPoint2WayPoint(legion::interface::PathPoint in)
{
  legion::interface::TrajectoryPoint out;
...
void ParkingManager::UpdateVehicleCurrentPose(
    const legion::interface::PathPoint vehicle_current_pose)
{
  vehicle_current_pose_ = vehicle_current_pose;
}

void ParkingManager::UpdateTargetParkingPose(const legion::interface::PathPoint target_parking_pos)
{
...
void ParkingManager::UpdateTargetParkingPose(legion::interface::ParkingInfo parking_info)
{
