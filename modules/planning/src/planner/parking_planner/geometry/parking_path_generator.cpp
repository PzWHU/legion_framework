...
class ParkingPathGenerator
***********************************************************/
bool ParkingPathGenerator::ExtendPath(const legion::interface::PathPoint &start_pos,
                                      const double &extend_length, const double &interval,
                                      const legion::common::GearPosition &gear_position,
                                      std::vector<legion::interface::PathPoint> &extend_path)
{
  double _interval = interval;
  legion::interface::PathPoint temp_p;
  temp_p = start_pos;

  if (gear_position == legion::common::GearPosition::GEAR_REVERSE) _interval *= -1.0;
                                               ...
                                               std::vector<legion::interface::PathPoint> &out_path)
{
  ...
  legion::interface::PathPoint temp_pose;
  out_path.clear();
...
bool ParkingPathGenerator::IsEnoughFreeSpace(const legion::interface::PathPoint &pose)
{
...
void ParkingPathGenerator::SetVehicleParam(const legion::interface::VehicleParam &vehicle_param)
{
