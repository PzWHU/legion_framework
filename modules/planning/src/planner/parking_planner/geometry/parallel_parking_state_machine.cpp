...
void ParkingManager::ParallelParkingBeginStateUpdate(const std::string &state_name, int state)
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_BEGIN_STATE;
  // 延迟0.5s
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_FORWARD_BEGIN_STATE;
  // 清空轨迹点
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_FORWARD_STATE;
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_FORWARD_FINISH_STATE;
  // 延迟１s
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_BACKWARD_BEGIN_STATE;
  if (parking_path_.size() <= 0) {
    parking_status_ = Status(Status::ErrorCode::PLANNING_ERROR, "parking_path_ is null");
    start_time_ = TimeTool::NowToSeconds();
    parking_sm_->NextState("ParallelParkingFinish");
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_BACKWARD_STATE;
  double match_dis = DBL_MAX;
  int index = -1;
  // 基于倒车路径和当前位置更新路径
  if (parking_path_.size() > 0) {
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_BACKWARD_FINISH_STATE;
...
{
  behaviour_lat_state_ =
      legion::interface::ADCTrajectory::BehaviourLatState::PARALLEL_PARKING_FINISH_STATE;
