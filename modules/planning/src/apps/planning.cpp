...
void Planning::Init() {
  ...
  {
    planning_conf_ = std::make_shared<legion::planning::PlanningConf>();
...
void Planning::TaskStop() {
...

  {
    ...
    plan_status_ = Status(Status::ErrorCode::PLANNING_ERROR_NOT_READY,
                          "PLANNING_ERROR_NOT_READY.");
    adc_trajectory_ = legion::interface::ADCTrajectory();
    routing_response_ = legion::interface::RoutingResponse();
    traffic_events_ = legion::interface::TrafficEvents();
    guide_info_ = legion::interface::GuideInfo();
    parking_info_ = legion::interface::ParkingInfo();
    stop_info_ = legion::interface::StopInfo();
    traffic_light_msg_ = legion::interface::TrafficLightMsg();
    location_ = legion::interface::Location();
    prediction_obstacles_ = legion::interface::PredictionObstacles();
    lane_list_ = legion::interface::LaneList();
    chassis_ = legion::interface::Chassis();
    sotif_monitor_result_ = legion::interface::SotifMonitorResult();
    obu_cmd_msg_ = legion::interface::ObuCmdMsg();
    drivable_region_ = legion::interface::DrivableRegion();
    parking_out_info_ = legion::interface::ParkingOutInfo();
    Reset();
  }
  ...
  if (enable_log_debug_ && trajectory_point_log_file_.is_open()) {
    ...
    for (auto& trajectory_point : trajectory_points) {
      ...
      sstream << trajectory_point.path_point().s() << ","
              ...
              << ",";
      trajectory_point_log_file_ << sstream.str() << std::endl;
    }
  }
}
...
void Planning::PublishPlanningCmd(legion::interface::PlanningCmd planning_cmd) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Planning::PublishPlanningAnalysis(
    legion::interface::PlanningAnalysis planning_analysis) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Planning::PublishTrajectoryArray(
    legion::interface::TrajectoryArray trajectory_array) {
#if LCM_ENABLE
  if (message_manager_.count("LCM") > 0)
...
void Planning::PublishEvents(legion::interface::Events events) {
  legion::interface::Header header;
...
std::shared_ptr<legion::planning::PlanningConf> Planning::GetConf() const {
...
void Planning::HandleRoutingResponse(
  ...
  }
  {
    ...
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = routing_response.header();
    ...
    if (routing_response_.replan_flag() ==
        legion::interface::RoutingResponse::ReplanFlag::REPLAN_FLAG_HUMAN) {
      legion::interface::Header stop_info_header = routing_response.header();
...
void Planning::HandleGuideInfo(legion::interface::GuideInfo guide_info) {
  if (is_init_ == false) {
    return;
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = guide_info.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Planning::HandleTrafficEvents(
  ...
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = traffic_events.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Planning::HandleParkingInfo(legion::interface::ParkingInfo parking_info) {
  if (is_init_ == false) {
    return;
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = parking_info.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Planning::HandleStopInfo(legion::interface::StopInfo stop_info) {
  if (is_init_ == false) {
    return;
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = stop_info.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Planning::HandleLocation(legion::interface::Location location) {
  if (is_init_ == false) {
    return;
  }
  {
    ...
    location_ = location;
    legion::interface::PointENU utm_position = location_.utm_position();
...
void Planning::HandlePredictionObstacles(
  ...
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = prediction_obstacles.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Planning::HandleLaneList(legion::interface::LaneList lane_list) {
  if (is_init_ == false) {
    return;
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = lane_list.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Planning::HandleChassis(legion::interface::Chassis chassis) {
  if (is_init_ == false) {
    return;
  }
  {
...
void Planning::HandleSotifMonitorResult(
  ...
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = sotif_monitor_result.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Planning::HandleObuCmdMsg(legion::interface::ObuCmdMsg obu_cmd_msg) {
  if (is_init_ == false) {
    return;
  }
  {
...
void Planning::HandleDrivableRegion(
  ...
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = drivable_region.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Planning::HandleParkingOutInfo(
  ...
  }
  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (planning_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = parking_out_info.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
