...
bool Planning::AutoDrivingEnvInit() {
  ...
  last_adc_trajectory_ = legion::interface::ADCTrajectory();
  planning_cmd_ = legion::interface::PlanningCmd();
  planning_analysis_ = legion::interface::PlanningAnalysis();
  faults_ = legion::interface::Faults();
  events_ = legion::interface::Events();
...
void Planning::PlanningStartStateUpdate(const std::string &state_name,
  ...
  // 事件、故障初始化
  std::vector<legion::interface::Event> events;
...
void Planning::LatticePlannerStateUpdate(const std::string &state_name,
  ...
  if (plan_status_ != Status::Ok()) {
    ...
    if (adc_trajectory_.behaviour_lat_state() ==
            legion::interface::ADCTrajectory::BehaviourLatState::STATION_ARRIVED_STATE &&
        (local_view_.chassis_.moving_status() == MovingStatus::STATIONARY) &&
        (local_view_.chassis_.epb_level() == EPBLevel::APPLIED)) {
      driving_flag_ = DrivingFlag::DRIVING_INVALID;
    ...
    if (is_manual_lc == true) {
      if (frame_.LaneChangeDirection() ==
        ...
        legion::interface::Event event_lane_change;
      ...
      legion::interface::Event event_lane_change;
      if (adc_trajectory_.behaviour_lat_state() ==
          legion::interface::ADCTrajectory::BehaviourLatState::LANE_CHANGE_LEFT_PRE_STATE){
        ...
        events_.set_events(events);
          ...
          legion::interface::ADCTrajectory::BehaviourLatState::LANE_CHANGE_RIGHT_PRE_STATE) {
    ...
    legion::interface::Event event_stop;
    if(adc_trajectory_.behaviour_lon_state() ==
      legion::interface::ADCTrajectory::BehaviourLonState::STOP &&
...
void Planning::ESlowStopPlanStateUpdate(const std::string &state_name,
  ...
  adc_trajectory_.set_behaviour_lat_state(
      legion::interface::ADCTrajectory::BehaviourLatState::EMERGENCY_STATE);
  adc_trajectory_.set_driving_mode(common::DrivingMode::COMPLETE_AUTO_DRIVE);
  // b. 发出紧急事件和状态
  events_.clear_events();
  std::vector<legion::interface::Event> events;
  legion::interface::Event event_eslow_stop;
  ...
  if (stop_distance < planning_conf_->emergency_lower_limit_length()) {
    events_.clear_events();
    planner_flag_ = PlannerFlag::E_QUICK_STOP_PLAN;
    legion::interface::Event event_equick_stop;
...
void Planning::EQuickStopPlanStateUpdate(const std::string &state_name,
  ...
  legion::interface::PathPoint utm_pose;
...
void Planning::AfterHumanDriving() {
  ...
  adc_trajectory_.set_behaviour_lat_state(
      legion::interface::ADCTrajectory::BehaviourLatState::LAT_NOT_ACTIVE_STATE);
  adc_trajectory_.set_behaviour_lon_state(
      legion::interface::ADCTrajectory::BehaviourLonState::LON_NOT_ACTIVE_STATE);
}
