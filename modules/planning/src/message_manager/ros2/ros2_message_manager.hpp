...
using ::ros2_interface::msg::ADCTrajectory;
using ::ros2_interface::msg::Chassis;
using ::ros2_interface::msg::DrivableRegion;
using ::ros2_interface::msg::Events;
using ::ros2_interface::msg::Faults;
using ::ros2_interface::msg::LaneList;
using ::ros2_interface::msg::Location;
using ::ros2_interface::msg::GuideInfo;
using ::ros2_interface::msg::ObuCmdMsg;
using ::ros2_interface::msg::ParkingInfo;
using ::ros2_interface::msg::ParkingOutInfo;
using ::ros2_interface::msg::ParkingStateDisplay;
using ::ros2_interface::msg::PlanningAnalysis;
using ::ros2_interface::msg::PlanningCmd;
using ::ros2_interface::msg::PredictionObstacles;
using ::ros2_interface::msg::RoutingResponse;
using ::ros2_interface::msg::SotifMonitorResult;
using ::ros2_interface::msg::StopInfo;
using ::ros2_interface::msg::TrafficEvents;
using ::ros2_interface::msg::TrafficLightMsg;
using ::ros2_interface::msg::TrajectoryArray;

template <typename T>
Ros2MessageManager<T>::Ros2MessageManager() : Node{"planning"} {}
template <typename T>
void Ros2MessageManager<T>::Init(T* t) {
  ...
  // 使用 transient_local 确保晚启动的订阅者能收到历史消息
  adc_trajectory_pub_ = create_publisher<::ros2_interface::msg::ADCTrajectory>(
      "/planning/ADCTrajectory", 
      ROS2_QOS_LARGE_DATA(), 
      PubAllocT{});

  planning_cmd_pub_ = create_publisher<::ros2_interface::msg::PlanningCmd>(
      "/planning/PlanningCmd", 
      ROS2_QOS_CRITICAL_CMD(), 
      PubAllocT{});

  planning_analysis_pub_ =
      create_publisher<::ros2_interface::msg::PlanningAnalysis>(
          "/planning/PlanningAnalysis", 
          ROS2_QOS_DIAGNOSTIC(), 
          PubAllocT{});

  parking_state_display_pub_ =
      create_publisher<::ros2_interface::msg::ParkingStateDisplay>(
          "/planning/ParkingStateDisplay", 
          ROS2_QOS_NAVIGATION(), 
          PubAllocT{});

  trajectory_array_pub_ =
      create_publisher<::ros2_interface::msg::TrajectoryArray>(
          "/planning/TrajectoryArray", 
          ROS2_QOS_LARGE_DATA(), 
          PubAllocT{});
  ...
  events_pub_ = create_publisher<::ros2_interface::msg::Events>(
      "/planning/Events", 
      ROS2_QOS_DIAGNOSTIC(), 
      PubAllocT{});

  // 订阅方也使用 transient_local 以匹配发布方
  routing_response_sub_ = create_subscription<RoutingResponse>(
      "/routing/RoutingResponse", 
      ROS2_QOS_TRIGGER(),
      [this](const ros2_interface::msg::RoutingResponse::SharedPtr msg) {
        Ros2MessageManager::HandleRoutingResponseMessage(msg);
      },
      SubAllocT{});

  local_routing_response_sub_ = create_subscription<RoutingResponse>(
      "/local_map/LocalRoutingResponse", 
      ROS2_QOS_TRIGGER(),
      [this](const ros2_interface::msg::RoutingResponse::SharedPtr msg) {
        Ros2MessageManager::HandleLocalRoutingResponseMessage(msg);
      },
      SubAllocT{});

  parking_info_sub_ = create_subscription<ParkingInfo>(
      "/routing/ParkingInfo", 
      ROS2_QOS_TRIGGER(),
      [this](const ros2_interface::msg::ParkingInfo::SharedPtr msg) {
        Ros2MessageManager::HandleParkingInfoMessage(msg);
      },
      SubAllocT{});

  stop_info_sub_ = create_subscription<StopInfo>(
      "/routing/StopInfo", 
      ROS2_QOS_TRIGGER(),
      [this](const ros2_interface::msg::StopInfo::SharedPtr msg) {
        Ros2MessageManager::HandleStopInfoMessage(msg);
      },
      SubAllocT{});

  traffic_light_msg_sub_ = create_subscription<TrafficLightMsg>(
      "TrafficLightMsg", 
      QoS(KeepLast(1)).reliable().transient_local(),
      [this](const ros2_interface::msg::TrafficLightMsg::SharedPtr msg) {
        Ros2MessageManager::HandleTrafficLightMsgMessage(msg);
      },
      SubAllocT{});
  ...
  prediction_obstacles_sub_ = create_subscription<PredictionObstacles>(
      "/prediction/PredictionObstacles", 
      ROS2_QOS_LARGE_DATA(),
      [this](const ros2_interface::msg::PredictionObstacles::SharedPtr msg) {
        Ros2MessageManager::HandlePredictionObstaclesMessage(msg);
      },
      SubAllocT{});

  lane_list_sub_ = create_subscription<LaneList>(
      "/routing/LaneList", 
      ROS2_QOS_LARGE_DATA(),
      [this](const ros2_interface::msg::LaneList::SharedPtr msg) {
        Ros2MessageManager::HandleLaneListMessage(msg);
      },
      SubAllocT{});
  ...
  sotif_monitor_result_sub_ = create_subscription<SotifMonitorResult>(
      "/safety/sotif_monitor/SotifMonitorResult", 
      ROS2_QOS_PERIODIC_STATUS(),
      [this](const ros2_interface::msg::SotifMonitorResult::SharedPtr msg) {
        Ros2MessageManager::HandleSotifMonitorResultMessage(msg);
      },
      SubAllocT{});

  obu_cmd_msg_sub_ = create_subscription<ObuCmdMsg>(
  ...
  drivable_region_sub_ = create_subscription<DrivableRegion>(
      "/perception/fusion/traffic_sign_fusion/DrivableRegion", 
      ROS2_QOS_PERCEPTION(),
      [this](const ros2_interface::msg::DrivableRegion::SharedPtr msg) {
        Ros2MessageManager::HandleDrivableRegionMessage(msg);
      },
      SubAllocT{});

  parking_out_info_sub_ = create_subscription<ParkingOutInfo>(
      "/avp/parking_out/ParkingOutInfo", 
      ROS2_QOS_TRIGGER(),
      [this](const ros2_interface::msg::ParkingOutInfo::SharedPtr msg) {
        Ros2MessageManager::HandleParkingOutInfoMessage(msg);
      },
      SubAllocT{});

  guide_info_sub_ = create_subscription<GuideInfo>(
      "/routing/GuideInfo", 
      ROS2_QOS_NAVIGATION(),
      [this](const ros2_interface::msg::GuideInfo::SharedPtr msg) {
        Ros2MessageManager::HandleGuideInfoMessage(msg);
      },
      SubAllocT{});

  traffic_events_sub_ = create_subscription<TrafficEvents>(
      "/routing/TrafficEvents", 
      ROS2_QOS_TRIGGER(),
      [this](const ros2_interface::msg::TrafficEvents::SharedPtr msg) {
        Ros2MessageManager::HandleTrafficEventsMessage(msg);
      },
      SubAllocT{});
...
void Ros2MessageManager<T>::PublishADCTrajectory(
  ...
  for (auto it_trajectory_points : legion_trajectory_points) {
    ...
    ros_trajectory_points.emplace_back(adc_trajectory_trajectory_point);
  }
  adc_trajectory.trajectory_points = ros_trajectory_points;
  ...
  adc_trajectory_pub_->publish(adc_trajectory);
}

template <typename T>
void Ros2MessageManager<T>::PublishPlanningCmd(
    legion::interface::PlanningCmd msg) {
  if (is_init_ == false) return;
  ::ros2_interface::msg::PlanningCmd planning_cmd;
  MESSAGE_HEADER_ROS2_ASSIGN(std_msgs::msg, planning_cmd)
...
void Ros2MessageManager<T>::PublishPlanningAnalysis(
  ...
  for (auto it_evaluator_time : legion_evaluator_time) {
    ...
    ros_evaluator_time.emplace_back(planning_analysis_time_consume);
  }
  planning_analysis.evaluator_time = ros_evaluator_time;
  ...
  std::vector<ros2_interface::msg::Polygon2D> ros_vehicle_preiew_polygon;
  ...
  for (auto it_vehicle_preiew_polygon : legion_vehicle_preiew_polygon) {
    ...
    std::vector<ros2_interface::msg::Point2D> ros_points;
    ...
    for (auto it_points : legion_points) {
      ...
      planning_analysis_planning_parking_debug_polygon_2d_point_2d.y =
          it_points.y();
      ros_points.emplace_back(
          planning_analysis_planning_parking_debug_polygon_2d_point_2d);
    }
    planning_analysis_planning_parking_debug_polygon_2d.points = ros_points;
    ros_vehicle_preiew_polygon.emplace_back(
        planning_analysis_planning_parking_debug_polygon_2d);
  }
  planning_analysis_planning_parking_debug.vehicle_preiew_polygon =
      ros_vehicle_preiew_polygon;
  std::vector<ros2_interface::msg::Polygon2D> ros_obstacles_polygon;
  ...
  for (auto it_obstacles_polygon : legion_obstacles_polygon) {
    ...
    std::vector<ros2_interface::msg::Point2D> ros_points_1;
    ...
    for (auto it_points_1 : legion_points_1) {
      ...
      planning_analysis_planning_parking_debug_polygon_2d_point_2d.y =
          it_points_1.y();
      ros_points_1.emplace_back(
          planning_analysis_planning_parking_debug_polygon_2d_point_2d);
    }
    planning_analysis_planning_parking_debug_polygon_2d.points = ros_points_1;
    ros_obstacles_polygon.emplace_back(
        planning_analysis_planning_parking_debug_polygon_2d);
  }
  planning_analysis_planning_parking_debug.obstacles_polygon =
      ros_obstacles_polygon;
  std::vector<ros2_interface::msg::PathPoint> ros_path_points;
  ...
  for (auto it_path_points : legion_path_points) {
    ...
    ros_path_points.emplace_back(
        planning_analysis_planning_parking_debug_path_point);
  }
  planning_analysis_planning_parking_debug.path_points = ros_path_points;
  ...
  std::vector<ros2_interface::msg::Point2dList> ros_obstacles_vec;
  ...
  for (auto it_obstacles_vec : legion_obstacles_vec) {
    ...
    std::vector<ros2_interface::msg::Point2D> ros_point2d_list;
    ...
    for (auto it_point2d_list : legion_point2d_list) {
      ...
      planning_analysis_planning_parking_debug_point_2d_list_point_2d.y =
          it_point2d_list.y();
      ros_point2d_list.emplace_back(
          planning_analysis_planning_parking_debug_point_2d_list_point_2d);
    }
    planning_analysis_planning_parking_debug_point_2d_list.point2d_list =
        ros_point2d_list;
    ros_obstacles_vec.emplace_back(
        planning_analysis_planning_parking_debug_point_2d_list);
  }
  planning_analysis_planning_parking_debug.obstacles_vec = ros_obstacles_vec;
  std::vector<ros2_interface::msg::TrajectoryPoint> ros_warm_start_traj;
  ...
  for (auto it_warm_start_traj : legion_warm_start_traj) {
    ...
    ros_warm_start_traj.emplace_back(
        planning_analysis_planning_parking_debug_trajectory_point);
  }
  planning_analysis_planning_parking_debug.warm_start_traj =
      ros_warm_start_traj;
  std::vector<ros2_interface::msg::TrajectoryPoint> ros_smoothed_traj_stage1;
  ...
  for (auto it_smoothed_traj_stage1 : legion_smoothed_traj_stage1) {
    ...
    ros_smoothed_traj_stage1.emplace_back(
        planning_analysis_planning_parking_debug_trajectory_point);
  }
  planning_analysis_planning_parking_debug.smoothed_traj_stage1 =
      ros_smoothed_traj_stage1;
  std::vector<ros2_interface::msg::TrajectoryPoint> ros_smoothed_traj_stage2;
  ...
  for (auto it_smoothed_traj_stage2 : legion_smoothed_traj_stage2) {
    ...
    ros_smoothed_traj_stage2.emplace_back(
        planning_analysis_planning_parking_debug_trajectory_point);
  }
  planning_analysis_planning_parking_debug.smoothed_traj_stage2 =
      ros_smoothed_traj_stage2;
  ...
  for (auto it_trajectory_array : legion_trajectory_array) {
    ...
    std::vector<ros2_interface::msg::TrajectoryPoint> ros_trajectory_points_1;
  ...
  std::vector<ros2_interface::msg::TrajectoryPoint> ros_trajectory_points_2;
  ...
  for (auto it_trajectory_points_2 : legion_trajectory_points_2) {
    ...
    ros_trajectory_points_2.emplace_back(
        planning_analysis_planning_parking_debug_optimal_coarse_trajectory_trajectory_point);
  }
  planning_analysis_planning_parking_debug_optimal_coarse_trajectory
      .trajectory_points = ros_trajectory_points_2;
  ...
  std::vector<ros2_interface::msg::TrajectoryPoint> ros_trajectory_points_3;
  ...
  for (auto it_trajectory_points_3 : legion_trajectory_points_3) {
    ...
    ros_trajectory_points_3.emplace_back(
        planning_analysis_planning_parking_debug_optimal_smooth_trajectory_trajectory_point);
  }
  planning_analysis_planning_parking_debug_optimal_smooth_trajectory
      .trajectory_points = ros_trajectory_points_3;
  ...
  planning_analysis_pub_->publish(planning_analysis);
}

template <typename T>
void Ros2MessageManager<T>::PublishParkingStateDisplay(
    legion::interface::ParkingStateDisplay msg) {
  if (is_init_ == false) return;
  ::ros2_interface::msg::ParkingStateDisplay parking_state_display;
  MESSAGE_HEADER_ROS2_ASSIGN(std_msgs::msg, parking_state_display)
  ...
  parking_state_display_pub_->publish(parking_state_display);
}

template <typename T>
void Ros2MessageManager<T>::PublishTrajectoryArray(
    legion::interface::TrajectoryArray msg) {
  if (is_init_ == false) return;
  ::ros2_interface::msg::TrajectoryArray trajectory_array;
  MESSAGE_HEADER_ROS2_ASSIGN(std_msgs::msg, trajectory_array)
  std::vector<ros2_interface::msg::Path> ros_spline_s;
  ...
  for (auto it_spline_s : legion_spline_s) {
    ...
    for (auto it_path_points : legion_path_points) {
      ...
      trajectory_array_path_path_point.y_derivative =
          it_path_points.y_derivative();
      ros_path_points.emplace_back(trajectory_array_path_path_point);
    }
    trajectory_array_path.path_points = ros_path_points;
    ros_spline_s.emplace_back(trajectory_array_path);
  }
  trajectory_array.spline_s = ros_spline_s;
  std::vector<ros2_interface::msg::Path> ros_qp_smooth;
  ...
  for (auto it_qp_smooth : legion_qp_smooth) {
    ...
    std::vector<ros2_interface::msg::PathPoint> ros_path_points_1;
    ...
    for (auto it_path_points_1 : legion_path_points_1) {
      ...
      ros_path_points_1.emplace_back(trajectory_array_path_path_point);
    }
    trajectory_array_path.path_points = ros_path_points_1;
    ros_qp_smooth.emplace_back(trajectory_array_path);
  }
  trajectory_array.qp_smooth = ros_qp_smooth;
  std::vector<ros2_interface::msg::Trajectory> ros_trajectory_list;
  ...
  for (auto it_trajectory_list : legion_trajectory_list) {
    ...
    trajectory_array_trajectory.trajectory_points = ros_trajectory_points;
    ros_trajectory_list.emplace_back(trajectory_array_trajectory);
  }
  trajectory_array.trajectory_list = ros_trajectory_list;

  trajectory_array_pub_->publish(trajectory_array);
}
...
template <typename T>
void Ros2MessageManager<T>::PublishEvents(legion::interface::Events msg) {
  if (is_init_ == false) return;
  ::ros2_interface::msg::Events events;
  MESSAGE_HEADER_ROS2_ASSIGN(std_msgs::msg, events)
...
void Ros2MessageManager<T>::HandleRoutingResponseMessage(
    const ros2_interface::msg::RoutingResponse::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::RoutingResponse> msg =
      std::const_pointer_cast<ros2_interface::msg::RoutingResponse>(
          msg_obj_ptr);

  legion::interface::RoutingResponse routing_response;
  MESSAGE_HEADER_ROS2_PARSER(routing_response)
...
template <typename T>
void Ros2MessageManager<T>::HandleLocalRoutingResponseMessage(
    const ros2_interface::msg::RoutingResponse::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::RoutingResponse> msg =
      std::const_pointer_cast<ros2_interface::msg::RoutingResponse>(
...
template <typename T>
void Ros2MessageManager<T>::HandleParkingInfoMessage(
    const ros2_interface::msg::ParkingInfo::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::ParkingInfo> msg =
      std::const_pointer_cast<ros2_interface::msg::ParkingInfo>(msg_obj_ptr);

  legion::interface::ParkingInfo parking_info;
  MESSAGE_HEADER_ROS2_PARSER(parking_info)
  parking_info.set_parking_space_id(msg->parking_space_id);
  parking_info.set_parking_type((legion::common::ParkingType)msg->parking_type);
  ...
  for (auto it_parking_stoppers : msg->parking_stoppers) {
    ...
    MESSAGE_HEADER_ROS2_PARSER(parking_info_parking_stopper)
...
template <typename T>
void Ros2MessageManager<T>::HandleStopInfoMessage(
    const ros2_interface::msg::StopInfo::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::StopInfo> msg =
      std::const_pointer_cast<ros2_interface::msg::StopInfo>(msg_obj_ptr);

  legion::interface::StopInfo stop_info;
  MESSAGE_HEADER_ROS2_PARSER(stop_info)
...
template <typename T>
void Ros2MessageManager<T>::HandleTrafficLightMsgMessage(
    const ros2_interface::msg::TrafficLightMsg::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::TrafficLightMsg> msg =
      std::const_pointer_cast<ros2_interface::msg::TrafficLightMsg>(
          msg_obj_ptr);

  legion::interface::TrafficLightMsg traffic_light_msg;
  MESSAGE_HEADER_ROS2_PARSER(traffic_light_msg)
  ...
  traffic_light_msg_traffic_light_debug_cropbox.set_color(
      (legion::common::TrafficLightColor)
...
void Ros2MessageManager<T>::HandlePredictionObstaclesMessage(
    const ros2_interface::msg::PredictionObstacles::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::PredictionObstacles> msg =
      std::const_pointer_cast<ros2_interface::msg::PredictionObstacles>(
          msg_obj_ptr);

  legion::interface::PredictionObstacles prediction_obstacles;
  MESSAGE_HEADER_ROS2_PARSER(prediction_obstacles)
  std::vector<legion::interface::PredictionObstacle>
  ...
  for (auto it_prediction_obstacles_vector : msg->prediction_obstacles) {
    ...
    prediction_obstacles_prediction_obstacle_intent.set_type(
        (legion::interface::ObstacleIntent::Type)
...
template <typename T>
void Ros2MessageManager<T>::HandleLaneListMessage(
    const ros2_interface::msg::LaneList::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::LaneList> msg =
      std::const_pointer_cast<ros2_interface::msg::LaneList>(msg_obj_ptr);

  legion::interface::LaneList lane_list;
  MESSAGE_HEADER_ROS2_PARSER(lane_list)
  ...
  lane_list_hpp.set_planning_source(
      (legion::interface::HolisticPathPrediction::PlanningSource)
...
template <typename T>
void Ros2MessageManager<T>::HandleSotifMonitorResultMessage(
    const ros2_interface::msg::SotifMonitorResult::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::SotifMonitorResult> msg =
      std::const_pointer_cast<ros2_interface::msg::SotifMonitorResult>(
          msg_obj_ptr);

  legion::interface::SotifMonitorResult sotif_monitor_result;
  MESSAGE_HEADER_ROS2_PARSER(sotif_monitor_result)
...
template <typename T>
void Ros2MessageManager<T>::HandleDrivableRegionMessage(
    const ros2_interface::msg::DrivableRegion::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::DrivableRegion> msg =
      std::const_pointer_cast<ros2_interface::msg::DrivableRegion>(msg_obj_ptr);

  legion::interface::DrivableRegion drivable_region;
  MESSAGE_HEADER_ROS2_PARSER(drivable_region)
...
template <typename T>
void Ros2MessageManager<T>::HandleParkingOutInfoMessage(
    const ros2_interface::msg::ParkingOutInfo::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::ParkingOutInfo> msg =
      std::const_pointer_cast<ros2_interface::msg::ParkingOutInfo>(msg_obj_ptr);

  legion::interface::ParkingOutInfo parking_out_info;
  MESSAGE_HEADER_ROS2_PARSER(parking_out_info)
...
template <typename T>
void Ros2MessageManager<T>::HandleGuideInfoMessage(
    const ros2_interface::msg::GuideInfo::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::GuideInfo> msg =
      std::const_pointer_cast<ros2_interface::msg::GuideInfo>(msg_obj_ptr);

  legion::interface::GuideInfo guide_info;
  MESSAGE_HEADER_ROS2_PARSER(guide_info)
...
//   guide_info_current_road.set_curvature_size(msg->current_road.curvature_size);
  std::vector<legion::interface::CurvatureInfo> curvature_vec;
  for (auto it_curvature_vec : msg->current_road.curvature) {
    legion::interface::CurvatureInfo guide_info_current_road_curvature_info;
    guide_info_current_road_curvature_info.set_offset(it_curvature_vec.offset);
    guide_info_current_road_curvature_info.set_value(it_curvature_vec.value);
    curvature_vec.emplace_back(guide_info_current_road_curvature_info);
  }
  guide_info_current_road.set_curvature(&curvature_vec);
...
//   guide_info_next_road.set_curvature_size(msg->next_road.curvature_size);
  std::vector<legion::interface::CurvatureInfo> curvature_1_vec;
  for (auto it_curvature_1_vec : msg->next_road.curvature) {
    legion::interface::CurvatureInfo guide_info_next_road_curvature_info;
    guide_info_next_road_curvature_info.set_offset(it_curvature_1_vec.offset);
    guide_info_next_road_curvature_info.set_value(it_curvature_1_vec.value);
    curvature_1_vec.emplace_back(guide_info_next_road_curvature_info);
  }
  guide_info_next_road.set_curvature(&curvature_1_vec);
...
template <typename T>
void Ros2MessageManager<T>::HandleTrafficEventsMessage(
    const ros2_interface::msg::TrafficEvents::SharedPtr msg_obj_ptr) {
  if (is_init_ == false) return;
  std::shared_ptr<ros2_interface::msg::TrafficEvents> msg =
      std::const_pointer_cast<ros2_interface::msg::TrafficEvents>(msg_obj_ptr);

  legion::interface::TrafficEvents traffic_events;
  MESSAGE_HEADER_ROS2_PARSER(traffic_events)
