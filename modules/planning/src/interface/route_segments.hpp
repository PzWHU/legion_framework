...
public:
  ...
  inline void
  set_lane_points(std::vector<legion::planning::LanePointInner>* lane_points) {
  ...
  inline void set_lane_points(
      const std::vector<legion::planning::LanePointInner>& lane_points) {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    lane_points_ = lane_points;
  }

  inline void set_lane_points(const uint32_t index,
                              legion::planning::LanePointInner& lane_points) {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    lane_points_[index] = lane_points;
  }

  inline void add_lane_points(const legion::planning::LanePointInner& lane_points) {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    lane_points_.emplace_back(lane_points);
  }

  inline const legion::planning::LanePointInner& lane_points(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    return lane_points_[index];
  }

  inline std::vector<legion::planning::LanePointInner>* mutable_lane_points() {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    return &lane_points_;
  }

  inline void
  lane_points(std::vector<legion::planning::LanePointInner>& lane_points) const {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    lane_points.assign(lane_points_.begin(), lane_points_.end());
  }

  inline const std::vector<legion::planning::LanePointInner>& lane_points() const {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    return lane_points_;
  }
 ...
 protected:
  ...
  //道路参考线（z）
  std::vector<legion::planning::LanePointInner> lane_points_;
  std::string id_;
