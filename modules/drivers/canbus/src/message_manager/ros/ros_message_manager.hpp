...
void RosMessageManager<T>::Init(T* t) {
  ...
  wheel_info_pub_ = nh_.advertise<::ros_interface::WheelInfo>(
      "/drivers/canbus/WheelInfo", 10);

  chassis_pub_ =
      nh_.advertise<::ros_interface::Chassis>("/drivers/canbus/Chassis", 10);

  vehicle_pub_ =
      nh_.advertise<::ros_interface::Vehicle>("/drivers/canbus/Vehicle", 10);

  faults_pub_ =
      nh_.advertise<::ros_interface::Faults>("/drivers/canbus/Faults", 10);

  control_command_sub_ =
      nh_.subscribe("/control/ControlCommand", 30,
                    &RosMessageManager::HandleControlCommandMessage, this);

  vehicle_task_sub_ =
      nh_.subscribe("/legion_bridge/VehicleTask", 30,
                    &RosMessageManager::HandleVehicleTaskMessage, this);

  teleop_control_command_sub_ = nh_.subscribe(
      "/legion_bridge/TeleopControlCommand", 30,
      &RosMessageManager::HandleTeleopControlCommandMessage, this);
...
template <typename T>
void RosMessageManager<T>::PublishWheelInfo(legion::interface::WheelInfo msg) {
  if (is_init_ == false) return;
  ::ros_interface::WheelInfo wheel_info;
  MESSAGE_HEADER_ASSIGN(std_msgs, wheel_info)
...
template <typename T>
void RosMessageManager<T>::PublishChassis(legion::interface::Chassis msg) {
  if (is_init_ == false) return;
  ::ros_interface::Chassis chassis;
  MESSAGE_HEADER_ASSIGN(std_msgs, chassis)
...
template <typename T>
void RosMessageManager<T>::PublishVehicle(legion::interface::Vehicle msg) {
  if (is_init_ == false) return;
  ::ros_interface::Vehicle vehicle;
  MESSAGE_HEADER_ASSIGN(std_msgs, vehicle)
...
template <typename T>
void RosMessageManager<T>::HandleControlCommandMessage(
...
template <typename T>
void RosMessageManager<T>::HandleVehicleTaskMessage(
    const ros_interface::VehicleTask& msg_obj) {
  if (is_init_ == false) return;
  const ros_interface::VehicleTask* msg_obj_ptr = &msg_obj;
  ros_interface::VehicleTask* msg =
      const_cast<ros_interface::VehicleTask*>(msg_obj_ptr);

  legion::interface::VehicleTask vehicle_task;
  MESSAGE_HEADER_PARSER(vehicle_task)
...
template <typename T>
void RosMessageManager<T>::HandleTeleopControlCommandMessage(
    const ros_interface::ControlCommand& msg_obj) {
  if (is_init_ == false) return;
  const ros_interface::ControlCommand* msg_obj_ptr = &msg_obj;
  ros_interface::ControlCommand* msg =
