...
void LcmMessageManager<T>::Init(T* t) {
  ...
  lcm_->subscribe("/control/ControlCommand",
                  &LcmMessageManager::HandleControlCommandMessage, this);

  lcm_->subscribe("/legion_bridge/VehicleTask",
                  &LcmMessageManager::HandleVehicleTaskMessage, this);

  lcm_->subscribe("/legion_bridge/TeleopControlCommand",
                  &LcmMessageManager::HandleTeleopControlCommandMessage, this);
...
template <typename T>
void LcmMessageManager<T>::PublishWheelInfo(legion::interface::WheelInfo msg) {
  if (is_init_ == false) return;
  lcm_interface::WheelInfo wheel_info;
  MESSAGE_HEADER_ASSIGN(lcm_interface, wheel_info)
  ...
  lcm_->publish("/drivers/canbus/WheelInfo", &wheel_info);
}

template <typename T>
void LcmMessageManager<T>::PublishChassis(legion::interface::Chassis msg) {
  if (is_init_ == false) return;
  lcm_interface::Chassis chassis;
  MESSAGE_HEADER_ASSIGN(lcm_interface, chassis)
  ...
  chassis.chassis_error_code_size = chassis_error_code_vec.size();
  chassis.chassis_error_code = chassis_error_code_vec;

  lcm_->publish("/drivers/canbus/Chassis", &chassis);
}

template <typename T>
void LcmMessageManager<T>::PublishVehicle(legion::interface::Vehicle msg) {
  if (is_init_ == false) return;
  lcm_interface::Vehicle vehicle;
  MESSAGE_HEADER_ASSIGN(lcm_interface, vehicle)
  ...
  vehicle.seat_belt_size = seat_belt_vec.size();
  ...
  vehicle.battery_temperatures_size = battery_temperatures.size();
  ...
  vehicle.door_status_size = door_status_vec.size();
  ...
  lcm_->publish("/drivers/canbus/Vehicle", &vehicle);
}
...
void LcmMessageManager<T>::PublishFaults(legion::interface::Faults msg) {
  ...
  lcm_->publish("/drivers/canbus/Faults", &faults);
}

template <typename T>
void LcmMessageManager<T>::HandleControlCommandMessage(
...
template <typename T>
void LcmMessageManager<T>::HandleVehicleTaskMessage(
    const lcm::ReceiveBuffer* rbuf, const std::string& chan,
...
template <typename T>
void LcmMessageManager<T>::HandleTeleopControlCommandMessage(
    const lcm::ReceiveBuffer* rbuf, const std::string& chan,
    const lcm_interface::ControlCommand* msg) {
  if (is_init_ == false) return;
