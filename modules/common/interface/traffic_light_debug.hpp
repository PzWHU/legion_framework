...
class TrafficLightDebug {
 public:
  ...
  inline void set_cropbox(const legion::interface::TrafficLightBox& cropbox) {
    cropbox_ = cropbox;
    cropbox_ptr_ = &cropbox_;
  }

  inline const legion::interface::TrafficLightBox& cropbox() const {
    return cropbox_;
  }

  inline legion::interface::TrafficLightBox* mutable_cropbox() {
    return &cropbox_;
  }

  inline bool has_cropbox() { return (cropbox_ptr_ != nullptr); }

  inline void set_box(std::vector<legion::interface::TrafficLightBox>* box) {
    std::lock_guard<std::mutex> lock(*box_mutex_);
    box_.assign(box->begin(), box->end());
  }

  inline void set_box(
      const std::vector<legion::interface::TrafficLightBox>& box) {
    std::lock_guard<std::mutex> lock(*box_mutex_);
    box_ = box;
  }

  inline void set_box(const uint32_t index,
                      legion::interface::TrafficLightBox& box) {
    std::lock_guard<std::mutex> lock(*box_mutex_);
    box_[index] = box;
  }

  inline void add_box(const legion::interface::TrafficLightBox& box) {
    std::lock_guard<std::mutex> lock(*box_mutex_);
    box_.emplace_back(box);
  }

  inline const legion::interface::TrafficLightBox& box(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*box_mutex_);
    return box_[index];
  }

  inline std::vector<legion::interface::TrafficLightBox>* mutable_box() {
    std::lock_guard<std::mutex> lock(*box_mutex_);
    return &box_;
  }

  inline void box(std::vector<legion::interface::TrafficLightBox>& box) const {
    std::lock_guard<std::mutex> lock(*box_mutex_);
    box.assign(box_.begin(), box_.end());
  }

  inline const std::vector<legion::interface::TrafficLightBox>& box() const {
    std::lock_guard<std::mutex> lock(*box_mutex_);
    return box_;
  }
  ...
  inline void set_crop_roi(
      const std::vector<legion::interface::TrafficLightBox>& crop_roi) {
    std::lock_guard<std::mutex> lock(*crop_roi_mutex_);
    crop_roi_ = crop_roi;
  }

  inline void set_crop_roi(const uint32_t index,
                           legion::interface::TrafficLightBox& crop_roi) {
    std::lock_guard<std::mutex> lock(*crop_roi_mutex_);
    crop_roi_[index] = crop_roi;
  }

  inline void add_crop_roi(const legion::interface::TrafficLightBox& crop_roi) {
    std::lock_guard<std::mutex> lock(*crop_roi_mutex_);
    crop_roi_.emplace_back(crop_roi);
  }

  inline const legion::interface::TrafficLightBox& crop_roi(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*crop_roi_mutex_);
    return crop_roi_[index];
  }

  inline std::vector<legion::interface::TrafficLightBox>* mutable_crop_roi() {
    std::lock_guard<std::mutex> lock(*crop_roi_mutex_);
    return &crop_roi_;
  }

  inline void crop_roi(
      std::vector<legion::interface::TrafficLightBox>& crop_roi) const {
    std::lock_guard<std::mutex> lock(*crop_roi_mutex_);
    crop_roi.assign(crop_roi_.begin(), crop_roi_.end());
  }

  inline const std::vector<legion::interface::TrafficLightBox>& crop_roi()
      const {
    std::lock_guard<std::mutex> lock(*crop_roi_mutex_);
    return crop_roi_;
  }
  ...
  inline void set_projected_roi(
      const std::vector<legion::interface::TrafficLightBox>& projected_roi) {
    std::lock_guard<std::mutex> lock(*projected_roi_mutex_);
    projected_roi_ = projected_roi;
  }

  inline void set_projected_roi(
      const uint32_t index, legion::interface::TrafficLightBox& projected_roi) {
    std::lock_guard<std::mutex> lock(*projected_roi_mutex_);
    projected_roi_[index] = projected_roi;
  }

  inline void add_projected_roi(
      const legion::interface::TrafficLightBox& projected_roi) {
    std::lock_guard<std::mutex> lock(*projected_roi_mutex_);
    projected_roi_.emplace_back(projected_roi);
  }

  inline const legion::interface::TrafficLightBox& projected_roi(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*projected_roi_mutex_);
    return projected_roi_[index];
  }

  inline std::vector<legion::interface::TrafficLightBox>*
  ...
  inline void projected_roi(
      std::vector<legion::interface::TrafficLightBox>& projected_roi) const {
    std::lock_guard<std::mutex> lock(*projected_roi_mutex_);
    projected_roi.assign(projected_roi_.begin(), projected_roi_.end());
  }

  inline const std::vector<legion::interface::TrafficLightBox>& projected_roi()
      const {
    std::lock_guard<std::mutex> lock(*projected_roi_mutex_);
    return projected_roi_;
  }
  ...
  inline void set_rectified_roi(
      const std::vector<legion::interface::TrafficLightBox>& rectified_roi) {
    std::lock_guard<std::mutex> lock(*rectified_roi_mutex_);
    rectified_roi_ = rectified_roi;
  }

  inline void set_rectified_roi(
      const uint32_t index, legion::interface::TrafficLightBox& rectified_roi) {
    std::lock_guard<std::mutex> lock(*rectified_roi_mutex_);
    rectified_roi_[index] = rectified_roi;
  }

  inline void add_rectified_roi(
      const legion::interface::TrafficLightBox& rectified_roi) {
    std::lock_guard<std::mutex> lock(*rectified_roi_mutex_);
    rectified_roi_.emplace_back(rectified_roi);
  }

  inline const legion::interface::TrafficLightBox& rectified_roi(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*rectified_roi_mutex_);
  ...
  inline void rectified_roi(
      std::vector<legion::interface::TrafficLightBox>& rectified_roi) const {
    std::lock_guard<std::mutex> lock(*rectified_roi_mutex_);
    rectified_roi.assign(rectified_roi_.begin(), rectified_roi_.end());
  }

  inline const std::vector<legion::interface::TrafficLightBox>& rectified_roi()
      const {
    std::lock_guard<std::mutex> lock(*rectified_roi_mutex_);
    return rectified_roi_;
  }
  ...
  inline void set_debug_roi(
      const std::vector<legion::interface::TrafficLightBox>& debug_roi) {
    std::lock_guard<std::mutex> lock(*debug_roi_mutex_);
    debug_roi_ = debug_roi;
  }

  inline void set_debug_roi(const uint32_t index,
                            legion::interface::TrafficLightBox& debug_roi) {
    std::lock_guard<std::mutex> lock(*debug_roi_mutex_);
    debug_roi_[index] = debug_roi;
  }

  inline void add_debug_roi(
      const legion::interface::TrafficLightBox& debug_roi) {
    std::lock_guard<std::mutex> lock(*debug_roi_mutex_);
    debug_roi_.emplace_back(debug_roi);
  }

  inline const legion::interface::TrafficLightBox& debug_roi(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*debug_roi_mutex_);
    return debug_roi_[index];
  }

  inline std::vector<legion::interface::TrafficLightBox>* mutable_debug_roi() {
    std::lock_guard<std::mutex> lock(*debug_roi_mutex_);
    return &debug_roi_;
  }

  inline void debug_roi(
      std::vector<legion::interface::TrafficLightBox>& debug_roi) const {
    std::lock_guard<std::mutex> lock(*debug_roi_mutex_);
    debug_roi.assign(debug_roi_.begin(), debug_roi_.end());
  }

  inline const std::vector<legion::interface::TrafficLightBox>& debug_roi()
      const {
    std::lock_guard<std::mutex> lock(*debug_roi_mutex_);
    return debug_roi_;
  }
 ...
 protected:
  ...
  legion::interface::TrafficLightBox cropbox_;
  legion::interface::TrafficLightBox* cropbox_ptr_ = nullptr;
  std::vector<legion::interface::TrafficLightBox> box_;
  int32_t signal_num_;
  ...
  std::vector<legion::interface::TrafficLightBox> crop_roi_;
  std::vector<legion::interface::TrafficLightBox> projected_roi_;
  std::vector<legion::interface::TrafficLightBox> rectified_roi_;
  std::vector<legion::interface::TrafficLightBox> debug_roi_;
};
}  // namespace interface
}  // namespace legion
