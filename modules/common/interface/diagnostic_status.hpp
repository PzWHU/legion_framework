...
class DiagnosticStatus {
 public:
  DiagnosticStatus() {
    ...
    id_ = 0;
    level_ = legion::interface::DiagnosticStatus::Level::OK;
    name_.clear();
    message_.clear();
    hardware_id_.clear();
    clear_values();
  ...
  inline void set_level(
      const legion::interface::DiagnosticStatus::Level& level) {
    level_ = level;
    level_ptr_ = &level_;
  }

  inline const legion::interface::DiagnosticStatus::Level& level() const {
    return level_;
  }

  inline legion::interface::DiagnosticStatus::Level* mutable_level() {
    return &level_;
  }
  ...
  inline void set_values(std::vector<legion::interface::KeyValues>* values) {
    std::lock_guard<std::mutex> lock(*values_mutex_);
    values_.assign(values->begin(), values->end());
  }

  inline void set_values(
      const std::vector<legion::interface::KeyValues>& values) {
    std::lock_guard<std::mutex> lock(*values_mutex_);
    values_ = values;
  }

  inline void set_values(const uint32_t index,
                         legion::interface::KeyValues& values) {
    std::lock_guard<std::mutex> lock(*values_mutex_);
    values_[index] = values;
  }

  inline void add_values(const legion::interface::KeyValues& values) {
    std::lock_guard<std::mutex> lock(*values_mutex_);
    values_.emplace_back(values);
  }

  inline const legion::interface::KeyValues& values(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*values_mutex_);
    return values_[index];
  }

  inline std::vector<legion::interface::KeyValues>* mutable_values() {
    std::lock_guard<std::mutex> lock(*values_mutex_);
    return &values_;
  }

  inline void values(std::vector<legion::interface::KeyValues>& values) const {
    std::lock_guard<std::mutex> lock(*values_mutex_);
    values.assign(values_.begin(), values_.end());
  }

  inline const std::vector<legion::interface::KeyValues>& values() const {
    std::lock_guard<std::mutex> lock(*values_mutex_);
    return values_;
  }
 ...
 protected:
  ...
  //诊断状态
  legion::interface::DiagnosticStatus::Level level_;
  legion::interface::DiagnosticStatus::Level* level_ptr_ = nullptr;
  //测试/组件报告的描述
  std::string name_;
  ...
  //与状态关联的key-value组
  std::vector<legion::interface::KeyValues> values_;
};
}  // namespace interface
}  // namespace legion
