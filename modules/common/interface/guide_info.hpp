...
class GuideInfo {
 public:
  ...
  inline void set_current_road(
      const legion::interface::GuideRoad& current_road) {
    current_road_ = current_road;
    current_road_ptr_ = &current_road_;
  }

  inline const legion::interface::GuideRoad& current_road() const {
    return current_road_;
  }

  inline legion::interface::GuideRoad* mutable_current_road() {
    return &current_road_;
  }

  inline bool has_current_road() { return (current_road_ptr_ != nullptr); }

  inline void set_next_road(const legion::interface::GuideRoad& next_road) {
    next_road_ = next_road;
    next_road_ptr_ = &next_road_;
  }

  inline const legion::interface::GuideRoad& next_road() const {
    return next_road_;
  }

  inline legion::interface::GuideRoad* mutable_next_road() {
    return &next_road_;
  }
 ...
 protected:
  ...
  //当前道路的引导信息
  legion::interface::GuideRoad current_road_;
  legion::interface::GuideRoad* current_road_ptr_ = nullptr;
  //下一条规划道路的引导信息
  legion::interface::GuideRoad next_road_;
  legion::interface::GuideRoad* next_road_ptr_ = nullptr;
  //环岛状态: 0：未知或非环岛状态
  ...
  //当前行车推荐状态引导：(跨路口)
  //0：1km内（1km配置文件可配置）无需要左转、右转路口
  //1：需要左转路口（1km配置文件可配置） 2：需要右转路口  3：车道级规划失败
