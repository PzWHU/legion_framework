...
class FreeSpace {
 public:
  ...
  inline void set_freespace_region(
      std::vector<legion::interface::Polygon2D>* freespace_region) {
  ...
  inline void set_freespace_region(
      const std::vector<legion::interface::Polygon2D>& freespace_region) {
    std::lock_guard<std::mutex> lock(*freespace_region_mutex_);
    freespace_region_ = freespace_region;
  }

  inline void set_freespace_region(
      const uint32_t index, legion::interface::Polygon2D& freespace_region) {
    std::lock_guard<std::mutex> lock(*freespace_region_mutex_);
    freespace_region_[index] = freespace_region;
  }

  inline void add_freespace_region(
      const legion::interface::Polygon2D& freespace_region) {
    std::lock_guard<std::mutex> lock(*freespace_region_mutex_);
    freespace_region_.emplace_back(freespace_region);
  }

  inline const legion::interface::Polygon2D& freespace_region(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*freespace_region_mutex_);
    return freespace_region_[index];
  }

  inline std::vector<legion::interface::Polygon2D>* mutable_freespace_region() {
    std::lock_guard<std::mutex> lock(*freespace_region_mutex_);
    return &freespace_region_;
  }

  inline void freespace_region(
      std::vector<legion::interface::Polygon2D>& freespace_region) const {
    std::lock_guard<std::mutex> lock(*freespace_region_mutex_);
    freespace_region.assign(freespace_region_.begin(), freespace_region_.end());
  }

  inline const std::vector<legion::interface::Polygon2D>& freespace_region()
      const {
    std::lock_guard<std::mutex> lock(*freespace_region_mutex_);
    return freespace_region_;
  }
 ...
 protected:
  ...
  // freespace轮廓图像坐标
  std::vector<legion::interface::Polygon2D> freespace_region_;
  //错误码（default = OK）
