...
class UssObstacleList {
 public:
  ...
  inline void set_uss_obstacles(
      std::vector<legion::interface::UssObstacle>* uss_obstacles) {
    std::lock_guard<std::mutex> lock(*uss_obstacles_mutex_);
    uss_obstacles_.assign(uss_obstacles->begin(), uss_obstacles->end());
  }

  inline void set_uss_obstacles(
      const std::vector<legion::interface::UssObstacle>& uss_obstacles) {
    std::lock_guard<std::mutex> lock(*uss_obstacles_mutex_);
    uss_obstacles_ = uss_obstacles;
  }

  inline void set_uss_obstacles(const uint32_t index,
                                legion::interface::UssObstacle& uss_obstacles) {
    std::lock_guard<std::mutex> lock(*uss_obstacles_mutex_);
    uss_obstacles_[index] = uss_obstacles;
  }

  inline void add_uss_obstacles(
      const legion::interface::UssObstacle& uss_obstacles) {
    std::lock_guard<std::mutex> lock(*uss_obstacles_mutex_);
    uss_obstacles_.emplace_back(uss_obstacles);
  }

  inline const legion::interface::UssObstacle& uss_obstacles(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*uss_obstacles_mutex_);
    return uss_obstacles_[index];
  }

  inline std::vector<legion::interface::UssObstacle>* mutable_uss_obstacles() {
    std::lock_guard<std::mutex> lock(*uss_obstacles_mutex_);
    return &uss_obstacles_;
  }

  inline void uss_obstacles(
      std::vector<legion::interface::UssObstacle>& uss_obstacles) const {
    std::lock_guard<std::mutex> lock(*uss_obstacles_mutex_);
    uss_obstacles.assign(uss_obstacles_.begin(), uss_obstacles_.end());
  }

  inline const std::vector<legion::interface::UssObstacle>& uss_obstacles()
      const {
    std::lock_guard<std::mutex> lock(*uss_obstacles_mutex_);
    return uss_obstacles_;
  }
 ...
 protected:
  ...
  //检测出的超声波障碍物数组
  std::vector<legion::interface::UssObstacle> uss_obstacles_;
