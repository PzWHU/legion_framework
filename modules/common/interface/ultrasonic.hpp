...
class Ultrasonic {
 public:
  ...
  inline void set_ranges(
      std::vector<legion::interface::UltrasonicObstacle>* ranges) {
    std::lock_guard<std::mutex> lock(*ranges_mutex_);
    ranges_.assign(ranges->begin(), ranges->end());
  }

  inline void set_ranges(
      const std::vector<legion::interface::UltrasonicObstacle>& ranges) {
    std::lock_guard<std::mutex> lock(*ranges_mutex_);
    ranges_ = ranges;
  }

  inline void set_ranges(const uint32_t index,
                         legion::interface::UltrasonicObstacle& ranges) {
    std::lock_guard<std::mutex> lock(*ranges_mutex_);
    ranges_[index] = ranges;
  }

  inline void add_ranges(const legion::interface::UltrasonicObstacle& ranges) {
    std::lock_guard<std::mutex> lock(*ranges_mutex_);
    ranges_.emplace_back(ranges);
  }

  inline const legion::interface::UltrasonicObstacle& ranges(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*ranges_mutex_);
    return ranges_[index];
  }

  inline std::vector<legion::interface::UltrasonicObstacle>* mutable_ranges() {
    std::lock_guard<std::mutex> lock(*ranges_mutex_);
    return &ranges_;
  }

  inline void ranges(
      std::vector<legion::interface::UltrasonicObstacle>& ranges) const {
    std::lock_guard<std::mutex> lock(*ranges_mutex_);
    ranges.assign(ranges_.begin(), ranges_.end());
  }

  inline const std::vector<legion::interface::UltrasonicObstacle>& ranges()
      const {
    std::lock_guard<std::mutex> lock(*ranges_mutex_);
    return ranges_;
  }
 ...
 protected:
  ...
  //超声波障碍物数组
  std::vector<legion::interface::UltrasonicObstacle> ranges_;
};
}  // namespace interface
}  // namespace legion
