...
class RoutingRequest {
 public:
  ...
  inline void set_key_point_list(
      const std::vector<legion::interface::KeyPoint>& key_point_list) {
    std::lock_guard<std::mutex> lock(*key_point_list_mutex_);
    key_point_list_ = key_point_list;
  }

  inline void set_key_point_list(const uint32_t index,
                                 legion::interface::KeyPoint& key_point_list) {
    std::lock_guard<std::mutex> lock(*key_point_list_mutex_);
    key_point_list_[index] = key_point_list;
  }

  inline void add_key_point_list(
      const legion::interface::KeyPoint& key_point_list) {
    std::lock_guard<std::mutex> lock(*key_point_list_mutex_);
    key_point_list_.emplace_back(key_point_list);
  }

  inline const legion::interface::KeyPoint& key_point_list(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*key_point_list_mutex_);
    return key_point_list_[index];
  }

  inline std::vector<legion::interface::KeyPoint>* mutable_key_point_list() {
    std::lock_guard<std::mutex> lock(*key_point_list_mutex_);
    return &key_point_list_;
  }

  inline void key_point_list(
      std::vector<legion::interface::KeyPoint>& key_point_list) const {
    std::lock_guard<std::mutex> lock(*key_point_list_mutex_);
    key_point_list.assign(key_point_list_.begin(), key_point_list_.end());
  }

  inline const std::vector<legion::interface::KeyPoint>& key_point_list()
      const {
    std::lock_guard<std::mutex> lock(*key_point_list_mutex_);
    return key_point_list_;
  }
 ...
 protected:
  ...
  std::vector<legion::interface::KeyPoint> key_point_list_;
};
}  // namespace interface
}  // namespace legion
