...
class ObstacleList {
 public:
  ...
  inline void set_obstacle(std::vector<legion::interface::Obstacle>* obstacle) {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    obstacle_.assign(obstacle->begin(), obstacle->end());
  }

  inline void set_obstacle(
      const std::vector<legion::interface::Obstacle>& obstacle) {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    obstacle_ = obstacle;
  }
  ...
  inline void add_obstacle(const legion::interface::Obstacle& obstacle) {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    obstacle_.emplace_back(obstacle);
  }

  inline const legion::interface::Obstacle& obstacle(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    return obstacle_[index];
  }

  inline std::vector<legion::interface::Obstacle>* mutable_obstacle() {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    return &obstacle_;
  }

  inline void obstacle(
      std::vector<legion::interface::Obstacle>& obstacle) const {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    obstacle.assign(obstacle_.begin(), obstacle_.end());
  }

  inline const std::vector<legion::interface::Obstacle>& obstacle() const {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    return obstacle_;
  }
  ...
  void operator=(const ObstacleList& obstacle_list) { CopyFrom(obstacle_list); }

  void CopyFrom(const ObstacleList& obstacle_list) {
    header_ = obstacle_list.header();
    sensor_id_ = obstacle_list.sensor_id();
    obstacle_ = obstacle_list.obstacle();
    error_code_ = obstacle_list.error_code();
    is_valid_ = obstacle_list.is_valid();
    change_origin_flag_ = obstacle_list.change_origin_flag();
  }
 ...
 protected:
  ...
  //检测出的障碍物数组
  std::vector<legion::interface::Obstacle> obstacle_;
  //错误码（default = OK）
