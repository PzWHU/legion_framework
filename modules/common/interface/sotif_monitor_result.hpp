...
class SotifMonitorResult {
 public:
  ...
  inline void set_region_value(
      const std::vector<legion::interface::Region>& region_value) {
    std::lock_guard<std::mutex> lock(*region_value_mutex_);
    region_value_ = region_value;
  }

  inline void set_region_value(const uint32_t index,
                               legion::interface::Region& region_value) {
    std::lock_guard<std::mutex> lock(*region_value_mutex_);
    region_value_[index] = region_value;
  }

  inline void add_region_value(const legion::interface::Region& region_value) {
    std::lock_guard<std::mutex> lock(*region_value_mutex_);
    region_value_.emplace_back(region_value);
  }

  inline const legion::interface::Region& region_value(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*region_value_mutex_);
    return region_value_[index];
  }

  inline std::vector<legion::interface::Region>* mutable_region_value() {
    std::lock_guard<std::mutex> lock(*region_value_mutex_);
    return &region_value_;
  }

  inline void region_value(
      std::vector<legion::interface::Region>& region_value) const {
    std::lock_guard<std::mutex> lock(*region_value_mutex_);
    region_value.assign(region_value_.begin(), region_value_.end());
  }

  inline const std::vector<legion::interface::Region>& region_value() const {
    std::lock_guard<std::mutex> lock(*region_value_mutex_);
    return region_value_;
  }
  ...
  inline void set_grid_map(std::vector<legion::interface::Grid>* grid_map) {
    std::lock_guard<std::mutex> lock(*grid_map_mutex_);
    grid_map_.assign(grid_map->begin(), grid_map->end());
  }

  inline void set_grid_map(
      const std::vector<legion::interface::Grid>& grid_map) {
    std::lock_guard<std::mutex> lock(*grid_map_mutex_);
    grid_map_ = grid_map;
  }

  inline void set_grid_map(const uint32_t index,
                           legion::interface::Grid& grid_map) {
    std::lock_guard<std::mutex> lock(*grid_map_mutex_);
    grid_map_[index] = grid_map;
  }

  inline void add_grid_map(const legion::interface::Grid& grid_map) {
    std::lock_guard<std::mutex> lock(*grid_map_mutex_);
    grid_map_.emplace_back(grid_map);
  }

  inline const legion::interface::Grid& grid_map(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*grid_map_mutex_);
    return grid_map_[index];
  }

  inline std::vector<legion::interface::Grid>* mutable_grid_map() {
    std::lock_guard<std::mutex> lock(*grid_map_mutex_);
    return &grid_map_;
  }

  inline void grid_map(std::vector<legion::interface::Grid>& grid_map) const {
    std::lock_guard<std::mutex> lock(*grid_map_mutex_);
    grid_map.assign(grid_map_.begin(), grid_map_.end());
  }

  inline const std::vector<legion::interface::Grid>& grid_map() const {
    std::lock_guard<std::mutex> lock(*grid_map_mutex_);
    return grid_map_;
  }
 ...
 protected:
  ...
  //区域态势
  std::vector<legion::interface::Region> region_value_;
  //栅格地图
  std::vector<legion::interface::Grid> grid_map_;
};
}  // namespace interface
}  // namespace legion
