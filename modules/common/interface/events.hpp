...
class Events {
 public:
  ...
  inline void set_events(std::vector<legion::interface::Event>* events) {
    std::lock_guard<std::mutex> lock(*events_mutex_);
    events_.assign(events->begin(), events->end());
  }

  inline void set_events(const std::vector<legion::interface::Event>& events) {
    std::lock_guard<std::mutex> lock(*events_mutex_);
    events_ = events;
  }

  inline void set_events(const uint32_t index,
                         legion::interface::Event& events) {
    std::lock_guard<std::mutex> lock(*events_mutex_);
    events_[index] = events;
  }

  inline void add_events(const legion::interface::Event& events) {
    std::lock_guard<std::mutex> lock(*events_mutex_);
    events_.emplace_back(events);
  }

  inline const legion::interface::Event& events(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*events_mutex_);
    return events_[index];
  }

  inline std::vector<legion::interface::Event>* mutable_events() {
    std::lock_guard<std::mutex> lock(*events_mutex_);
    return &events_;
  }

  inline void events(std::vector<legion::interface::Event>& events) const {
    std::lock_guard<std::mutex> lock(*events_mutex_);
    events.assign(events_.begin(), events_.end());
  }

  inline const std::vector<legion::interface::Event>& events() const {
    std::lock_guard<std::mutex> lock(*events_mutex_);
    return events_;
  }
 ...
 protected:
  ...
  //时间集
  std::vector<legion::interface::Event> events_;
};
}  // namespace interface
}  // namespace legion
