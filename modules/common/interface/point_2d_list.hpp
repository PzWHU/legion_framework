...
class Point2dList {
 public:
  ...
  inline void set_point2d_list(
      std::vector<legion::interface::Point2D>* point2d_list) {
    std::lock_guard<std::mutex> lock(*point2d_list_mutex_);
    point2d_list_.assign(point2d_list->begin(), point2d_list->end());
  }

  inline void set_point2d_list(
      const std::vector<legion::interface::Point2D>& point2d_list) {
    std::lock_guard<std::mutex> lock(*point2d_list_mutex_);
    point2d_list_ = point2d_list;
  }

  inline void set_point2d_list(const uint32_t index,
                               legion::interface::Point2D& point2d_list) {
    std::lock_guard<std::mutex> lock(*point2d_list_mutex_);
    point2d_list_[index] = point2d_list;
  }

  inline void add_point2d_list(const legion::interface::Point2D& point2d_list) {
    std::lock_guard<std::mutex> lock(*point2d_list_mutex_);
    point2d_list_.emplace_back(point2d_list);
  }

  inline const legion::interface::Point2D& point2d_list(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*point2d_list_mutex_);
    return point2d_list_[index];
  }

  inline std::vector<legion::interface::Point2D>* mutable_point2d_list() {
    std::lock_guard<std::mutex> lock(*point2d_list_mutex_);
    return &point2d_list_;
  }

  inline void point2d_list(
      std::vector<legion::interface::Point2D>& point2d_list) const {
    std::lock_guard<std::mutex> lock(*point2d_list_mutex_);
    point2d_list.assign(point2d_list_.begin(), point2d_list_.end());
  }

  inline const std::vector<legion::interface::Point2D>& point2d_list() const {
    std::lock_guard<std::mutex> lock(*point2d_list_mutex_);
    return point2d_list_;
  }
 ...
 protected:
  ...
  std::vector<legion::interface::Point2D> point2d_list_;
};
}  // namespace interface
}  // namespace legion
