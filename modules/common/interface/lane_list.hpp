...
class LaneList {
 public:
  ...
  inline void set_camera_laneline(
      const std::vector<legion::interface::LaneLine>& camera_laneline) {
    std::lock_guard<std::mutex> lock(*camera_laneline_mutex_);
    camera_laneline_ = camera_laneline;
  }

  inline void set_camera_laneline(
      const uint32_t index, legion::interface::LaneLine& camera_laneline) {
    std::lock_guard<std::mutex> lock(*camera_laneline_mutex_);
    camera_laneline_[index] = camera_laneline;
  }

  inline void add_camera_laneline(
      const legion::interface::LaneLine& camera_laneline) {
    std::lock_guard<std::mutex> lock(*camera_laneline_mutex_);
    camera_laneline_.emplace_back(camera_laneline);
  }

  inline const legion::interface::LaneLine& camera_laneline(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*camera_laneline_mutex_);
    return camera_laneline_[index];
  }

  inline std::vector<legion::interface::LaneLine>* mutable_camera_laneline() {
    std::lock_guard<std::mutex> lock(*camera_laneline_mutex_);
    return &camera_laneline_;
  }

  inline void camera_laneline(
      std::vector<legion::interface::LaneLine>& camera_laneline) const {
    std::lock_guard<std::mutex> lock(*camera_laneline_mutex_);
    camera_laneline.assign(camera_laneline_.begin(), camera_laneline_.end());
  }

  inline const std::vector<legion::interface::LaneLine>& camera_laneline()
      const {
    std::lock_guard<std::mutex> lock(*camera_laneline_mutex_);
    return camera_laneline_;
  }
  ...
  inline void set_hpp(const legion::interface::HolisticPathPrediction& hpp) {
    hpp_ = hpp;
    hpp_ptr_ = &hpp_;
  }

  inline const legion::interface::HolisticPathPrediction& hpp() const {
    return hpp_;
  }

  inline legion::interface::HolisticPathPrediction* mutable_hpp() {
    return &hpp_;
  }

  inline bool has_hpp() { return (hpp_ptr_ != nullptr); }

  inline void set_road_marks(
  ...
  inline void set_road_marks(
      const std::vector<legion::interface::RoadMark>& road_marks) {
    std::lock_guard<std::mutex> lock(*road_marks_mutex_);
    road_marks_ = road_marks;
  }

  inline void set_road_marks(const uint32_t index,
                             legion::interface::RoadMark& road_marks) {
    std::lock_guard<std::mutex> lock(*road_marks_mutex_);
    road_marks_[index] = road_marks;
  }

  inline void add_road_marks(const legion::interface::RoadMark& road_marks) {
    std::lock_guard<std::mutex> lock(*road_marks_mutex_);
    road_marks_.emplace_back(road_marks);
  }

  inline const legion::interface::RoadMark& road_marks(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*road_marks_mutex_);
    return road_marks_[index];
  }

  inline std::vector<legion::interface::RoadMark>* mutable_road_marks() {
    std::lock_guard<std::mutex> lock(*road_marks_mutex_);
    return &road_marks_;
  }

  inline void road_marks(
      std::vector<legion::interface::RoadMark>& road_marks) const {
    std::lock_guard<std::mutex> lock(*road_marks_mutex_);
    road_marks.assign(road_marks_.begin(), road_marks_.end());
  }

  inline const std::vector<legion::interface::RoadMark>& road_marks() const {
    std::lock_guard<std::mutex> lock(*road_marks_mutex_);
    return road_marks_;
  }
 ...
 protected:
  ...
  //车道线检测结果数组
  std::vector<legion::interface::LaneLine> camera_laneline_;
  //行驶预测线
  legion::interface::HolisticPathPrediction hpp_;
  legion::interface::HolisticPathPrediction* hpp_ptr_ = nullptr;
  //路面标识
  std::vector<legion::interface::RoadMark> road_marks_;
};
}  // namespace interface
}  // namespace legion
