...
class LocalMap {
 public:
  ...
  inline void set_ego_point(const legion::interface::Point3D& ego_point) {
    ego_point_ = ego_point;
    ego_point_ptr_ = &ego_point_;
  }

  inline const legion::interface::Point3D& ego_point() const {
    return ego_point_;
  }

  inline legion::interface::Point3D* mutable_ego_point() { return &ego_point_; }
  ...
  inline void set_lane_list(
      std::vector<legion::interface::LaneInfoExtend>* lane_list) {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list_.assign(lane_list->begin(), lane_list->end());
  }

  inline void set_lane_list(
      const std::vector<legion::interface::LaneInfoExtend>& lane_list) {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list_ = lane_list;
  }

  inline void set_lane_list(const uint32_t index,
                            legion::interface::LaneInfoExtend& lane_list) {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list_[index] = lane_list;
  }

  inline void add_lane_list(
      const legion::interface::LaneInfoExtend& lane_list) {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list_.emplace_back(lane_list);
  }

  inline const legion::interface::LaneInfoExtend& lane_list(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    return lane_list_[index];
  }

  inline std::vector<legion::interface::LaneInfoExtend>* mutable_lane_list() {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    return &lane_list_;
  }

  inline void lane_list(
      std::vector<legion::interface::LaneInfoExtend>& lane_list) const {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list.assign(lane_list_.begin(), lane_list_.end());
  }

  inline const std::vector<legion::interface::LaneInfoExtend>& lane_list()
      const {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    return lane_list_;
  }
 ...
 protected:
  ...
  legion::interface::Point3D ego_point_;
  legion::interface::Point3D* ego_point_ptr_ = nullptr;
  //生成该LocalMap局部地图消息的范围，以ego_point为中心，range为边长的正方形，单位：m
  double range_;
  double* range_ptr_ = nullptr;
  //自车周边LocalMap范围内的车道信息列表
  std::vector<legion::interface::LaneInfoExtend> lane_list_;
};
}  // namespace interface
}  // namespace legion
