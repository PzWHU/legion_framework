...
class JunctionInfo {
 public:
  JunctionInfo() {
    ...
    id_ = 0;
    light_flag_ = legion::common::IsValid::INVALID;
    light_color_ = legion::common::TrafficLightColor::COLOR_UNKNOWN;
    light_remain_time_ = 0.0;
    distance_to_stop_ = 0.0;
    direction_flag_ = legion::common::IsValid::INVALID;
    direction_ = legion::common::Direction::DIR_INVALID;
  ...
  inline void set_light_flag(const legion::common::IsValid& light_flag) {
    light_flag_ = light_flag;
    light_flag_ptr_ = &light_flag_;
  }

  inline const legion::common::IsValid& light_flag() const {
    return light_flag_;
  }

  inline legion::common::IsValid* mutable_light_flag() { return &light_flag_; }

  inline bool has_light_flag() { return (light_flag_ptr_ != nullptr); }

  inline void set_light_color(
      const legion::common::TrafficLightColor& light_color) {
    light_color_ = light_color;
    light_color_ptr_ = &light_color_;
  }

  inline const legion::common::TrafficLightColor& light_color() const {
    return light_color_;
  }

  inline legion::common::TrafficLightColor* mutable_light_color() {
    return &light_color_;
  }
  ...
  inline void set_direction_flag(
      const legion::common::IsValid& direction_flag) {
    direction_flag_ = direction_flag;
    direction_flag_ptr_ = &direction_flag_;
  }

  inline const legion::common::IsValid& direction_flag() const {
    return direction_flag_;
  }

  inline legion::common::IsValid* mutable_direction_flag() {
    return &direction_flag_;
  }

  inline bool has_direction_flag() { return (direction_flag_ptr_ != nullptr); }

  inline void set_direction(const legion::common::Direction& direction) {
    direction_ = direction;
    direction_ptr_ = &direction_;
  }

  inline const legion::common::Direction& direction() const {
    return direction_;
  }

  inline legion::common::Direction* mutable_direction() { return &direction_; }
  ...
  inline void set_stop_line(
      const std::vector<legion::interface::Point3D>& stop_line) {
    std::lock_guard<std::mutex> lock(*stop_line_mutex_);
    stop_line_ = stop_line;
  }

  inline void set_stop_line(const uint32_t index,
                            legion::interface::Point3D& stop_line) {
    std::lock_guard<std::mutex> lock(*stop_line_mutex_);
    stop_line_[index] = stop_line;
  }

  inline void add_stop_line(const legion::interface::Point3D& stop_line) {
    std::lock_guard<std::mutex> lock(*stop_line_mutex_);
    stop_line_.emplace_back(stop_line);
  }

  inline const legion::interface::Point3D& stop_line(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*stop_line_mutex_);
    return stop_line_[index];
  }

  inline std::vector<legion::interface::Point3D>* mutable_stop_line() {
    std::lock_guard<std::mutex> lock(*stop_line_mutex_);
    return &stop_line_;
  }

  inline void stop_line(
      std::vector<legion::interface::Point3D>& stop_line) const {
    std::lock_guard<std::mutex> lock(*stop_line_mutex_);
    stop_line.assign(stop_line_.begin(), stop_line_.end());
  }

  inline const std::vector<legion::interface::Point3D>& stop_line() const {
    std::lock_guard<std::mutex> lock(*stop_line_mutex_);
    return stop_line_;
  }
 ...
 protected:
  ...
  legion::common::IsValid light_flag_;
  legion::common::IsValid* light_flag_ptr_ = nullptr;
  //信号灯颜色，绿灯可通行，false红灯不能通行
  legion::common::TrafficLightColor light_color_;
  legion::common::TrafficLightColor* light_color_ptr_ = nullptr;
  //信号灯剩余时间
  ...
  //是否有效，无效就可能是清除了，或者已经过路口了
  legion::common::IsValid direction_flag_;
  legion::common::IsValid* direction_flag_ptr_ = nullptr;
  //路口通行方向，0：无效、1：左、2：前、3：右
  legion::common::Direction direction_;
  legion::common::Direction* direction_ptr_ = nullptr;
  //距路口距离
  double distance_to_junction_;
  double* distance_to_junction_ptr_ = nullptr;
  //路口停止线
  std::vector<legion::interface::Point3D> stop_line_;
};
}  // namespace interface
}  // namespace legion
