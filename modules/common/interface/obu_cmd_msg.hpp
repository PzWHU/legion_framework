...
class ObuCmdMsg {
 public:
  ...
  inline void set_obu_cmd_list(
      const std::vector<legion::interface::ObuCmd>& obu_cmd_list) {
    std::lock_guard<std::mutex> lock(*obu_cmd_list_mutex_);
    obu_cmd_list_ = obu_cmd_list;
  }

  inline void set_obu_cmd_list(const uint32_t index,
                               legion::interface::ObuCmd& obu_cmd_list) {
    std::lock_guard<std::mutex> lock(*obu_cmd_list_mutex_);
    obu_cmd_list_[index] = obu_cmd_list;
  }

  inline void add_obu_cmd_list(const legion::interface::ObuCmd& obu_cmd_list) {
    std::lock_guard<std::mutex> lock(*obu_cmd_list_mutex_);
    obu_cmd_list_.emplace_back(obu_cmd_list);
  }

  inline const legion::interface::ObuCmd& obu_cmd_list(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*obu_cmd_list_mutex_);
    return obu_cmd_list_[index];
  }

  inline std::vector<legion::interface::ObuCmd>* mutable_obu_cmd_list() {
    std::lock_guard<std::mutex> lock(*obu_cmd_list_mutex_);
    return &obu_cmd_list_;
  }

  inline void obu_cmd_list(
      std::vector<legion::interface::ObuCmd>& obu_cmd_list) const {
    std::lock_guard<std::mutex> lock(*obu_cmd_list_mutex_);
    obu_cmd_list.assign(obu_cmd_list_.begin(), obu_cmd_list_.end());
  }

  inline const std::vector<legion::interface::ObuCmd>& obu_cmd_list() const {
    std::lock_guard<std::mutex> lock(*obu_cmd_list_mutex_);
    return obu_cmd_list_;
  }
 ...
 protected:
  ...
  //命令列表
  std::vector<legion::interface::ObuCmd> obu_cmd_list_;
};
}  // namespace interface
}  // namespace legion
