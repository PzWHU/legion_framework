...
class HMIParkingInfoList {
 public:
  ...
  inline void set_parking_info(
      std::vector<legion::interface::HMIParkingInfo>* parking_info) {
    std::lock_guard<std::mutex> lock(*parking_info_mutex_);
    parking_info_.assign(parking_info->begin(), parking_info->end());
  }

  inline void set_parking_info(
      const std::vector<legion::interface::HMIParkingInfo>& parking_info) {
    std::lock_guard<std::mutex> lock(*parking_info_mutex_);
    parking_info_ = parking_info;
  }

  inline void set_parking_info(
      const uint32_t index, legion::interface::HMIParkingInfo& parking_info) {
    std::lock_guard<std::mutex> lock(*parking_info_mutex_);
    parking_info_[index] = parking_info;
  }

  inline void add_parking_info(
      const legion::interface::HMIParkingInfo& parking_info) {
    std::lock_guard<std::mutex> lock(*parking_info_mutex_);
    parking_info_.emplace_back(parking_info);
  }

  inline const legion::interface::HMIParkingInfo& parking_info(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*parking_info_mutex_);
    return parking_info_[index];
  }

  inline std::vector<legion::interface::HMIParkingInfo>*
  ...
  inline void parking_info(
      std::vector<legion::interface::HMIParkingInfo>& parking_info) const {
    std::lock_guard<std::mutex> lock(*parking_info_mutex_);
    parking_info.assign(parking_info_.begin(), parking_info_.end());
  }

  inline const std::vector<legion::interface::HMIParkingInfo>& parking_info()
      const {
    std::lock_guard<std::mutex> lock(*parking_info_mutex_);
    return parking_info_;
  }
 ...
 protected:
  ...
  std::vector<legion::interface::HMIParkingInfo> parking_info_;
