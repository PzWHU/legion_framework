...
class RoutingResponse {
 public:
  RoutingResponse() {
    ...
    plan_status_ = legion::interface::RoutingResponse::PlanStatus::PLAN_SUCCESS;
    replan_flag_ =
        legion::interface::RoutingResponse::ReplanFlag::REPLAN_FLAG_NONE;
    route_reason_ =
        legion::interface::RoutingResponse::RouteReason::ROUTE_REASON_HMI;
    clear_lane_list();
  }
  ...
  inline void set_plan_status(
      const legion::interface::RoutingResponse::PlanStatus& plan_status) {
    plan_status_ = plan_status;
    plan_status_ptr_ = &plan_status_;
  }

  inline const legion::interface::RoutingResponse::PlanStatus& plan_status()
      const {
    return plan_status_;
  }

  inline legion::interface::RoutingResponse::PlanStatus* mutable_plan_status() {
    return &plan_status_;
  }
  ...
  inline void set_replan_flag(
      const legion::interface::RoutingResponse::ReplanFlag& replan_flag) {
    replan_flag_ = replan_flag;
    replan_flag_ptr_ = &replan_flag_;
  }

  inline const legion::interface::RoutingResponse::ReplanFlag& replan_flag()
      const {
    return replan_flag_;
  }

  inline legion::interface::RoutingResponse::ReplanFlag* mutable_replan_flag() {
    return &replan_flag_;
  }
  ...
  inline void set_route_reason(
      const legion::interface::RoutingResponse::RouteReason& route_reason) {
    route_reason_ = route_reason;
    route_reason_ptr_ = &route_reason_;
  }

  inline const legion::interface::RoutingResponse::RouteReason& route_reason()
      const {
    return route_reason_;
  }

  inline legion::interface::RoutingResponse::RouteReason*
  ...
  inline void set_lane_list(
      const std::vector<legion::interface::LaneInfo>& lane_list) {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list_ = lane_list;
  }

  inline void set_lane_list(const uint32_t index,
                            legion::interface::LaneInfo& lane_list) {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list_[index] = lane_list;
  }

  inline void add_lane_list(const legion::interface::LaneInfo& lane_list) {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list_.emplace_back(lane_list);
  }

  inline const legion::interface::LaneInfo& lane_list(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    return lane_list_[index];
  }

  inline std::vector<legion::interface::LaneInfo>* mutable_lane_list() {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    return &lane_list_;
  }

  inline void lane_list(
      std::vector<legion::interface::LaneInfo>& lane_list) const {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    lane_list.assign(lane_list_.begin(), lane_list_.end());
  }

  inline const std::vector<legion::interface::LaneInfo>& lane_list() const {
    std::lock_guard<std::mutex> lock(*lane_list_mutex_);
    return lane_list_;
  }
 ...
 protected:
  ...
  //规划结果状态
  legion::interface::RoutingResponse::PlanStatus plan_status_;
  legion::interface::RoutingResponse::PlanStatus* plan_status_ptr_ = nullptr;
  // REPLAN_FLAG_NONE=0  //不需要重规划 REPLAN_FLAG_NAVI=1 //网络层重规划
  // REPLAN_FLAG_HUMAN=2 //人为重规划
  legion::interface::RoutingResponse::ReplanFlag replan_flag_;
  legion::interface::RoutingResponse::ReplanFlag* replan_flag_ptr_ = nullptr;
  //规划原因 ROUTE_REASON_HMI = 0 //hmi上触发规划 ROUTE_REASON_PLATOON_HEADER =
  //1 //车队头车下发规划 ROUTE_REASON_PLATOON_OTHER = 2 //车队其他车辆下发规划
  legion::interface::RoutingResponse::RouteReason route_reason_;
  legion::interface::RoutingResponse::RouteReason* route_reason_ptr_ = nullptr;
  //分段路径规划结果
  std::vector<legion::interface::LaneInfo> lane_list_;
};
}  // namespace interface
}  // namespace legion
