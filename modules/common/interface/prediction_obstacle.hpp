...
class PredictionObstacle {
 public:
  ...
  inline void set_perception_obstacle(
      const legion::interface::PerceptionObstacle& perception_obstacle) {
    perception_obstacle_ = perception_obstacle;
    perception_obstacle_ptr_ = &perception_obstacle_;
  }

  inline const legion::interface::PerceptionObstacle& perception_obstacle()
      const {
    return perception_obstacle_;
  }

  inline legion::interface::PerceptionObstacle* mutable_perception_obstacle() {
    return &perception_obstacle_;
  }
  ...
  inline void set_trajectory(
      ...
      legion::interface::TrajectoryInPrediction& trajectory) {
    std::lock_guard<std::mutex> lock(*trajectory_mutex_);
    trajectory_[index] = trajectory;
  }

  inline void add_trajectory(
      const legion::interface::TrajectoryInPrediction& trajectory) {
    std::lock_guard<std::mutex> lock(*trajectory_mutex_);
    trajectory_.emplace_back(trajectory);
  }

  inline const legion::interface::TrajectoryInPrediction& trajectory(
  ...
  inline void trajectory(std::vector<legion::interface::TrajectoryInPrediction>&
                             trajectory) const {
    std::lock_guard<std::mutex> lock(*trajectory_mutex_);
  ...
  inline void set_intent(const legion::interface::ObstacleIntent& intent) {
    intent_ = intent;
    intent_ptr_ = &intent_;
  }

  inline const legion::interface::ObstacleIntent& intent() const {
    return intent_;
  }

  inline legion::interface::ObstacleIntent* mutable_intent() {
    return &intent_;
  }
  ...
  inline void set_interactive_tag(
      const legion::interface::ObstacleInteractiveTag& interactive_tag) {
    interactive_tag_ = interactive_tag;
    interactive_tag_ptr_ = &interactive_tag_;
  }

  inline const legion::interface::ObstacleInteractiveTag& interactive_tag()
      const {
    return interactive_tag_;
  }

  inline legion::interface::ObstacleInteractiveTag* mutable_interactive_tag() {
    return &interactive_tag_;
  }
 ...
 protected:
  ...
  legion::interface::PerceptionObstacle perception_obstacle_;
  legion::interface::PerceptionObstacle* perception_obstacle_ptr_ = nullptr;
  // GPS time in seconds
  double timestamp_;
  ...
  std::vector<legion::interface::TrajectoryInPrediction> trajectory_;
  //估计障碍物的意图        0-UNKNOWN, 1-STOP, 2-STATIONARY, 3-移动 MOVING,
  //4-HANGE_LANE, 5-LOW_ACCELERATION, 6-HIGH_ACCELERATION, 7-LOW_DECELERATION,
  //8-HIGH_DECELERATION,
  legion::interface::ObstacleIntent intent_;
  legion::interface::ObstacleIntent* intent_ptr_ = nullptr;
  ...
  legion::interface::ObstacleInteractiveTag interactive_tag_;
  legion::interface::ObstacleInteractiveTag* interactive_tag_ptr_ = nullptr;
