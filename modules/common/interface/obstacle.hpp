...
class Obstacle {
 public:
  Obstacle() {
    ...
    tracking_time_ = 0.0;
    type_ = legion::common::ObstacleType::OBSTACLE_UNKNOWN;
    confidence_ = 0.0;
    confidence_type_ =
        legion::interface::Obstacle::ConfidenceType::CONFIDENCE_UNKNOWN;
    clear_drops();
    sub_type_ = legion::common::ObstacleSubType::ST_UNKNOWN;
    ...
    cipv_flag_ = 0;
    lane_position_ = legion::common::LanePosition::NEXT_LEFT_LANE;
    pihp_percentage_ = 0.0;
    blinker_flag_ = legion::common::BlinkerFlag::BLINKER_FLAG_OFF;
    fusion_type_ = legion::interface::Obstacle::FusionType::CAMERA;
  }
  ~Obstacle() = default;
  ...
  inline void set_timestamp(const legion::interface::Time& timestamp) {
    timestamp_ = timestamp;
    timestamp_ptr_ = &timestamp_;
  }

  inline const legion::interface::Time& timestamp() const { return timestamp_; }

  inline legion::interface::Time* mutable_timestamp() { return &timestamp_; }
  ...
  inline void set_create_time(const legion::interface::Time& create_time) {
    create_time_ = create_time;
    create_time_ptr_ = &create_time_;
  }

  inline const legion::interface::Time& create_time() const {
    return create_time_;
  }

  inline legion::interface::Time* mutable_create_time() {
    return &create_time_;
  }

  inline bool has_create_time() { return (create_time_ptr_ != nullptr); }

  inline void set_last_updated_time(
      const legion::interface::Time& last_updated_time) {
    last_updated_time_ = last_updated_time;
    last_updated_time_ptr_ = &last_updated_time_;
  }

  inline const legion::interface::Time& last_updated_time() const {
    return last_updated_time_;
  }

  inline legion::interface::Time* mutable_last_updated_time() {
    return &last_updated_time_;
  }
  ...
  inline void set_center_pos_vehicle(
      const legion::interface::Point3D& center_pos_vehicle) {
    center_pos_vehicle_ = center_pos_vehicle;
    center_pos_vehicle_ptr_ = &center_pos_vehicle_;
  }

  inline const legion::interface::Point3D& center_pos_vehicle() const {
    return center_pos_vehicle_;
  }

  inline legion::interface::Point3D* mutable_center_pos_vehicle() {
    return &center_pos_vehicle_;
  }
  ...
  inline void set_center_pos_abs(
      const legion::interface::Point3D& center_pos_abs) {
    center_pos_abs_ = center_pos_abs;
    center_pos_abs_ptr_ = &center_pos_abs_;
  }

  inline const legion::interface::Point3D& center_pos_abs() const {
    return center_pos_abs_;
  }

  inline legion::interface::Point3D* mutable_center_pos_abs() {
    return &center_pos_abs_;
  }
  ...
  inline void set_velocity_vehicle(
      const legion::interface::Point3D& velocity_vehicle) {
    velocity_vehicle_ = velocity_vehicle;
    velocity_vehicle_ptr_ = &velocity_vehicle_;
  }

  inline const legion::interface::Point3D& velocity_vehicle() const {
    return velocity_vehicle_;
  }

  inline legion::interface::Point3D* mutable_velocity_vehicle() {
    return &velocity_vehicle_;
  }
  ...
  inline void set_velocity_abs(const legion::interface::Point3D& velocity_abs) {
    velocity_abs_ = velocity_abs;
    velocity_abs_ptr_ = &velocity_abs_;
  }

  inline const legion::interface::Point3D& velocity_abs() const {
    return velocity_abs_;
  }

  inline legion::interface::Point3D* mutable_velocity_abs() {
    return &velocity_abs_;
  }
  ...
  inline void set_image_key_points(
      const std::vector<legion::interface::ImageKeyPoint>& image_key_points) {
    std::lock_guard<std::mutex> lock(*image_key_points_mutex_);
  ...
  inline void set_image_key_points(
      ...
      legion::interface::ImageKeyPoint& image_key_points) {
    std::lock_guard<std::mutex> lock(*image_key_points_mutex_);
    image_key_points_[index] = image_key_points;
  }

  inline void add_image_key_points(
      const legion::interface::ImageKeyPoint& image_key_points) {
    std::lock_guard<std::mutex> lock(*image_key_points_mutex_);
    image_key_points_.emplace_back(image_key_points);
  }

  inline const legion::interface::ImageKeyPoint& image_key_points(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*image_key_points_mutex_);
    return image_key_points_[index];
  }

  inline std::vector<legion::interface::ImageKeyPoint>*
  ...
  inline void image_key_points(
      std::vector<legion::interface::ImageKeyPoint>& image_key_points) const {
    std::lock_guard<std::mutex> lock(*image_key_points_mutex_);
    image_key_points.assign(image_key_points_.begin(), image_key_points_.end());
  }

  inline const std::vector<legion::interface::ImageKeyPoint>& image_key_points()
      const {
    std::lock_guard<std::mutex> lock(*image_key_points_mutex_);
    return image_key_points_;
  }
  ...
  inline void set_polygon_point_abs(
      const std::vector<legion::interface::Point3D>& polygon_point_abs) {
    std::lock_guard<std::mutex> lock(*polygon_point_abs_mutex_);
    polygon_point_abs_ = polygon_point_abs;
  }

  inline void set_polygon_point_abs(
      const uint32_t index, legion::interface::Point3D& polygon_point_abs) {
    std::lock_guard<std::mutex> lock(*polygon_point_abs_mutex_);
    polygon_point_abs_[index] = polygon_point_abs;
  }

  inline void add_polygon_point_abs(
      const legion::interface::Point3D& polygon_point_abs) {
    std::lock_guard<std::mutex> lock(*polygon_point_abs_mutex_);
    polygon_point_abs_.emplace_back(polygon_point_abs);
  }

  inline const legion::interface::Point3D& polygon_point_abs(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*polygon_point_abs_mutex_);
    return polygon_point_abs_[index];
  }

  inline std::vector<legion::interface::Point3D>* mutable_polygon_point_abs() {
    std::lock_guard<std::mutex> lock(*polygon_point_abs_mutex_);
    return &polygon_point_abs_;
  }

  inline void polygon_point_abs(
      std::vector<legion::interface::Point3D>& polygon_point_abs) const {
    std::lock_guard<std::mutex> lock(*polygon_point_abs_mutex_);
    polygon_point_abs.assign(polygon_point_abs_.begin(),
                             polygon_point_abs_.end());
  }

  inline const std::vector<legion::interface::Point3D>& polygon_point_abs()
      const {
    std::lock_guard<std::mutex> lock(*polygon_point_abs_mutex_);
    return polygon_point_abs_;
  }
  ...
  inline void set_polygon_point_vehicle(
      const std::vector<legion::interface::Point3D>& polygon_point_vehicle) {
    std::lock_guard<std::mutex> lock(*polygon_point_vehicle_mutex_);
    polygon_point_vehicle_ = polygon_point_vehicle;
  }

  inline void set_polygon_point_vehicle(
      const uint32_t index, legion::interface::Point3D& polygon_point_vehicle) {
    std::lock_guard<std::mutex> lock(*polygon_point_vehicle_mutex_);
    polygon_point_vehicle_[index] = polygon_point_vehicle;
  }

  inline void add_polygon_point_vehicle(
      const legion::interface::Point3D& polygon_point_vehicle) {
    std::lock_guard<std::mutex> lock(*polygon_point_vehicle_mutex_);
    polygon_point_vehicle_.emplace_back(polygon_point_vehicle);
  }

  inline const legion::interface::Point3D& polygon_point_vehicle(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*polygon_point_vehicle_mutex_);
  ...
  inline void polygon_point_vehicle(
      std::vector<legion::interface::Point3D>& polygon_point_vehicle) const {
    std::lock_guard<std::mutex> lock(*polygon_point_vehicle_mutex_);
    polygon_point_vehicle.assign(polygon_point_vehicle_.begin(),
                                 polygon_point_vehicle_.end());
  }

  inline const std::vector<legion::interface::Point3D>& polygon_point_vehicle()
      const {
    std::lock_guard<std::mutex> lock(*polygon_point_vehicle_mutex_);
    return polygon_point_vehicle_;
  }
  ...
  inline void set_type(const legion::common::ObstacleType& type) {
    type_ = type;
    type_ptr_ = &type_;
  }

  inline const legion::common::ObstacleType& type() const { return type_; }

  inline legion::common::ObstacleType* mutable_type() { return &type_; }
  ...
  inline void set_confidence_type(
      const legion::interface::Obstacle::ConfidenceType& confidence_type) {
    confidence_type_ = confidence_type;
    confidence_type_ptr_ = &confidence_type_;
  }

  inline const legion::interface::Obstacle::ConfidenceType& confidence_type()
      const {
    return confidence_type_;
  }

  inline legion::interface::Obstacle::ConfidenceType*
  ...
  inline void set_drops(std::vector<legion::interface::Point3D>* drops) {
    std::lock_guard<std::mutex> lock(*drops_mutex_);
    drops_.assign(drops->begin(), drops->end());
  }

  inline void set_drops(const std::vector<legion::interface::Point3D>& drops) {
    std::lock_guard<std::mutex> lock(*drops_mutex_);
    drops_ = drops;
  }

  inline void set_drops(const uint32_t index,
                        legion::interface::Point3D& drops) {
    std::lock_guard<std::mutex> lock(*drops_mutex_);
    drops_[index] = drops;
  }

  inline void add_drops(const legion::interface::Point3D& drops) {
    std::lock_guard<std::mutex> lock(*drops_mutex_);
    drops_.emplace_back(drops);
  }

  inline const legion::interface::Point3D& drops(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*drops_mutex_);
    return drops_[index];
  }

  inline std::vector<legion::interface::Point3D>* mutable_drops() {
    std::lock_guard<std::mutex> lock(*drops_mutex_);
    return &drops_;
  }

  inline void drops(std::vector<legion::interface::Point3D>& drops) const {
    std::lock_guard<std::mutex> lock(*drops_mutex_);
    drops.assign(drops_.begin(), drops_.end());
  }

  inline const std::vector<legion::interface::Point3D>& drops() const {
    std::lock_guard<std::mutex> lock(*drops_mutex_);
    return drops_;
  }
  ...
  inline void set_acceleration_vehicle(
      const legion::interface::Point3D& acceleration_vehicle) {
    acceleration_vehicle_ = acceleration_vehicle;
    acceleration_vehicle_ptr_ = &acceleration_vehicle_;
  }

  inline const legion::interface::Point3D& acceleration_vehicle() const {
    return acceleration_vehicle_;
  }

  inline legion::interface::Point3D* mutable_acceleration_vehicle() {
    return &acceleration_vehicle_;
  }
  ...
  inline void set_acceleration_abs(
      const legion::interface::Point3D& acceleration_abs) {
    acceleration_abs_ = acceleration_abs;
    acceleration_abs_ptr_ = &acceleration_abs_;
  }

  inline const legion::interface::Point3D& acceleration_abs() const {
    return acceleration_abs_;
  }

  inline legion::interface::Point3D* mutable_acceleration_abs() {
    return &acceleration_abs_;
  }
  ...
  inline void set_anchor_point_image(
      const legion::interface::Point2D& anchor_point_image) {
    anchor_point_image_ = anchor_point_image;
    anchor_point_image_ptr_ = &anchor_point_image_;
  }

  inline const legion::interface::Point2D& anchor_point_image() const {
    return anchor_point_image_;
  }

  inline legion::interface::Point2D* mutable_anchor_point_image() {
    return &anchor_point_image_;
  }
  ...
  inline void set_anchor_point_vehicle(
      const legion::interface::Point3D& anchor_point_vehicle) {
    anchor_point_vehicle_ = anchor_point_vehicle;
    anchor_point_vehicle_ptr_ = &anchor_point_vehicle_;
  }

  inline const legion::interface::Point3D& anchor_point_vehicle() const {
    return anchor_point_vehicle_;
  }

  inline legion::interface::Point3D* mutable_anchor_point_vehicle() {
    return &anchor_point_vehicle_;
  }
  ...
  inline void set_anchor_point_abs(
      const legion::interface::Point3D& anchor_point_abs) {
    anchor_point_abs_ = anchor_point_abs;
    anchor_point_abs_ptr_ = &anchor_point_abs_;
  }

  inline const legion::interface::Point3D& anchor_point_abs() const {
    return anchor_point_abs_;
  }

  inline legion::interface::Point3D* mutable_anchor_point_abs() {
    return &anchor_point_abs_;
  }
  ...
  inline void set_bbox2d_rear(const legion::interface::BBox2D& bbox2d_rear) {
    bbox2d_rear_ = bbox2d_rear;
    bbox2d_rear_ptr_ = &bbox2d_rear_;
  }

  inline const legion::interface::BBox2D& bbox2d_rear() const {
    return bbox2d_rear_;
  }

  inline legion::interface::BBox2D* mutable_bbox2d_rear() {
    return &bbox2d_rear_;
  }

  inline bool has_bbox2d_rear() { return (bbox2d_rear_ptr_ != nullptr); }

  inline void set_sub_type(const legion::common::ObstacleSubType& sub_type) {
    sub_type_ = sub_type;
    sub_type_ptr_ = &sub_type_;
  }

  inline const legion::common::ObstacleSubType& sub_type() const {
    return sub_type_;
  }

  inline legion::common::ObstacleSubType* mutable_sub_type() {
    return &sub_type_;
  }
  ...
  inline void add_position_abs_covariance(
      const double& position_abs_covariance) {
    std::lock_guard<std::mutex> lock(*position_abs_covariance_mutex_);
  ...
  inline void position_abs_covariance(
      std::vector<double>& position_abs_covariance) const {
    std::lock_guard<std::mutex> lock(*position_abs_covariance_mutex_);
  ...
  inline void add_velocity_abs_covariance(
      const double& velocity_abs_covariance) {
    std::lock_guard<std::mutex> lock(*velocity_abs_covariance_mutex_);
  ...
  inline void velocity_abs_covariance(
      std::vector<double>& velocity_abs_covariance) const {
    std::lock_guard<std::mutex> lock(*velocity_abs_covariance_mutex_);
  ...
  inline void set_acceleration_abs_covariance(
      const uint32_t index, double& acceleration_abs_covariance) {
    std::lock_guard<std::mutex> lock(*acceleration_abs_covariance_mutex_);
    acceleration_abs_covariance_[index] = acceleration_abs_covariance;
  }

  inline void add_acceleration_abs_covariance(
      const double& acceleration_abs_covariance) {
    std::lock_guard<std::mutex> lock(*acceleration_abs_covariance_mutex_);
  ...
  inline void set_position_vehicle_covariance(
      const uint32_t index, double& position_vehicle_covariance) {
    std::lock_guard<std::mutex> lock(*position_vehicle_covariance_mutex_);
    position_vehicle_covariance_[index] = position_vehicle_covariance;
  }

  inline void add_position_vehicle_covariance(
      const double& position_vehicle_covariance) {
    std::lock_guard<std::mutex> lock(*position_vehicle_covariance_mutex_);
  ...
  inline void set_velocity_vehicle_covariance(
      const uint32_t index, double& velocity_vehicle_covariance) {
    std::lock_guard<std::mutex> lock(*velocity_vehicle_covariance_mutex_);
    velocity_vehicle_covariance_[index] = velocity_vehicle_covariance;
  }

  inline void add_velocity_vehicle_covariance(
      const double& velocity_vehicle_covariance) {
    std::lock_guard<std::mutex> lock(*velocity_vehicle_covariance_mutex_);
  ...
  inline void set_acceleration_vehicle_covariance(
      const uint32_t index, double& acceleration_vehicle_covariance) {
    std::lock_guard<std::mutex> lock(*acceleration_vehicle_covariance_mutex_);
  ...
  inline void set_theta_vehicle_covariance(
      const double& theta_vehicle_covariance) {
  ...
  inline void set_sensor_calibrator(
      const legion::interface::SensorCalibrator& sensor_calibrator) {
    sensor_calibrator_ = sensor_calibrator;
    sensor_calibrator_ptr_ = &sensor_calibrator_;
  }

  inline const legion::interface::SensorCalibrator& sensor_calibrator() const {
    return sensor_calibrator_;
  }

  inline legion::interface::SensorCalibrator* mutable_sensor_calibrator() {
    return &sensor_calibrator_;
  }
  ...
  inline void set_lane_position(
      const legion::common::LanePosition& lane_position) {
    lane_position_ = lane_position;
    lane_position_ptr_ = &lane_position_;
  }

  inline const legion::common::LanePosition& lane_position() const {
    return lane_position_;
  }

  inline legion::common::LanePosition* mutable_lane_position() {
    return &lane_position_;
  }
  ...
  inline void set_blinker_flag(
      const legion::common::BlinkerFlag& blinker_flag) {
    blinker_flag_ = blinker_flag;
    blinker_flag_ptr_ = &blinker_flag_;
  }

  inline const legion::common::BlinkerFlag& blinker_flag() const {
    return blinker_flag_;
  }

  inline legion::common::BlinkerFlag* mutable_blinker_flag() {
    return &blinker_flag_;
  }

  inline bool has_blinker_flag() { return (blinker_flag_ptr_ != nullptr); }

  inline void set_fusion_type(
      const legion::interface::Obstacle::FusionType& fusion_type) {
    fusion_type_ = fusion_type;
    fusion_type_ptr_ = &fusion_type_;
  }

  inline const legion::interface::Obstacle::FusionType& fusion_type() const {
    return fusion_type_;
  }

  inline legion::interface::Obstacle::FusionType* mutable_fusion_type() {
    return &fusion_type_;
  }
 ...
 protected:
  ...
  //时间戳
  legion::interface::Time timestamp_;
  legion::interface::Time* timestamp_ptr_ = nullptr;
  //障碍物id
  int32_t id_;
  ...
  //障碍物被识别的时间戳
  legion::interface::Time create_time_;
  legion::interface::Time* create_time_ptr_ = nullptr;
  //障碍物上一次更新的时间
  legion::interface::Time last_updated_time_;
  legion::interface::Time* last_updated_time_ptr_ = nullptr;
  //车辆坐标系下障碍物的中心位置
  legion::interface::Point3D center_pos_vehicle_;
  legion::interface::Point3D* center_pos_vehicle_ptr_ = nullptr;
  //世界坐标系下障碍物的中心位置
  legion::interface::Point3D center_pos_abs_;
  legion::interface::Point3D* center_pos_abs_ptr_ = nullptr;
  //车辆坐标系下障碍物的朝向
  ...
  //车辆坐标系下障碍物的速度
  legion::interface::Point3D velocity_vehicle_;
  legion::interface::Point3D* velocity_vehicle_ptr_ = nullptr;
  //世界坐标系下障碍物的速度
  legion::interface::Point3D velocity_abs_;
  legion::interface::Point3D* velocity_abs_ptr_ = nullptr;
  //障碍物长度
  double length_;
  ...
  //图像坐标系下障碍物多边形
  std::vector<legion::interface::ImageKeyPoint> image_key_points_;
  //世界坐标系下障碍物多边形
  std::vector<legion::interface::Point3D> polygon_point_abs_;
  //车辆坐标系下障碍物多边形
  std::vector<legion::interface::Point3D> polygon_point_vehicle_;
  //障碍物被追踪的时间
  ...
  legion::common::ObstacleType type_;
  legion::common::ObstacleType* type_ptr_ = nullptr;
  //障碍物类别置信度
  double confidence_;
  double* confidence_ptr_ = nullptr;
  //障碍物置信度类别： CONFIDENCE_UNKNOWN = 0; CONFIDENCE_CNN = 1;
  //CONFIDENCE_RADAR = 2;
  legion::interface::Obstacle::ConfidenceType confidence_type_;
  legion::interface::Obstacle::ConfidenceType* confidence_type_ptr_ = nullptr;
  //障碍物轨迹点
  std::vector<legion::interface::Point3D> drops_;
  //车辆坐标系下障碍物的加速度
  legion::interface::Point3D acceleration_vehicle_;
  legion::interface::Point3D* acceleration_vehicle_ptr_ = nullptr;
  //世界坐标系下障碍物的加速度
  legion::interface::Point3D acceleration_abs_;
  legion::interface::Point3D* acceleration_abs_ptr_ = nullptr;
  //障碍物尾框中心点(图像坐标系)
  legion::interface::Point2D anchor_point_image_;
  legion::interface::Point2D* anchor_point_image_ptr_ = nullptr;
  //障碍物尾框中心点(车辆坐标系)
  legion::interface::Point3D anchor_point_vehicle_;
  legion::interface::Point3D* anchor_point_vehicle_ptr_ = nullptr;
  //障碍物尾框中心点(世界坐标系)
  legion::interface::Point3D anchor_point_abs_;
  legion::interface::Point3D* anchor_point_abs_ptr_ = nullptr;
  //障碍物图像框
  legion::interface::BBox2D bbox2d_;
  legion::interface::BBox2D* bbox2d_ptr_ = nullptr;
  //障碍物图像尾框
  legion::interface::BBox2D bbox2d_rear_;
  legion::interface::BBox2D* bbox2d_rear_ptr_ = nullptr;
  ...
  legion::common::ObstacleSubType sub_type_;
  legion::common::ObstacleSubType* sub_type_ptr_ = nullptr;
  //障碍物近地点到地面的高度
  ...
  //传感器标定参数
  legion::interface::SensorCalibrator sensor_calibrator_;
  legion::interface::SensorCalibrator* sensor_calibrator_ptr_ = nullptr;
  ...
  legion::common::LanePosition lane_position_;
  legion::common::LanePosition* lane_position_ptr_ = nullptr;
  //临车道车辆压线比例
  ...
  legion::common::BlinkerFlag blinker_flag_;
  legion::common::BlinkerFlag* blinker_flag_ptr_ = nullptr;
  //融合障碍物类型 0-CAMERA 1-RADAR 2-LIDAR 3-ULTRASONIC 4-FUSED 5-UNKNOWN
  legion::interface::Obstacle::FusionType fusion_type_;
  legion::interface::Obstacle::FusionType* fusion_type_ptr_ = nullptr;
};
}  // namespace interface
}  // namespace legion
