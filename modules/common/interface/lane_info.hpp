...
class LaneInfo {
 public:
  ...
  inline void set_lane_points(
      const std::vector<legion::interface::LanePoint>& lane_points) {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    lane_points_ = lane_points;
  }

  inline void set_lane_points(const uint32_t index,
                              legion::interface::LanePoint& lane_points) {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    lane_points_[index] = lane_points;
  }

  inline void add_lane_points(const legion::interface::LanePoint& lane_points) {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    lane_points_.emplace_back(lane_points);
  }

  inline const legion::interface::LanePoint& lane_points(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    return lane_points_[index];
  }

  inline std::vector<legion::interface::LanePoint>* mutable_lane_points() {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    return &lane_points_;
  }

  inline void lane_points(
      std::vector<legion::interface::LanePoint>& lane_points) const {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    lane_points.assign(lane_points_.begin(), lane_points_.end());
  }

  inline const std::vector<legion::interface::LanePoint>& lane_points() const {
    std::lock_guard<std::mutex> lock(*lane_points_mutex_);
    return lane_points_;
  }
 ...
 protected:
  ...
  //道路参考线（z）
  std::vector<legion::interface::LanePoint> lane_points_;
};
}  // namespace interface
}  // namespace legion
