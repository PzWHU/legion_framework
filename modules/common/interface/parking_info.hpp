...
class ParkingInfo {
 public:
  ParkingInfo() {
    ...
    parking_status_ = legion::common::ParkingStatus::PARKING_ENABLE;
    confidence_ = 0.0;
    theta_ = 0.0;
    width_ = 0.0;
    length_ = 0.0;
    yaw_offset_ = 0.0;
    sensor_id_ = legion::common::SensorID::CAMERA_FRONT_CENTER;
    is_lane_width_valid_ = false;
    lane_width_ = 0.0;
    clear_parking_stoppers();
    parking_direction_type_ = legion::common::Direction::DIR_INVALID;
    left_occupied_status_ =
        legion::common::OccupiedStatus::UNKNOWN_OCCUPIED_STATUS;
    right_occupied_status_ =
        legion::common::OccupiedStatus::UNKNOWN_OCCUPIED_STATUS;
    parking_source_type_ = legion::common::ParkingSourceType::LINE_PARKING;
  }
  ~ParkingInfo() = default;
  ...
  inline void set_parking_status(
      const legion::common::ParkingStatus& parking_status) {
    parking_status_ = parking_status;
    parking_status_ptr_ = &parking_status_;
  }

  inline const legion::common::ParkingStatus& parking_status() const {
    return parking_status_;
  }

  inline legion::common::ParkingStatus* mutable_parking_status() {
    return &parking_status_;
  }
  ...
  inline void set_center_point_of_parking(
      const legion::interface::Point3D& center_point_of_parking) {
    center_point_of_parking_ = center_point_of_parking;
    center_point_of_parking_ptr_ = &center_point_of_parking_;
  }

  inline const legion::interface::Point3D& center_point_of_parking() const {
    return center_point_of_parking_;
  }

  inline legion::interface::Point3D* mutable_center_point_of_parking() {
    return &center_point_of_parking_;
  }
  ...
  inline void set_polygon(const legion::interface::Polygon3D& polygon) {
    polygon_ = polygon;
    polygon_ptr_ = &polygon_;
  }

  inline const legion::interface::Polygon3D& polygon() const {
    return polygon_;
  }

  inline legion::interface::Polygon3D* mutable_polygon() { return &polygon_; }

  inline bool has_polygon() { return (polygon_ptr_ != nullptr); }

  inline void set_sensor_id(const legion::common::SensorID& sensor_id) {
    sensor_id_ = sensor_id;
    sensor_id_ptr_ = &sensor_id_;
  }

  inline const legion::common::SensorID& sensor_id() const {
    return sensor_id_;
  }

  inline legion::common::SensorID* mutable_sensor_id() { return &sensor_id_; }
  ...
  inline void set_parking_stoppers(
      const std::vector<legion::interface::ParkingStopper>& parking_stoppers) {
    std::lock_guard<std::mutex> lock(*parking_stoppers_mutex_);
  ...
  inline void set_parking_stoppers(
      ...
      legion::interface::ParkingStopper& parking_stoppers) {
    std::lock_guard<std::mutex> lock(*parking_stoppers_mutex_);
    parking_stoppers_[index] = parking_stoppers;
  }

  inline void add_parking_stoppers(
      const legion::interface::ParkingStopper& parking_stoppers) {
    std::lock_guard<std::mutex> lock(*parking_stoppers_mutex_);
    parking_stoppers_.emplace_back(parking_stoppers);
  }

  inline const legion::interface::ParkingStopper& parking_stoppers(
      uint32_t index) const {
    std::lock_guard<std::mutex> lock(*parking_stoppers_mutex_);
    return parking_stoppers_[index];
  }

  inline std::vector<legion::interface::ParkingStopper>*
  ...
  inline void parking_stoppers(
      std::vector<legion::interface::ParkingStopper>& parking_stoppers) const {
    std::lock_guard<std::mutex> lock(*parking_stoppers_mutex_);
    parking_stoppers.assign(parking_stoppers_.begin(), parking_stoppers_.end());
  }

  inline const std::vector<legion::interface::ParkingStopper>&
  ...
  inline void set_parking_direction_type(
      const legion::common::Direction& parking_direction_type) {
    parking_direction_type_ = parking_direction_type;
    parking_direction_type_ptr_ = &parking_direction_type_;
  }

  inline const legion::common::Direction& parking_direction_type() const {
    return parking_direction_type_;
  }

  inline legion::common::Direction* mutable_parking_direction_type() {
    return &parking_direction_type_;
  }
  ...
  inline void set_left_occupied_status(
      const legion::common::OccupiedStatus& left_occupied_status) {
    left_occupied_status_ = left_occupied_status;
    left_occupied_status_ptr_ = &left_occupied_status_;
  }

  inline const legion::common::OccupiedStatus& left_occupied_status() const {
    return left_occupied_status_;
  }

  inline legion::common::OccupiedStatus* mutable_left_occupied_status() {
    return &left_occupied_status_;
  }
  ...
  inline void set_right_occupied_status(
      const legion::common::OccupiedStatus& right_occupied_status) {
    right_occupied_status_ = right_occupied_status;
    right_occupied_status_ptr_ = &right_occupied_status_;
  }

  inline const legion::common::OccupiedStatus& right_occupied_status() const {
    return right_occupied_status_;
  }

  inline legion::common::OccupiedStatus* mutable_right_occupied_status() {
    return &right_occupied_status_;
  }
  ...
  inline void set_parking_source_type(
      const legion::common::ParkingSourceType& parking_source_type) {
    parking_source_type_ = parking_source_type;
    parking_source_type_ptr_ = &parking_source_type_;
  }

  inline const legion::common::ParkingSourceType& parking_source_type() const {
    return parking_source_type_;
  }

  inline legion::common::ParkingSourceType* mutable_parking_source_type() {
    return &parking_source_type_;
  }
 ...
 protected:
  ...
  legion::common::ParkingStatus parking_status_;
  legion::common::ParkingStatus* parking_status_ptr_ = nullptr;
  //车位置信度
  double confidence_;
  double* confidence_ptr_ = nullptr;
  //世界坐标系下停车位中心点
  legion::interface::Point3D center_point_of_parking_;
  legion::interface::Point3D* center_point_of_parking_ptr_ = nullptr;
  ...
  //世界坐标系下的车位多边形
  legion::interface::Polygon3D polygon_;
  legion::interface::Polygon3D* polygon_ptr_ = nullptr;
  //车位数据来自相机id 0-front_center相机 1-front_left相机 2-front_right相机
  //3-left_front相机 4-left_back右后相机 5-right_front相机 6-right_back相机
  //7-back相机 8-相机融合
  legion::common::SensorID sensor_id_;
  legion::common::SensorID* sensor_id_ptr_ = nullptr;
  //车道宽度是否有效
  ...
  //车位内部的限位器数组
  std::vector<legion::interface::ParkingStopper> parking_stoppers_;
  //  DIR_INVALID = 0,   LEFT = 1,   UP = 2,   RIGHT = 3,   DOWN = 4,
  legion::common::Direction parking_direction_type_;
  legion::common::Direction* parking_direction_type_ptr_ = nullptr;
  // 0-unknown 未知，1-empty 空闲，2-occupied 被占用
  legion::common::OccupiedStatus left_occupied_status_;
  legion::common::OccupiedStatus* left_occupied_status_ptr_ = nullptr;
  // 0-unknown 未知，1-empty 空闲，2-occupied 被占用
  legion::common::OccupiedStatus right_occupied_status_;
  legion::common::OccupiedStatus* right_occupied_status_ptr_ = nullptr;
  // 0-线车位， 1-空间车位， 2-融合车位
  legion::common::ParkingSourceType parking_source_type_;
  legion::common::ParkingSourceType* parking_source_type_ptr_ = nullptr;
};
}  // namespace interface
}  // namespace legion
