...
Polygon2d::Polygon2d(const double &cx, const double &cy, const double &yaw,
                     const double &width, const double &length) {
  points_.clear();

  Vec2d p_front_center, p_back_center, tmp_point;

  p_front_center.set_x(cx + 0.5 * length * cos(yaw));
  p_front_center.set_y(cy + 0.5 * length * sin(yaw));

  p_back_center.set_x(cx - 0.5 * length * cos(yaw));
  p_back_center.set_y(cy - 0.5 * length * sin(yaw));

  // front left
  tmp_point.set_x(p_front_center.x() - 0.5 * width * sin(yaw));
  tmp_point.set_y(p_front_center.y() + 0.5 * width * cos(yaw));
  points_.push_back(tmp_point);
  ...
  // back right
  tmp_point.set_x(p_back_center.x() + 0.5 * width * sin(yaw));
  tmp_point.set_y(p_back_center.y() - 0.5 * width * cos(yaw));
  points_.push_back(tmp_point);
...
double Polygon2d::DistanceTo(const Vec2d &point) const {
  ...
  if (points_.size() < 3) {
    AERROR << "points_.size() < 3: " << points_.size();
    return -1;
  }
...
double Polygon2d::DistanceTo(const Polygon2d &polygon) const {
  ...
  if (points_.size() < 3 || polygon.num_points() < 3) {
    AERROR << "points_.size() < 3: " << points_.size();
    return -1;
  }
...
bool Polygon2d::HasOverlap(const Polygon2d &polygon) const {
  ...
  // std::cout << "collision-------check-------obs points num----: " <<
...
bool Polygon2d::ComputeOverlap(const Polygon2d &other_polygon,
  ...
  if (points_.size() < 3 || !is_convex_ || !other_polygon.is_convex() ||
      overlap_polygon == nullptr)
    return false;
...
bool Polygon2d::HasOverlap(const LineSegment2d &line_segment) const {
  ////CHECK_GE(points_.size(), 3);
  if (points_.size() < 3) {
    AERROR << "points size error." << points_.size();
    return false;
  }
...
bool Polygon2d::GetOverlap(const LineSegment2d &line_segment,
  ...
  ////CHECK_NOTNULL(first);
  ////CHECK_NOTNULL(last);
  if (points_.size() < 3 || first == nullptr || last == nullptr) return false;
...
void Polygon2d::ExtremePoints(const double heading, Vec2d *const first,
  ...
  ////CHECK_NOTNULL(first);
  ////CHECK_NOTNULL(last);
  if (points_.size() < 3 || first == nullptr || last == nullptr) return;
