...
/**
 * @namespace legion::common::math
 * @brief legion::common::math
 */
namespace legion {
namespace common {
namespace math {
...
public:
   ...
   */
  void
  Predict(const Eigen::Matrix<T, UN, 1>& u = Eigen::Matrix<T, UN, 1>::Zero());
...
template <typename T, unsigned int XN, unsigned int ZN, unsigned int UN>
inline void
KalmanFilter<T, XN, ZN, UN>::Predict(const Eigen::Matrix<T, UN, 1>& u) {
...
template <typename T, unsigned int XN, unsigned int ZN, unsigned int UN>
inline void
KalmanFilter<T, XN, ZN, UN>::Correct(const Eigen::Matrix<T, ZN, 1>& z) {
  CHECK(is_initialized_);
  y_ = z - H_ * x_;
