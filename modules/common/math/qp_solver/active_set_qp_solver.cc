...
ActiveSetQpSolver::ActiveSetQpSolver(
    ...
    : QpSolver(kernel_matrix, offset, affine_inequality_matrix,
      ...
      qp_eps_num_(default_active_set_eps_num_),
      qp_eps_den_(default_active_set_eps_den_),
      qp_eps_iter_ref_(default_active_set_eps_iter_ref_),
      debug_info_(default_enable_active_set_debug_info_) {}
...
bool ActiveSetQpSolver::Solve() {
  ...
  //TODO(duanchengwen) 自定义添加,需检验是否正确
  if(index != kernel_matrix_.rows() * kernel_matrix_.cols())
  {
    AERROR << "index != kernel_matrix_.rows() * kernel_matrix_.cols()";
    return false;
  }
  ...
  if(index != affine_equality_matrix_.rows() * num_param_)
  {
    AERROR << "index != affine_equality_matrix_.rows() * num_param_";
    return false;
  }
  ...
  {
    AERROR << "index != affine_equality_matrix_.rows() * num_param_ + affine_inequality_boundary_.rows() * num_param_";
    return false;
  }
