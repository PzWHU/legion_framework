...
class State {
 ...
 public:
  State(std::string _state_name, uint64_t _state_id,
      ...
      : child_state_(NULL),
        state_name_(_state_name),
        state_id_(_state_id),
        parent_state_(_parent_state),
        entered_key_("") {
#if 0
    CallbackUpdateFunc =
  ...
  void OnEntry(void) {
    ...
    if (CallbackEntryFunc) CallbackEntryFunc(state_name_);
  }
  void OnExit(void) {
    // ADEBUG << "[" << state_name_ << ":Exit]";
    if (child_state_) child_state_->OnExit();

    SetChild(nullptr);

    if (CallbackExitFunc) CallbackExitFunc(state_name_);
  }
  ...
  void ShowStateName(void) { AINFO << state_name_ << "-"; }
