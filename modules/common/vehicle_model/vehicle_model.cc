...
namespace legion {
namespace common {
json VehicleModel::vehicle_model_json_;
...
VehicleState VehicleModel::Predict(const double predicted_time_horizon,
  ...
  VehicleModelConf vehicle_model_config;
  ...
  std::ifstream in(FLAGS_vehicle_model_config_filename);
  AINFO << "file_path: " << FLAGS_vehicle_model_config_filename << std::endl;
  in >> vehicle_model_json_;
  //   if (vehicle_model_json_.is_null()) {
  //   AERROR << "vehicle_model_json_ is null";
  //   return false;
  // }
  VehicleModelConf::ModelType model_type = (VehicleModelConf::ModelType)vehicle_model_json_["model_type"];
  vehicle_model_config.set_model_type(model_type);
  RearCenteredKinematicBicycleModelConf rc_kinematic_bicycle_model;
  double dt = vehicle_model_json_["rc_kinematic_bicycle_model"]["dt"];
  rc_kinematic_bicycle_model.set_dt(dt);
  vehicle_model_config.set_rc_kinematic_bicycle_model(rc_kinematic_bicycle_model);

  // Some models not supported for now
  ACHECK(vehicle_model_config.model_type() !=
         VehicleModelConf::COM_CENTERED_DYNAMIC_BICYCLE_MODEL);
  ACHECK(vehicle_model_config.model_type() != VehicleModelConf::MLP_MODEL);

  VehicleState predicted_vehicle_state;
  if (vehicle_model_config.model_type() ==
      VehicleModelConf::REAR_CENTERED_KINEMATIC_BICYCLE_MODEL) {
