...
class TimeTool {
 public:
  ...
  static legion::interface::Time Now2TmeStruct() {
  ...
  static int64_t TimeStruct2Ms(legion::interface::Time time) {
    int64_t ms;
    ms = time.sec();
    ms *= 1000;
  ...
  static int64_t TimeStruct2Us(legion::interface::Time time) {
    int64_t us;
    us = time.sec();
    us *= 1000000;
    us += time.nsec() / 1000;
  ...
  static int64_t TimeStruct2Ns(legion::interface::Time time) {
    int64_t us;
    us = time.sec();
    us *= 1000000000;
    us += time.nsec();
  ...
  static double TimeStruct2S(legion::interface::Time time) {
    double s;
    s = (double)time.sec();
  ...
  static double GetTimeDiffNow(legion::interface::Time time) {
    return NowToSeconds() - TimeStruct2S(time);
  }

  static double GetTimeDiffNow(double time) { return NowToSeconds() - time; }

  static double GetTimeDiff(legion::interface::Time s,
                            legion::interface::Time e) {
    return (TimeStruct2S(e) - TimeStruct2S(s));
  }

  static double GetTimeDiff(double s, double e) { return (e - s); }

  static legion::interface::Time Second2Time(double s) {
    legion::interface::Time t;
    int int_sec = (int)s;
