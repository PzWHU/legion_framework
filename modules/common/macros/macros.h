...
// 周期性状态消息（Chassis, Location, Vehicle, WheelInfo等）
// 高频率、多订阅方、需要实时性
#define ROS2_QOS_PERIODIC_STATUS() \
    rclcpp::QoS(rclcpp::KeepLast(10)).reliable().durability_volatile()

// 关键控制指令（ControlCommand, PlanningCmd, VehicleTask等）
// 必须可靠传输
#define ROS2_QOS_CRITICAL_CMD() \
    rclcpp::QoS(rclcpp::KeepLast(10)).reliable().durability_volatile()

// 触发/一次性消息（RoutingResponse, TrafficEvents等）
// 低频、必须可靠、可能只发一次，需要 transient_local 确保晚启动订阅者能收到
#define ROS2_QOS_TRIGGER() \
    rclcpp::QoS(rclcpp::KeepLast(5)).reliable().transient_local()

// 诊断/调试消息（Faults, Events, Analysis等）
// 需要buffer支持录包
#define ROS2_QOS_DIAGNOSTIC() \
    rclcpp::QoS(rclcpp::KeepLast(10)).reliable().durability_volatile()

// 导航信息消息（GuideInfo, NaviInfoMsg等）
// 中等频率
#define ROS2_QOS_NAVIGATION() \
    rclcpp::QoS(rclcpp::KeepLast(5)).reliable().durability_volatile()

// 大数据量消息（ADCTrajectory, PredictionObstacles等）
// 允许适度丢帧，降低CPU开销
#define ROS2_QOS_LARGE_DATA() \
    rclcpp::QoS(rclcpp::KeepLast(5)).reliable().durability_volatile()

// 感知消息（TrafficLightMsg, DrivableRegion等）
#define ROS2_QOS_PERCEPTION() \
    rclcpp::QoS(rclcpp::KeepLast(10)).reliable().durability_volatile()
...
#define LOGGING_INIT(conf_ptr)                                              \
  ...
  logging_conf.log_level = (legion::common::LogLevel)conf_ptr->log_level(); \
...
#define THIS_FAULTCODESET legion::interface::SafeFaultCodeSet
  ...
  for (int k = 0; k < module_list_size; k++) {                               \
    ...
    for (int i = 0; i < fault_code_list_size; i++) {                         \
      ...
      for (int j = 0; j < type_list_size; j++) {                             \
        ...
        legion::interface::FaultProperty faulttype;                          \
...
#else
#define THIS_FAULTCODESET legion::interface::FaultCodeSet
    ...
    if (fault_enable == true) {                                             \
      ...
      for (uint32_t i = 0; i < fault_code_list_size; i++) {                 \
        ...
        for (uint32_t j = 0; j < type_list_size; j++) {                     \
          ...
          faulttype.set_enable(enable);                                     \
          switch (type) {                                                   \
            case legion::interface::FAULT_TYPE::DATA_TIMEOUT:               \
            case legion::interface::FAULT_TYPE::DATA_INTERRUPT:             \
            case legion::interface::FAULT_TYPE::TIME_ERROR:                 \
            case legion::interface::FAULT_TYPE::FEEDBACK_ABNORMAL:          \
            case legion::interface::FAULT_TYPE::CALCULATE_TIMEOUT:          \
              faulttype.set_timeout(FAULT(k, i, j)["timeout"]);             \
              break;                                                        \
            case legion::interface::FAULT_TYPE::DATA_LOST:                  \
              faulttype.set_max_count(FAULT(k, i, j)["max_count"]);         \
              break;                                                        \
            case legion::interface::FAULT_TYPE::DATA_ABNORMAL:              \
          ...
          auto fault_object =                                               \
              legion::interface::FaultClientFactory::RegisterFaultClients(  \
...
#define MESSAGE_HEADER_PARSER(obj)                 \
  ...
  header.set_seq(seq);                             \
  legion::interface::Time interface_time;          \
...
#define DDS_FAULTS_PARSER(param, obj)                        \
  ...
  std::vector<legion::interface::Fault> interface_faults;    \
...
#define FAULTS_RECEIVE(obj)                          \
  legion::interface::Fault fault_temp;               \
  std::vector<legion::interface::Fault> faults_temp; \
