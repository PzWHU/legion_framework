...
template <typename T> void RosMessageManager<T>::Init(T* t) {
  ...
  control_command_pub_ = nh_.advertise<::ros_interface::ControlCommand>(
      "/control/ControlCommand", 10);

  control_analysis_pub_ = nh_.advertise<::ros_interface::ControlAnalysis>(
      "/control/ControlAnalysis", 10);

  faults_pub_ = nh_.advertise<::ros_interface::Faults>("/control/Faults", 10);

  events_pub_ = nh_.advertise<::ros_interface::Events>("/control/Events", 10);
...
void RosMessageManager<T>::TaskStart()
{
  ...
  planning_cmd_sub_ =
      nh_.subscribe("/planning/PlanningCmd", 30,
                    &RosMessageManager::HandlePlanningCmdMessage, this);
...
template <typename T>
bool RosMessageManager<T>::DeActivate()
{
    std::unique_lock<std::mutex> lock(mode_mutex_);
    if(is_active_==false)
    {
...
template <typename T>
void RosMessageManager<T>::PublishControlCommand(
    legion::interface::ControlCommand msg) {
  if (is_init_ == false)
    return;
  ::ros_interface::ControlCommand control_command;
  MESSAGE_HEADER_ASSIGN(std_msgs, control_command)
...
template <typename T>
void RosMessageManager<T>::PublishControlAnalysis(
    legion::interface::ControlAnalysis msg) {
  if (is_init_ == false)
    return;
  ::ros_interface::ControlAnalysis control_analysis;
  MESSAGE_HEADER_ASSIGN(std_msgs, control_analysis)
  ...
  control_analysis_pub_.publish(control_analysis);
}
...
template <typename T>
void RosMessageManager<T>::HandlePlanningCmdMessage(
    const ros_interface::PlanningCmd& msg_obj) {
  if (is_active_ == false)
    return;
  const ros_interface::PlanningCmd* msg_obj_ptr = &msg_obj;
  ros_interface::PlanningCmd* msg =
      const_cast<ros_interface::PlanningCmd*>(msg_obj_ptr);

  legion::interface::PlanningCmd planning_cmd;
  ...
  planning_cmd.set_horn_ctrl((legion::common::SwitchStatus)msg->horn_ctrl);
  ...
  planning_cmd.set_lf_door_ctrl((legion::common::DoorStatus)msg->lf_door_ctrl);
  planning_cmd.set_rf_door_ctrl((legion::common::DoorStatus)msg->rf_door_ctrl);
  planning_cmd.set_lr_door_ctrl((legion::common::DoorStatus)msg->lr_door_ctrl);
  planning_cmd.set_rr_door_ctrl((legion::common::DoorStatus)msg->rr_door_ctrl);
...
template <typename T>
void RosMessageManager<T>::RegisterLoop()
{
