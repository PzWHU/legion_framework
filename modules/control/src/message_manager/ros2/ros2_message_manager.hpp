...
using ::ros2_interface::msg::ControlAnalysis;
...
template <typename T>
Ros2MessageManager<T>::Ros2MessageManager() : Node{"control"} {}
template <typename T> void Ros2MessageManager<T>::Init(T* t) {
  ...
  control_analysis_pub_ =
      create_publisher<::ros2_interface::msg::ControlAnalysis>(
  ...
  planning_cmd_sub_ = create_subscription<PlanningCmd>(
      "/planning/PlanningCmd", 
      ROS2_QOS_CRITICAL_CMD(),
      [this](const ros2_interface::msg::PlanningCmd::SharedPtr msg) {
        Ros2MessageManager::HandlePlanningCmdMessage(msg);
      },
      SubAllocT{});
...
template <typename T>
void Ros2MessageManager<T>::PublishControlCommand(
...
template <typename T>
void Ros2MessageManager<T>::PublishControlAnalysis(
    legion::interface::ControlAnalysis msg) {
  if (is_init_ == false)
    return;
  ::ros2_interface::msg::ControlAnalysis control_analysis;
  MESSAGE_HEADER_ROS2_ASSIGN(std_msgs::msg, control_analysis)
...
template <typename T>
void Ros2MessageManager<T>::HandlePlanningCmdMessage(
    const ros2_interface::msg::PlanningCmd::SharedPtr msg_obj_ptr) {
  if (is_init_ == false)
    return;
  std::shared_ptr<ros2_interface::msg::PlanningCmd> msg =
      std::const_pointer_cast<ros2_interface::msg::PlanningCmd>(msg_obj_ptr);

  legion::interface::PlanningCmd planning_cmd;
  MESSAGE_HEADER_ROS2_PARSER(planning_cmd)
