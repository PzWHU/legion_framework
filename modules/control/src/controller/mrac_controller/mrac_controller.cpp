...
double MracController::Control(const double command, const Matrix state,
  ...
  if (!reference_model_enabled_ || !adaption_model_enabled_) {
    ADEBUG
        ...
        << "; The adaption_model building status: " << adaption_model_enabled_;
...
Status MracController::SetReferenceModel(const MracConf &mrac_conf) {
  ...
  if (((mrac_conf.reference_time_constant() < Epsilon && model_order_ == 1)) ||
    ...
    sstream
        ...
        << model_order_;
    AERROR<<sstream.str();
    return Status(Status::ErrorCode::CONTROL_INIT_ERROR, sstream.str());
  }
...
Status MracController::SetAdaptionModel(const MracConf &mrac_conf) {
  const int p_size = mrac_conf.adaption_matrix_p().size();
  const int x_size = mrac_conf.adaption_state_gain().size();
  const int aw_size = mrac_conf.anti_windup_compensation_gain().size();
  if (p_size != model_order_ * model_order_ || x_size > model_order_ ||
      aw_size > model_order_) {
    stringstream sstream;
    sstream << "mrac controller error: adaption matrix p element number: "
...
void MracController::BuildReferenceModel() {
  ...
  if (ts_ <= 0.0) {
    ...
    if (model_order_ == 1) {
      ...
      ADEBUG << "reference model order beyond the designed range, "
                 "model_order: "
              << model_order_;
      reference_model_enabled_ = false;
    }
...
void MracController::BuildAdaptionModel() {
  ...
  if (model_order_ <= 2) {
    ...
    if (!CheckLyapunovPD(matrix_a_reference_, matrix_p_adaption_)) {
      ADEBUG << "Solution of the algebraic Lyapunov equation is not symmetric "
                 "positive definite";
      adaption_model_enabled_ = false;
    }
  } else {
    ADEBUG << "Adaption model order beyond the designed range, "
               "model_order: "
            << model_order_;
    adaption_model_enabled_ = false;
  }
...
void MracController::SetInitialReferenceState(
  ...
  if (state_reference_init.rows() != model_order_ ||
    ...
    ADEBUG << "failed to set the initial reference states, due to the given "
               "state size: "
...
void MracController::SetInitialActionState(const Matrix state_action_init) {
  if (state_action_init.rows() != model_order_ ||
    ...
    ADEBUG << "failed to set the initial action states, due to the given "
               "state size: "
