...
namespace legion 
{
  namespace control 
  {
    legion::interface::ControlCommand *lon_speed_cmd_;
    ...
    void LonSpeedController::GearTransition(legion::interface::ControlCommand *cmd) 
    {
      if (cmd == nullptr) 
      {
      ...
      if (gear_sm_debug_.is_stopped() == true &&gear_sm_debug_.enable_epb_applied() == true &&
          (gear_sm_debug_.tar_gear() ==legion::common::GearPosition::GEAR_NEUTRAL || gear_sm_debug_.tar_gear() == legion::common::GearPosition::GEAR_PARKING)) 
      {
      ...
      else 
      {
        ...
        //       legion::common::GearPosition::GEAR_REVERSE)   //EV350给刹车的时候无法挂档问题
        epb_sm_->NextState("release");
      }

      //目标档位和当前档位不一致  并且当前EPB处于非拉起状态  设置刹车值
      if (gear_sm_debug_.tar_gear() != gear_sm_debug_.cur_gear() && gear_sm_debug_.cur_epb_state() != legion::common::EPBLevel::APPLIED) 
      {
      ...
      if ((gear_sm_debug_.tar_gear() == gear_sm_debug_.cur_gear() &&
          (gear_sm_debug_.tar_gear() == legion::common::GearPosition::GEAR_NEUTRAL ||
            gear_sm_debug_.tar_gear() == legion::common::GearPosition::GEAR_PARKING)) &&
          gear_sm_debug_.cur_epb_state() != legion::common::EPBLevel::APPLIED) 
      {
        brake_sm_debug_.set_is_geartransition_stop(true);
        // cmd->set_brake_value(gear_sm_debug_.brake_value_when_gear_transitioning());
      }
    ...
    void LonSpeedController::GearNEntry(const std::string &state_name, int state) 
    {
      ...
      lon_speed_cmd_->set_gear_location(legion::common::GearPosition::GEAR_NEUTRAL);
    }
    ...
    void LonSpeedController::GearNUpdate(const std::string &state_name, int state) 
    {
      ...
      if (gear_sm_debug_.cur_gear() == legion::common::GearPosition::GEAR_NEUTRAL) 
      {
        if (gear_sm_debug_.tar_gear() == legion::common::GearPosition::GEAR_DRIVE) 
        {
          gear_sm_->NextState("D");
        } 
        else if (gear_sm_debug_.tar_gear() == legion::common::GearPosition::GEAR_REVERSE) 
        {
    ...
    //   lon_speed_cmd_->set_gear_location(legion::common::GearPosition::GEAR_PARKING);
    // }
    ...
    //   if (gear_sm_debug_.cur_gear() == legion::common::GearPosition::GEAR_PARKING) 
    //   {
    //     if (gear_sm_debug_.tar_gear() == legion::common::GearPosition::GEAR_DRIVE) 
    //     {
    //       gear_sm_->NextState("D");
    ...
    void LonSpeedController::GearDEntry(const std::string &state_name, int state) {
      ...
      lon_speed_cmd_->set_gear_location(legion::common::GearPosition::GEAR_DRIVE);
    }
    ...
    void LonSpeedController::GearREntry(const std::string &state_name, int state) {
      ...
      lon_speed_cmd_->set_gear_location(legion::common::GearPosition::GEAR_REVERSE);
    }
    ...
    void LonSpeedController::GearRUpdate(const std::string &state_name, int state) 
    {
      ...
      if (gear_sm_debug_.tar_gear() != legion::common::GearPosition::GEAR_REVERSE &&
          gear_sm_debug_.is_stopped() == true)
        gear_sm_->NextState("N");
    }
    ...
    void LonSpeedController::EPBAppliedEntry(const std::string &state_name, int state) {
      lon_speed_cmd_->set_epb_level(legion::common::EPBLevel::APPLIED);
    }
    ...
    void LonSpeedController::EPBReleaseEntry(const std::string &state_name, int state) {
      lon_speed_cmd_->set_epb_level(legion::common::EPBLevel::RELEASED);
    }
