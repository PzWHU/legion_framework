...
using namespace legion::control;
...
class PurePursuitController : public Controller {
 ...
 private:
   ...
   * @param[in]  chassis          底盘信息
   */
  void ProcessLogs(const legion::interface::ControlCommand *cmd,
  ...
  struct TargetPointInfo {
    legion::common::math::Vec2d position;  // 目标点坐标
   ...
   */
  legion::interface::TrajectoryPoint ComputeTargetPoint(
   ...
   * @return double 距离
   */
  double ComputePointsDistanceSquared(const legion::common::math::Vec2d pos1,
                                      const legion::common::math::Vec2d pos2);
  /**
   * @brief 计算2点相对1点，x、y位移距离
   * @param[in]  pos1             1点坐标
   * @param[in]  pos2             2点坐标
   * @return legion::common::math::Vec2d x、y位移距离
   */
  legion::common::math::Vec2d ComputeRelativeXYOffset(
