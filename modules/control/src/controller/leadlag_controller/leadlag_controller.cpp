/******************************************************************************
 * Copyright 2019 The Legion Authors. All Rights Reserved.
 *
...
double LeadlagController::Control(const double error, const double dt) {
  ...
  if (dt <= 0.0) {
    AERROR << "dt <= 0, will use the last output, dt: " << dt;
...
void LeadlagController::TransformC2d(const double dt) {
  if (dt <= 0.0) {
    AERROR << "dt <= 0, continuous-discrete transformation failed, dt: "
              << dt;
    ...
    kd0_ = Ts_ * a0 - 2 * a1;
    if (kd1_ <= 0.0) {
      AERROR << "kd1 <= 0, continuous-discrete transformation failed, kd1: "
                << kd1_;
      transfromc2d_enabled_ = false;
    } else {
