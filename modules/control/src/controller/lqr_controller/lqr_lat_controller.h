...
class LQRLatController : public Controller {
 ...
 private:
   ...
   * @param[in]  chassis          底盘信息
   */
  void UpdateMatrix(const legion::interface::Chassis *chassis);
  /**
   * @brief 计算前馈
  ...
  double ComputeFeedForward(double ref_curvature,
                            const legion::interface::Chassis *chassis) const;
  /**
   * @brief 更新车辆状态
   ...
   * @param[out]  debug            诊断调试信息
   */
  void UpdateState(const legion::interface::ADCTrajectory *trajectory,
                   const legion::interface::LocalizationEstimate *localization,
                   const legion::interface::Chassis *chassis,
                   SimpleLateralDebug *debug);
  /**
