...
namespace legion
{
  namespace control
  {
    ...
    legion::common::Status LQRLatController::Init(
    ...
    void LQRLatController::UpdateState(
        ...
        const legion::interface::Chassis *chassis, SimpleLateralDebug *debug)
    {
      ...
      else
      {
        ...
        else
        {
          ...
          case 0:
            add_lr_ = lr_;
            if (vehicle_state->gear() == legion::common::GEAR_REVERSE)
            {
              state_machine = 1;
            }
            else
          ...
          case 1:
            add_lr_ = 0.0;
            if (vehicle_state->gear() == legion::common::GEAR_DRIVE)
            {
              state_machine = 0;
            }
            else
      ...
      {
        // std::cout<<"preview_window_: "<<preview_window_<<std::endl;
    ...
    {
      auto vehicle_state = injector_->vehicle_state();
      legion::interface::TrajectoryPoint target_point;
      ...
      if (vehicle_state->gear() == legion::common::GearPosition::GEAR_REVERSE)
      {
    ...
    void LQRLatController::UpdateMatrix(const legion::interface::Chassis *chassis)
    {
      double v;
      // At reverse driving, replace the lateral translational motion dynamics with
      // the corresponding kinematic models
      if (injector_->vehicle_state()->gear() == legion::common::GEAR_REVERSE)
      {
    ...
    double LQRLatController::ComputeFeedForward(
        double ref_curvature, const legion::interface::Chassis *chassis) const
    {
    ...
    // TODO  主要计算当前的
    legion::common::Status LQRLatController::ComputeControlCommand(
        const legion::interface::LocalizationEstimate *localization,
        const legion::interface::Chassis *chassis,
        const legion::interface::ADCTrajectory *trajectory,
    ...
    {
      int64_t current_time = legion::common::TimeTool::Now2Us();
      auto vehicle_state = injector_->vehicle_state();
      ...
      // int64_t time_diff_5 = legion::common::TimeTool::Now2Us() - current_time;
      //         std::cout << "time_diff5555:::" << time_diff_5 << endl;
      // 轨迹分析器更新轨迹
      trajectory_analyzer_ =
          std::move(TrajectoryAnalyzer(&target_tracking_trajectory));
      // int64_t time_diff_4 = legion::common::TimeTool::Now2Us() - current_time;
      ...
      if (((trajectory_transform_to_com_reverse_ &&
            vehicle_state->gear() == legion::common::GEAR_REVERSE) ||
           (trajectory_transform_to_com_drive_ &&
            vehicle_state->gear() == legion::common::GEAR_DRIVE)) &&
          enable_look_ahead_back_control_)
      {
      ...
      // int64_t time_diff_3 = legion::common::TimeTool::Now2Us() - current_time;
      ...
      // std::cout << "front_steer_angle_feedforward:" << front_steer_angle_feedforward << std::endl;
      // std::cout << "matrix_state_: " << std::endl
      //           << matrix_state_ << std::endl;
      // std::cout << "matrix_k_: " << std::endl
      //           << matrix_k_ << std::endl;
      // std::cout << "-(matrix_k_ * matrix_state_)(0, 0) * 180 / M_PI: " << std::endl
      //           << -(matrix_k_ * matrix_state_)(0, 0) * 180 / M_PI << std::endl;
      ...
      /* if (std::abs(vehicle_state->linear_velocity()) < lock_steer_speed_ &&
          (vehicle_state->gear() == legion::common::GearPosition::GEAR_DRIVE ||
           vehicle_state->gear() == legion::common::GearPosition::GEAR_REVERSE) &&
          chassis->steer_driving_mode() ==
              legion::common::DrivingMode::COMPLETE_AUTO_DRIVE)
      {
        front_steer_angle = pre_front_steer_angle_;
      } */

      if (cmd->steer_driving_mode() == legion::common::DrivingMode::COMPLETE_MANUAL ||
          trajectory->behaviour_lat_state() ==
              legion::interface::ADCTrajectory::BehaviourLatState::PARKING_FINISH_STATE /*||
          trajectory->behaviour_lat_state() == 
              legion::interface::ADCTrajectory::BehaviourLatState::FINISH_STATE*/)
      {
      ...
      if (steer_mode_ == VehicleParam::STEER_MODE_4WS)
      {
        // std::cout << "-(matrix_k_ * matrix_state_)(1, 0) * 180 / M_PI: " << std::endl
        //           << -(matrix_k_ * matrix_state_)(1, 0) * 180 / M_PI << std::endl;
      ...
      if(vehicle_state->gear() == legion::common::GearPosition::GEAR_NEUTRAL)
      {
      ...
      {
      case 0:
        if(trajectory->driving_mode() == legion::common::DrivingMode::COMPLETE_MANUAL &&
        trajectory->behaviour_lat_state() == legion::interface::ADCTrajectory::BehaviourLatState::PARKING_FINISH_STATE)
        {
          steer_back_step_ = 1;
          cmd->set_front_steering_target(0.0);
          cmd->set_rear_steering_target(0.0);
          cmd->set_steer_driving_mode(legion::common::DrivingMode::COMPLETE_AUTO_DRIVE);
      ...
      case 3:
        ...
        cmd->set_front_steering_target(0.0);
        cmd->set_rear_steering_target(0.0);
        if(trajectory->behaviour_lat_state() != legion::interface::ADCTrajectory::BehaviourLatState::PARKING_FINISH_STATE
           /*&& trajectory->behaviour_lat_state() != legion::interface::ADCTrajectory::BehaviourLatState::INITIAL_STATE*/)
        {
          steer_back_step_ = 0;
        }
        break;
      ...
      lqr_calculate_time_ = legion::common::TimeTool::Now2Us() - current_time;
    ...
    void LQRLatController::ProcessLogs(const legion::interface::ControlCommand *cmd,
                                       ...
                                       const legion::interface::Chassis *chassis)
    {
      ...
      if (enable_log_debug_)
      {
        ...
        // steer_log_file_ << sstream.str() << std::endl;
      }
      //   AWARN<<sstream.str();
    }

    void LQRLatController::SetAnalysis(
        const legion::interface::ControlCommand *cmd,
        const SimpleLateralDebug *debug, const legion::interface::Chassis *chassis,
        legion::interface::ControlAnalysis *analysis)
    {
    ...
    legion::common::Status LQRLatController::Reset() { return Status::Ok(); }
  } // namespace control
} // namespace legion
