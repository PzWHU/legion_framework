...
LonController::LonController() : name_("longitudinal controller") {
  ...
  //   speed_log_file_.is_open() <<std::endl; speed_log_file_ << std::fixed;
...
legion::common::Status LonController::ComputeControlCommand(
  ...
  // if (!(chassis_->gear_location() == legion::common::GearPosition::GEAR_DRIVE
  // ||
  ...
  // std::cout<<"acceleration_cmd01:"<<acceleration_cmd<<std::endl;
  ...
  //           legion::common::GearPosition::GEAR_REVERSE) ||
  //     chassis_->brake_driving_mode() ==
  //         legion::common::DrivingMode::MANUAL_INTERVENTION ||
  //     chassis_->epb_level() != legion::common::EPBLevel::RELEASED) {
  //   acceleration_cmd = 0;
  //   Reset();
  // }
  ...
  // std::cout<<"chassis_->speed_mps(): "<<chassis_->speed_mps()<<std::endl;
  // std::cout<<"acceleration_lookup: "<<acceleration_lookup<<std::endl;
  // if (use_calibration_table_) {
  ...
  //      //
  //      legion::interface::ADCTrajectory::ADCTrajectoryType::ADCTrajectory_TYPE_OBSTACLE)
  //      //     //前方无障碍物
  //      if (trajectory_message_->behaviour_state() ==
  //              legion::interface::ADCTrajectory::BehaviourState::GOAL_STATE
  //              ||
  //          trajectory_message_->adc_trajectory_type() ==
  //              legion::interface::ADCTrajectory::BehaviourState::
  //                  FINISH_STATE ||
  ...
  if (simple_longitudinal_debug_->is_full_stop() &&
      simple_longitudinal_debug_->current_speed() <
          vehicle_param_.max_abs_speed_when_stopped()) {
    gear_sm_debug_.set_tar_gear(legion::common::GearPosition::GEAR_NEUTRAL);
  } else {
  ...
  if (is_stopped_ &&
      gear_sm_debug_.tar_gear() != legion::common::GearPosition::GEAR_DRIVE &&
      gear_sm_debug_.tar_gear() != legion::common::GearPosition::GEAR_REVERSE) {
  ...
  // 档位和EPB切换逻辑
  GearTransition(cmd);
  // std::cout<<"throttle_cmd: "<<cmd->accel_value()<<std::endl;
  // std::cout<<"brake_value: "<<cmd->brake_value()<<std::endl;
  if (gear_sm_) {
    gear_sm_->OnUpdate();
  }
  ...
  if (chassis->epb_level() != legion::common::EPBLevel::RELEASED) {
    cmd->set_accel_value(0.0);
  ...
  if (last_accel_value_ > cmd->accel_value()) {
    cmd->set_brake_lamp_ctrl(legion::common::SwitchStatus::SWITCH_STATUS_ON);
  } else {
    cmd->set_brake_lamp_ctrl(legion::common::SwitchStatus::SWITCH_STATUS_OFF);
  }
  ...
  if (enable_log_debug_ && speed_log_file_.is_open()) {
    ...
    sstream << TimeTool::Now2Ms() << "," << cmd->accel_driving_mode() << ","
            ...
            << ",";
    speed_log_file_ << sstream.str() << std::endl;
    // AWARN << sstream.str();
  }

  SetAnalysis(cmd, simple_longitudinal_debug_.get(), chassis, analysis);

  lon_calculate_time_ = legion::common::TimeTool::Now2Us() - current_time;
...
void LonController::SetAnalysis(const legion::interface::ControlCommand *cmd,
...
void LonController::ComputeLongitudinalErrors(
  ...
  // if (current_point.gear() == legion::common::GearPosition::GEAR_REVERSE)
  // {
...
void LonController::IdleSpeedControlOfOilCar(const double &speed_reference,
  ...
  if (speed_reference < vehicle_speed_in_idle_) {
    ...
    if (chassis_->gear_location() == legion::common::GearPosition::GEAR_REVERSE)
