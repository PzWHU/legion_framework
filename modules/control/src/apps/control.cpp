...
void Control::PublishControlAnalysis(
    legion::interface::ControlAnalysis control_analysis) {
#if LCM_ENABLE
...
void Control::PublishEvents(legion::interface::Events events) {
#if LCM_ENABLE
...
void Control::HandleADCTrajectory(
...

  {
    ...
    if (control_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = adc_trajectory.header();
    ...
    if (control_conf_->use_adctrajectory_system_timestamp_enable() == true) {
      adc_trajectory_ = adc_trajectory;
      legion::interface::Header header = adc_trajectory_.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Control::HandleChassis(legion::interface::Chassis chassis) {
  if (is_init_ == false) {
    return;
  }
  {
...
// void Control::HandleVehicle(legion::interface::Vehicle vehicle) {
//   if (is_init_ == false) {
//     return;
//   }
//   {
...
//     std::lock_guard<std::mutex> lock(mutex_);
//     if (control_conf_->use_system_timestamp() == true) {
//       legion::interface::Header header = obstacle_list.header();
//       header.set_stamp(TimeTool::Now2TmeStruct());
...
void Control::HandlePlanningCmd(legion::interface::PlanningCmd planning_cmd) {
  if (is_init_ == false) {
    return;
  }

  {
    std::lock_guard<std::mutex> lock(mutex_);
    if (control_conf_->use_system_timestamp() == true) {
      legion::interface::Header header = planning_cmd.header();
      header.set_stamp(TimeTool::Now2TmeStruct());
...
void Control::HandleLocation(legion::interface::Location location) {
  if (is_init_ == false) {
    return;
  }
  {
    ...
    legion::interface::Pose pose;

    legion::interface::PointENU position;
    ...
    legion::interface::Quaternion orientation;
...
void Control::ComputeControlCommandOnTimer() {
  ...
  if (control_cmd_.accel_driving_mode() ==
    ...
    control_cmd_ = legion::interface::ControlCommand();
    control_analysis_ = legion::interface::ControlAnalysis();
    trajectory_initialized_ = false;
  }
}
...
void Control::Estop() {
  ...
  control_cmd_.set_gear_location(legion::common::GEAR_NEUTRAL);
  AWARN << estop_reason_;
}
...
Status Control::CheckTimestamp(const LocalView& local_view) {
  ...
  if (localization_diff > (control_conf_->max_localization_miss_num() *
    ...
    // monitor_logger_buffer_.ERROR("legion::interface::LocalizationEstimate msg
    // lost");
  ...
  if (chassis_diff > (control_conf_->max_chassis_miss_num() *
    ...
    AERROR << "Chassis msg lost for " << std::fixed << std::setprecision(2)
           << chassis_diff << "s";
    // monitor_logger_buffer_.ERROR("legion::common msg lost");
    return Status(Status::ErrorCode::CONTROL_COMPUTE_ERROR,
                  "Chassis msg timeout");
  }
...
void Control::SetBcmControlCmd(legion::interface::ControlCommand* control_cmd,
                               legion::interface::PlanningCmd planning_cmd) {
  control_cmd->set_turn_lamp_ctrl(
      (legion::common::TurnSignal)planning_cmd.turn_lamp_ctrl());
  control_cmd->set_high_beam_ctrl(
      (legion::common::SwitchStatus)planning_cmd.high_beam_ctrl());
  control_cmd->set_low_beam_ctrl(
      (legion::common::SwitchStatus)planning_cmd.low_beam_ctrl());
  control_cmd->set_front_wiper_ctrl(
      (legion::common::SwitchStatus)planning_cmd.front_wiper_ctrl());
  control_cmd->set_rear_wiper_ctrl(
      (legion::common::SwitchStatus)planning_cmd.rear_wiper_ctrl());
  control_cmd->set_position_lamp_ctrl(
      (legion::common::SwitchStatus)planning_cmd.position_lamp_ctrl());
  control_cmd->set_front_fog_lamp_ctrl(
      (legion::common::SwitchStatus)planning_cmd.front_fog_lamp_ctrl());
  control_cmd->set_rear_fog_lamp_ctrl(
      (legion::common::SwitchStatus)planning_cmd.rear_fog_lamp_ctrl());
  // control_cmd->set_brake_lamp_ctrl(
  //     (legion::common::SwitchStatus)planning_cmd.brake_lamp_ctrl());
  control_cmd->set_alarm_lamp_ctrl(
      (legion::common::SwitchStatus)planning_cmd.alarm_lamp_ctrl());
  control_cmd->set_lf_door_ctrl(
      (legion::common::DoorStatus)planning_cmd.lf_door_ctrl());
  control_cmd->set_rf_door_ctrl(
      (legion::common::DoorStatus)planning_cmd.rf_door_ctrl());
  control_cmd->set_lr_door_ctrl(
      (legion::common::DoorStatus)planning_cmd.lr_door_ctrl());
  control_cmd->set_rr_door_ctrl(
      (legion::common::DoorStatus)planning_cmd.rr_door_ctrl());
}
...
//           legion::interface::Obstacle::FusionType::CAMERA) {
//         // ?????????
//         if (center_point.x() <=
//             -control_conf_->vehicle_param().back_edge_to_center())
//           continue;
...
//       if (obstacle[i].fusion_type() ==
//           legion::interface::Obstacle::FusionType::ULTRASONIC) {
