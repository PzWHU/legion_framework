...
void DigitalFilter::set_dead_zone(const double deadzone) {
  ...
  std::cout << "Setting digital filter dead zone = " << dead_zone_;
}

double DigitalFilter::Filter(const double x_insert) {
  if (denominators_.empty() || numerators_.empty() || std::isnan(x_insert)) {
    std::cout << "DigitalFilter empty denominators or numerators error"
              << std::endl;
    return 0.0;
  }
