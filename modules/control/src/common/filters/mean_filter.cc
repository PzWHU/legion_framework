...
MF::MeanFilter(const uint8 window_size) : window_size_(window_size) {
  ...
  if (window_size_ < 0) {
    std::cout << "MeanFilter() window_size_ < 0" << endl;
    return;
  }
  // CHECK_LE(window_size_, kMaxWindowSize);
  if (window_size_ > kMaxWindowSize) {
    std::cout << "MeanFilter() window_size_ > kMaxWindowSize" << endl;
    return;
  }
  initialized_ = true;
}
...
double MF::Update(const double measurement) {
  ...
  if (initialized_ == false) {
    std::cout << "Update() initialized_ == false" << endl;
    return 0.0;
  }
  // CHECK_LE(values_.size(), window_size_);
  if (values_.size() > window_size_) {
    std::cout << "Update() values_.size() > window_size_e" << endl;
    return 0.0;
  }
  // CHECK_LE(min_candidates_.size(), window_size_);
  if (min_candidates_.size() > window_size_) {
    std::cout << "Update() min_candidates_.size() > window_size_e" << endl;
    return 0.0;
  }
  // CHECK_LE(max_candidates_.size(), window_size_);
  if (max_candidates_.size() > window_size_) {
    std::cout << "Update() max_candidates_.size() > window_size_e" << endl;
    return 0.0;
  }
  ++time_;
...
bool MF::ShouldPopOldestCandidate(const uint8 old_time) const {
  if (old_time < window_size_) {
    ...
    if (time_ > old_time + window_size_) {
      std::cout << "ShouldPopOldestCandidate() time_ > old_time + window_size_"
                << endl;
      return 0.0;
    }
    ...
    if (old_time < time_ + window_size_) {
      std::cout << "ShouldPopOldestCandidate() old_time < time_ + window_size_"
                << endl;
      return 0.0;
    }
...
void MF::RemoveEarliest() {
  ...
  if (values_.size() != window_size_) {
    std::cout << "RemoveEarliest() values_.size() != window_size_" << endl;
    return;
  }
