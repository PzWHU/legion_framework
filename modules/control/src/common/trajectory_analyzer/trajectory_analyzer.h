...
class TrajectoryAnalyzer {
 public:
   ...
   */
  legion::interface::TrajectoryPoint QueryNearestPointByAbsoluteTime(
   ...
   * @return a point of trajectory
   */
  legion::interface::TrajectoryPoint QueryNearestPointByRelativeTime(
      const double t, bool query_forward_time_point_only) const;
   ...
   * @return a point of trajectory
   */
  legion::interface::TrajectoryPoint QueryNearestPointByPosition(const double x,
...
size_t QueryNearestPointIndexByPosition(const double x, const double y) const;
   ...
   */
  legion::interface::PathPoint QueryMatchedPathPoint(const double x, const double y) const;
  ...
  void ToTrajectoryFrame(const double x, const double y, const double theta,
                         const double v, const legion::interface::PathPoint &matched_point,
   ...
   * @param rear_to_com_distance Distance from rear wheels to
   *        the vehicle's center of mass.
   * @param path_point legion::interface::PathPoint along the published planning
   * trajectory.
   * @return The position of the vehicle's center of mass.
   */
  common::math::Vec2d ComputeCOMPosition(
      const double rear_to_com_distance,
      const legion::interface::PathPoint &path_point) const;

  /**
   * @brief get all points of the trajectory
   * @return a vector of trajectory points
   */
  const std::vector<legion::interface::TrajectoryPoint> &trajectory_points() const;

 private:
  legion::interface::PathPoint FindMinDistancePoint(const legion::interface::TrajectoryPoint &p0,
                                         const legion::interface::TrajectoryPoint &p1,
                                         const double x, const double y) const;
