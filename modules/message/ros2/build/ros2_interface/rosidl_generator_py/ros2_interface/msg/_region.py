# generated from rosidl_generator_py/resource/_idl.py.em
# with input from ros2_interface:msg/Region.idl
# generated code does not contain a copyright notice


# Import statements for member types

import builtins  # noqa: E402, I100

import math  # noqa: E402, I100

import rosidl_parser.definition  # noqa: E402, I100


class Metaclass_Region(type):
    """Metaclass of message 'Region'."""

    _CREATE_ROS_MESSAGE = None
    _CONVERT_FROM_PY = None
    _CONVERT_TO_PY = None
    _DESTROY_ROS_MESSAGE = None
    _TYPE_SUPPORT = None

    __constants = {
    }

    @classmethod
    def __import_type_support__(cls):
        try:
            from rosidl_generator_py import import_type_support
            module = import_type_support('ros2_interface')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'ros2_interface.msg.Region')
            logger.debug(
                'Failed to import needed modules for type support:\n' +
                traceback.format_exc())
        else:
            cls._CREATE_ROS_MESSAGE = module.create_ros_message_msg__msg__region
            cls._CONVERT_FROM_PY = module.convert_from_py_msg__msg__region
            cls._CONVERT_TO_PY = module.convert_to_py_msg__msg__region
            cls._TYPE_SUPPORT = module.type_support_msg__msg__region
            cls._DESTROY_ROS_MESSAGE = module.destroy_ros_message_msg__msg__region

            from ros2_interface.msg import Point3D
            if Point3D.__class__._TYPE_SUPPORT is None:
                Point3D.__class__.__import_type_support__()

    @classmethod
    def __prepare__(cls, name, bases, **kwargs):
        # list constant names here so that they appear in the help text of
        # the message class under "Data and other attributes defined here:"
        # as well as populate each message instance
        return {
        }


class Region(metaclass=Metaclass_Region):
    """Message class 'Region'."""

    __slots__ = [
        '_name_region',
        '_score',
        '_rank_risk',
        '_region_polygon',
    ]

    _fields_and_field_types = {
        'name_region': 'int32',
        'score': 'double',
        'rank_risk': 'int32',
        'region_polygon': 'sequence<ros2_interface/Point3D>',
    }

    SLOT_TYPES = (
        rosidl_parser.definition.BasicType('int32'),  # noqa: E501
        rosidl_parser.definition.BasicType('double'),  # noqa: E501
        rosidl_parser.definition.BasicType('int32'),  # noqa: E501
        rosidl_parser.definition.UnboundedSequence(rosidl_parser.definition.NamespacedType(['ros2_interface', 'msg'], 'Point3D')),  # noqa: E501
    )

    def __init__(self, **kwargs):
        assert all('_' + key in self.__slots__ for key in kwargs.keys()), \
            'Invalid arguments passed to constructor: %s' % \
            ', '.join(sorted(k for k in kwargs.keys() if '_' + k not in self.__slots__))
        self.name_region = kwargs.get('name_region', int())
        self.score = kwargs.get('score', float())
        self.rank_risk = kwargs.get('rank_risk', int())
        self.region_polygon = kwargs.get('region_polygon', [])

    def __repr__(self):
        typename = self.__class__.__module__.split('.')
        typename.pop()
        typename.append(self.__class__.__name__)
        args = []
        for s, t in zip(self.__slots__, self.SLOT_TYPES):
            field = getattr(self, s)
            fieldstr = repr(field)
            # We use Python array type for fields that can be directly stored
            # in them, and "normal" sequences for everything else.  If it is
            # a type that we store in an array, strip off the 'array' portion.
            if (
                isinstance(t, rosidl_parser.definition.AbstractSequence) and
                isinstance(t.value_type, rosidl_parser.definition.BasicType) and
                t.value_type.typename in ['float', 'double', 'int8', 'uint8', 'int16', 'uint16', 'int32', 'uint32', 'int64', 'uint64']
            ):
                if len(field) == 0:
                    fieldstr = '[]'
                else:
                    assert fieldstr.startswith('array(')
                    prefix = "array('X', "
                    suffix = ')'
                    fieldstr = fieldstr[len(prefix):-len(suffix)]
            args.append(s[1:] + '=' + fieldstr)
        return '%s(%s)' % ('.'.join(typename), ', '.join(args))

    def __eq__(self, other):
        if not isinstance(other, self.__class__):
            return False
        if self.name_region != other.name_region:
            return False
        if self.score != other.score:
            return False
        if self.rank_risk != other.rank_risk:
            return False
        if self.region_polygon != other.region_polygon:
            return False
        return True

    @classmethod
    def get_fields_and_field_types(cls):
        from copy import copy
        return copy(cls._fields_and_field_types)

    @builtins.property
    def name_region(self):
        """Message field 'name_region'."""
        return self._name_region

    @name_region.setter
    def name_region(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'name_region' field must be of type 'int'"
            assert value >= -2147483648 and value < 2147483648, \
                "The 'name_region' field must be an integer in [-2147483648, 2147483647]"
        self._name_region = value

    @builtins.property
    def score(self):
        """Message field 'score'."""
        return self._score

    @score.setter
    def score(self, value):
        if __debug__:
            assert \
                isinstance(value, float), \
                "The 'score' field must be of type 'float'"
            assert not (value < -1.7976931348623157e+308 or value > 1.7976931348623157e+308) or math.isinf(value), \
                "The 'score' field must be a double in [-1.7976931348623157e+308, 1.7976931348623157e+308]"
        self._score = value

    @builtins.property
    def rank_risk(self):
        """Message field 'rank_risk'."""
        return self._rank_risk

    @rank_risk.setter
    def rank_risk(self, value):
        if __debug__:
            assert \
                isinstance(value, int), \
                "The 'rank_risk' field must be of type 'int'"
            assert value >= -2147483648 and value < 2147483648, \
                "The 'rank_risk' field must be an integer in [-2147483648, 2147483647]"
        self._rank_risk = value

    @builtins.property
    def region_polygon(self):
        """Message field 'region_polygon'."""
        return self._region_polygon

    @region_polygon.setter
    def region_polygon(self, value):
        if __debug__:
            from ros2_interface.msg import Point3D
            from collections.abc import Sequence
            from collections.abc import Set
            from collections import UserList
            from collections import UserString
            assert \
                ((isinstance(value, Sequence) or
                  isinstance(value, Set) or
                  isinstance(value, UserList)) and
                 not isinstance(value, str) and
                 not isinstance(value, UserString) and
                 all(isinstance(v, Point3D) for v in value) and
                 True), \
                "The 'region_polygon' field must be a set or sequence and each value of type 'Point3D'"
        self._region_polygon = value
