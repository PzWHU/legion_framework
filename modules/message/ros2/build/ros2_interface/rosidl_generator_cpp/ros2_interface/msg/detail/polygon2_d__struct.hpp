// generated from rosidl_generator_cpp/resource/idl__struct.hpp.em
// with input from ros2_interface:msg/Polygon2D.idl
// generated code does not contain a copyright notice

#ifndef ROS2_INTERFACE__MSG__DETAIL__POLYGON2_D__STRUCT_HPP_
#define ROS2_INTERFACE__MSG__DETAIL__POLYGON2_D__STRUCT_HPP_

#include <algorithm>
#include <array>
#include <memory>
#include <string>
#include <vector>

#include "rosidl_runtime_cpp/bounded_vector.hpp"
#include "rosidl_runtime_cpp/message_initialization.hpp"


// Include directives for member types
// Member 'points'
#include "ros2_interface/msg/detail/point2_d__struct.hpp"

#ifndef _WIN32
# define DEPRECATED__ros2_interface__msg__Polygon2D __attribute__((deprecated))
#else
# define DEPRECATED__ros2_interface__msg__Polygon2D __declspec(deprecated)
#endif

namespace ros2_interface
{

namespace msg
{

// message struct
template<class ContainerAllocator>
struct Polygon2D_
{
  using Type = Polygon2D_<ContainerAllocator>;

  explicit Polygon2D_(rosidl_runtime_cpp::MessageInitialization _init = rosidl_runtime_cpp::MessageInitialization::ALL)
  {
    if (rosidl_runtime_cpp::MessageInitialization::ALL == _init ||
      rosidl_runtime_cpp::MessageInitialization::ZERO == _init)
    {
      this->coordinate_system = 0l;
    }
  }

  explicit Polygon2D_(const ContainerAllocator & _alloc, rosidl_runtime_cpp::MessageInitialization _init = rosidl_runtime_cpp::MessageInitialization::ALL)
  {
    (void)_alloc;
    if (rosidl_runtime_cpp::MessageInitialization::ALL == _init ||
      rosidl_runtime_cpp::MessageInitialization::ZERO == _init)
    {
      this->coordinate_system = 0l;
    }
  }

  // field types and members
  using _coordinate_system_type =
    int32_t;
  _coordinate_system_type coordinate_system;
  using _points_type =
    std::vector<ros2_interface::msg::Point2D_<ContainerAllocator>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<ros2_interface::msg::Point2D_<ContainerAllocator>>>;
  _points_type points;

  // setters for named parameter idiom
  Type & set__coordinate_system(
    const int32_t & _arg)
  {
    this->coordinate_system = _arg;
    return *this;
  }
  Type & set__points(
    const std::vector<ros2_interface::msg::Point2D_<ContainerAllocator>, typename std::allocator_traits<ContainerAllocator>::template rebind_alloc<ros2_interface::msg::Point2D_<ContainerAllocator>>> & _arg)
  {
    this->points = _arg;
    return *this;
  }

  // constant declarations

  // pointer types
  using RawPtr =
    ros2_interface::msg::Polygon2D_<ContainerAllocator> *;
  using ConstRawPtr =
    const ros2_interface::msg::Polygon2D_<ContainerAllocator> *;
  using SharedPtr =
    std::shared_ptr<ros2_interface::msg::Polygon2D_<ContainerAllocator>>;
  using ConstSharedPtr =
    std::shared_ptr<ros2_interface::msg::Polygon2D_<ContainerAllocator> const>;

  template<typename Deleter = std::default_delete<
      ros2_interface::msg::Polygon2D_<ContainerAllocator>>>
  using UniquePtrWithDeleter =
    std::unique_ptr<ros2_interface::msg::Polygon2D_<ContainerAllocator>, Deleter>;

  using UniquePtr = UniquePtrWithDeleter<>;

  template<typename Deleter = std::default_delete<
      ros2_interface::msg::Polygon2D_<ContainerAllocator>>>
  using ConstUniquePtrWithDeleter =
    std::unique_ptr<ros2_interface::msg::Polygon2D_<ContainerAllocator> const, Deleter>;
  using ConstUniquePtr = ConstUniquePtrWithDeleter<>;

  using WeakPtr =
    std::weak_ptr<ros2_interface::msg::Polygon2D_<ContainerAllocator>>;
  using ConstWeakPtr =
    std::weak_ptr<ros2_interface::msg::Polygon2D_<ContainerAllocator> const>;

  // pointer types similar to ROS 1, use SharedPtr / ConstSharedPtr instead
  // NOTE: Can't use 'using' here because GNU C++ can't parse attributes properly
  typedef DEPRECATED__ros2_interface__msg__Polygon2D
    std::shared_ptr<ros2_interface::msg::Polygon2D_<ContainerAllocator>>
    Ptr;
  typedef DEPRECATED__ros2_interface__msg__Polygon2D
    std::shared_ptr<ros2_interface::msg::Polygon2D_<ContainerAllocator> const>
    ConstPtr;

  // comparison operators
  bool operator==(const Polygon2D_ & other) const
  {
    if (this->coordinate_system != other.coordinate_system) {
      return false;
    }
    if (this->points != other.points) {
      return false;
    }
    return true;
  }
  bool operator!=(const Polygon2D_ & other) const
  {
    return !this->operator==(other);
  }
};  // struct Polygon2D_

// alias to use template instance with default allocator
using Polygon2D =
  ros2_interface::msg::Polygon2D_<std::allocator<void>>;

// constant definitions

}  // namespace msg

}  // namespace ros2_interface

#endif  // ROS2_INTERFACE__MSG__DETAIL__POLYGON2_D__STRUCT_HPP_
