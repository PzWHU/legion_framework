...
# Loop for the subscriber
    ...
    # Create a publisher on the topic
    try:
        c2_od_obstacle_list_sub = fastdds_obstacle_list.ObstacleListDataReader(0, "rt/perception/camera/camera_2d_object_detect/C2ODObstacleList")
        c3_od_obstacle_list_sub = fastdds_obstacle_list.ObstacleListDataReader(0, "rt/perception/camera/camera_3d_object_detect/C3ODObstacleList")
    ...
    c2_od_obstacle_list = fastdds_obstacle_list.ObstacleList()
    c3_od_obstacle_list = fastdds_obstacle_list.ObstacleList()

    try:
        while True:
            # Wait for a message for 2 seconds
            if (c2_od_obstacle_list_sub.wait_for_sample(2)):
                # Read the received message
                if (c2_od_obstacle_list_sub.take_sample(c2_od_obstacle_list)):
                    print("success")
            ...
            # Wait for a message for 2 seconds
            if (c3_od_obstacle_list_sub.wait_for_sample(2)):
                # Read the received message
                if (c3_od_obstacle_list_sub.take_sample(c3_od_obstacle_list)):
                    print("success")
