%module fastdds_planning_analysis 
%{
...
#include "PlanningAnalysisPubSubTypes.h"
...
namespace std {
    %template(VectorTimeConsume) vector<ros2_interface::msg::TimeConsume>;
}
...
namespace msg{
    ...
    public:
        ...
        void frame_update_time(
            double _frame_update_time);
        double frame_update_time() const;
        void generator_time(
            double _generator_time);
        double generator_time() const;
        void evaluator_time(
            const std::vector<ros2_interface::msg::TimeConsume> &_evaluator_time);
        const std::vector<ros2_interface::msg::TimeConsume>& evaluator_time() const;
        void selector_time(
            double _selector_time);
        double selector_time() const;
        void planning_parking_debug(
                const ros2_interface::msg::PlanningParkingDebug &_planning_parking_debug);
        const ros2_interface::msg::PlanningParkingDebug &planning_parking_debug() const;
        void dis_to_center_line(
            double _dis_to_center_line);
        double dis_to_center_line() const;
        void diff_to_velocity(
            double _diff_to_velocity);
        double diff_to_velocity() const;
        void referencline_kappa(
            double _referencline_kappa);
        double referencline_kappa() const;
    };
    ...
    class PlanningAnalysisPubSubType
    {
    public:
        typedef PlanningAnalysis type;

        PlanningAnalysisPubSubType();
        ~PlanningAnalysisPubSubType();
    };
...
%template(PlanningAnalysisDataWriter) FastddsDataWriter<ros2_interface::msg::PlanningAnalysisPubSubType>;
%template(PlanningAnalysisDataReader) FastddsDataReader<ros2_interface::msg::PlanningAnalysisPubSubType>;
